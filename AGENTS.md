...
> 이 절은 사용자가 다시 변경하기 전까지 아래의 Goal graph, 시작 gate, checkpoint,
> 자동화·통제 산출물 갱신 지침보다 우선한다.

- 현재 단계의 목표는 사용자 기능을 먼저 완성하는 것이다.
- 일반 기능 작업에서는 Goal graph event, gate receipt, canonical transaction,
  hash snapshot, 전용 builder/checker, 통제·자동화 문서를 새로 만들거나 확장하지 않는다.
- 기존 continuation·Goal graph·checkpoint 불일치만으로 기능 구현을 막지 않는다.
  해당 문제는 필요하면 별도 위험으로 짧게 보고하되, 사용자가 명시적으로 요청하지
  않는 한 복구 작업으로 범위를 넓히지 않는다.
- 작업 순서는 `관련 기능 코드 확인 → 최소 구현 → 기능 직접 테스트`로 단순하게 유지한다.
  전체 저장소 gate 대신 변경 기능과 직접 관련된 테스트를 우선한다.
- 기존 통제 자료는 삭제하거나 현재 정책으로 다시 쓰지 않고 참고 자료로만 보존한다.
- 기능 구현이 모두 끝난 뒤 사용자가 요청하면 자동화, 통제, 추적성, 전체 gate를 한 번에
  정리하고 검증한다.
- 이 기간에는 daylog·local-memory·checkpoint 갱신을 일반 기능 완료조건으로 삼지 않는다.
  기능과 직접 관련 없는 기록 작업 때문에 구현 범위를 늘리거나 완료를 지연하지 않는다.
