"""Legacy v1 YOLO ``.pt`` adapter and model readiness checks.

The adapter loads lazily, validates the model class order against the v1 API
contract, and returns normalized report metadata. WalkSafe v2 provider
selection lives in ``backend.app.services.detect_v2``.
"""
...
# Cache identity includes artifact metadata so replacing a weight file in place
# invalidates the process-local adapter without loading it on every health call.
_ADAPTER_LOCK = Lock()
_ADAPTER_CACHE_STAT_KEY: Optional[tuple[object, ...]] = None
_ADAPTER_CACHE_KEY: Optional[tuple[object, ...]] = None
...
def _reset_adapter_cache() -> None:
    global _ADAPTER_CACHE_STAT_KEY, _ADAPTER_CACHE_KEY, _ADAPTER_LOAD_RESULT
    with _ADAPTER_LOCK:
        _ADAPTER_CACHE_STAT_KEY = None
...
def _health_response(reason: Optional[str], model_status: str = "unavailable") -> DetectHealthResponse:
    ...
    model_artifact_path = settings.model_artifact_path.name if settings.model_artifact_path else None
...
async def run_detection(
    ...
    *,
    ...
    return await to_thread.run_sync(
        lambda: _run_detection_sync(image_bytes, content_type, context),
        limiter=limiter,
    )


def _run_detection_sync(image_bytes: bytes, content_type: str, context: DetectContext) -> DetectResponse:
    settings = get_settings()
    load_result = _adapter_load_result(settings)
...
def _adapter_load_result(settings: Any) -> AdapterLoadResult:
    ...
    global _ADAPTER_CACHE_STAT_KEY, _ADAPTER_CACHE_KEY, _ADAPTER_LOAD_RESULT
    with _ADAPTER_LOCK:
        if _ADAPTER_CACHE_STAT_KEY == stat_key and _ADAPTER_LOAD_RESULT is not None:
    ...
    with model_path.open("rb") as stream:
        for chunk in iter(lambda: stream.read(1024 * 1024), b""):
            digest.update(chunk)
    cache_key = (*stat_key, digest.hexdigest())
    ...
    with _ADAPTER_LOCK:
        ...
        _ADAPTER_CACHE_STAT_KEY = stat_key
...
def _compatible_class_id_map(
    ...
    # Historical tactile checkpoints can legitimately expose only the first v1
    # class. Require model or training metadata to prove that mapping.
