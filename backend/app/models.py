...
class FirstRunSignup(Base):
    """FP-010 stage 4~8 credential lifecycle for one first-run signup.

    Email stands in for phone verification until release; see
    product/decisions.md, 2026-08-30. The address is never stored, only its
    keyed pseudonym, and both opaque handles are random rather than derived
    from anything the user typed.
    """

    __tablename__ = "first_run_signups"
    __table_args__ = (
        CheckConstraint(
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    email_hmac = Column(String(64), nullable=False, unique=True, index=True)
    submission_handle = Column(String(36), nullable=False, unique=True, index=True)
    verification_code_sha256 = Column(String(64), nullable=False)
    ...
    actor_binding = Column(String(38), nullable=True, unique=True, index=True)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
