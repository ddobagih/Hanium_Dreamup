"""Serialize persisted reports and derive administrator review signals.

Review flags are recomputed from database columns plus allowlisted conventions
inside the stored metadata payload. They help triage CSV candidates but do not
approve, merge, or submit a report to an external institution.
"""
...
def location_quality(report: Report) -> LocationQuality:
    if report.latitude is None or report.longitude is None:
        return "missing"
    if not math.isfinite(report.latitude) or not math.isfinite(report.longitude):
        return "missing"
    if report.accuracy_m is None or report.accuracy_m > MEDIUM_LOCATION_ACCURACY_M:
        return "low"
    if not math.isfinite(report.accuracy_m) or report.accuracy_m < 0:
        return "low"
...
def report_to_response(report: Report, duplicate_report_ids: Optional[Sequence[str]] = None) -> ReportResponse:
    ...
    if (
        ...
        and math.isfinite(report.latitude)
        and math.isfinite(report.longitude)
    ):
        gps = {
            ...
            "accuracy_m": (
                ...
                and math.isfinite(report.accuracy_m)
                and report.accuracy_m >= 0
                else None
            ),
        }
    ...
    return ReportResponse(
        ...
        gps=gps,
        heading=report.heading if report.heading is not None and math.isfinite(report.heading) else None,
        ...
        duplicate_count=len(resolved_duplicate_report_ids),
