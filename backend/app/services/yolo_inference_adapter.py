...
def load_yolo_model(
    ...
    **kwargs: Any,
    ...
    ``artifact_identity`` participates in the cache key but is deliberately not
    passed to Ultralytics. Replacing a checkpoint at the same path therefore
    cannot return the previously cached model object.
    """
...
def _box_class_id(box: Any) -> int | None:
    ...
    if value is None or not isfinite(value) or value < 0 or not value.is_integer():
        return None
    return int(value)


def _box_confidence(box: Any) -> float | None:
    value = _first_number(getattr(box, "conf", None))
