...
def test_model_cache_identity_invalidates_same_path_replacement(monkeypatch) -> None:
    loaded: list[object] = []
    fake_ultralytics = ModuleType("ultralytics")

    class FakeYolo:
        def __init__(self, model_path: str | None, **kwargs) -> None:  # noqa: ANN003
    ...
    first = load_yolo_model("same.pt", artifact_identity="a" * 64)
    reused = load_yolo_model("same.pt", artifact_identity="a" * 64)
    replaced = load_yolo_model("same.pt", artifact_identity="b" * 64)
...
@pytest.mark.parametrize("invalid_class_id", [0.5, float("nan"), float("inf"), float("-inf")])
def test_non_integral_or_non_finite_class_id_is_rejected(invalid_class_id: float) -> None:
    result = FakeResult(
        boxes=[FakeBox(cls=[invalid_class_id], conf=[0.9], xyxyn=[[0.1, 0.1, 0.2, 0.2]])]
    )

    assert ultralytics_result_to_raw_detections(result, model_key="custom_tactile") == []
...
def test_non_finite_or_out_of_range_confidence_is_rejected(invalid_confidence: float) -> None:
    result = FakeResult(
        boxes=[FakeBox(cls=[0], conf=[invalid_confidence], xyxyn=[[0.1, 0.1, 0.2, 0.2]])]
    )
...
def test_non_finite_bbox_coordinate_is_rejected(
    invalid_coordinate: float,
    coordinate_index: int,
) -> None:
    bbox = [0.1, 0.1, 0.2, 0.2]
    bbox[coordinate_index] = invalid_coordinate
    result = FakeResult(boxes=[FakeBox(cls=[0], conf=[0.9], xyxyn=[bbox])])

    assert ultralytics_result_to_raw_detections(result, model_key="custom_tactile") == []


def test_integral_class_id_confidence_bounds_and_finite_bbox_clipping_remain_supported() -> None:
    result = FakeResult(
        boxes=[
            FakeBox(cls=[0.0], conf=[0.0], xyxyn=[[-0.1, 0.1, 0.2, 0.3]]),
            FakeBox(cls=[1.0], conf=[1.0], xyxyn=[[0.8, 0.7, 1.2, 1.1]]),
        ]
    )

    detections = ultralytics_result_to_raw_detections(result, model_key="custom_tactile")

    assert [detection["model_class_id"] for detection in detections] == [0, 1]
    assert [detection["confidence"] for detection in detections] == [0.0, 1.0]
