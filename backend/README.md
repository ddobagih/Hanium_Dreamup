...
8. `walksafe-backend-migrate.service`를 수동 실행해 성공과 예상 Alembic head `202609010002`를 확인한다. migration 완료부터 issuer bind 성공 전까지 새 backend의 startup/readiness가 fail-closed하는 것이 정상이며 API를 시작하지 않는다. migration이 실패해도 API와 ingress를 정지한 채 원인을 해결하며 구 backend를 새 schema 위에서 시작하지 않는다.
