...
def test_general_test_dependency_lock_matches_source_and_backend_lock() -> None:
    ...
    for name in backend_versions.keys() & test_versions.keys():
        assert backend_versions[name] == test_versions[name]
...
def test_layer_runner_migrates_the_verified_test_database_before_tests() -> None:
    runner = Path("scripts/run_walksafe_test_layers_20260711.sh").read_text(encoding="utf-8")
...
def test_layer_runner_assigns_model_runtime_pytest_to_unit(tmp_path: Path) -> None:
    ...
    assert runner.count("-p no:cacheprovider") == 3
    ...
    completed = subprocess.run(
        ["bash", "scripts/run_walksafe_test_layers_20260711.sh", "unit"],
...
def test_ambient_database_ready_flag_cannot_bypass_preflight(layer: str) -> None:
    ...
    completed = subprocess.run(
        ["bash", "scripts/run_walksafe_test_layers_20260711.sh", layer],
        cwd=Path(__file__).resolve().parents[1],
        env=environment,
        capture_output=True,
        text=True,
