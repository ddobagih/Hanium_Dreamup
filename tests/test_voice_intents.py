...
@pytest.mark.parametrize("transcript", ["목적지 취소", "목적지 삭제", "목적지 없애줘", "목적지 지워줘"])
def test_cancel_destination_korean_variants(transcript):
    result = classify_intent(transcript)

    assert result.intent == "cancel_destination"
    assert result.score >= EXECUTION_SCORE_THRESHOLD
    assert result.slots == {}
...
@pytest.mark.parametrize(
    "transcript",
    ["다음 경로 뭐야?", "다음 길 알려줘", "다음 안내 뭐야", "다음 회전 뭐야"],
)
def test_next_navigation_instruction_korean_variants(transcript):
    result = classify_intent(transcript)

    assert result.intent == "next_navigation_instruction"
    assert result.score >= EXECUTION_SCORE_THRESHOLD
    assert result.slots == {}
...
@pytest.mark.parametrize(
    ("transcript", "destination"),
    [("목적지를 서울역으로 바꿔", "서울역"), ("목적지 시청으로 변경해", "시청")],
)
def test_change_destination_extracts_new_destination(transcript, destination):
    result = classify_intent(transcript)
...
@pytest.mark.parametrize("transcript", ["목적지 변경", "목적지 바꿔줘"])
def test_change_destination_without_name_reprompts(transcript):
    result = classify_intent(transcript)
...
def test_report_information_question_is_not_an_execution_command():
    result = classify_intent("신고 방법 알려줘")
    policy = evaluate_intent_policy(result)
...
def test_plain_do_not_speak_remains_the_explicit_voice_off_command():
    result = classify_intent("말하지 마")

    assert result.intent == "voice_off"
    assert evaluate_intent_policy(result).should_execute is True


def test_low_acoustic_confidence_blocks_known_intent():
    result = classify_intent("신고해")
    policy = evaluate_intent_policy(result, acoustic_allowed=False)
    payload = intent_response_payload(
        result,
        acoustic_confidence=0.2,
        avg_logprob=-1.4,
        no_speech_probability=0.2,
        acoustic_allowed=False,
    )

    assert policy.should_execute is False
    assert policy.reason == "low_acoustic_confidence"
    assert payload["action"] == "reprompt"
    assert payload["acoustic_confidence"] == 0.2
...
@pytest.mark.parametrize(
    ...
    [
        "서울역 말고 시청으로 안내해줘",
        "서울역이 아니라 시청으로 목적지 변경해",
        "서울역 또는 시청으로 안내해줘",
    ],
)
def test_ambiguous_or_corrected_destination_reprompts_instead_of_guessing(transcript):
    result = classify_intent(transcript)
    policy = evaluate_intent_policy(result)
