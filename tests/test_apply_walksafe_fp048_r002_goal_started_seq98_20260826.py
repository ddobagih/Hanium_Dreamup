...
from scripts import apply_walksafe_fp048_r002_goal_started_seq98_20260826 as start
...
def _evidence(source: dict[str, object] | None = None) -> start.GateEvidence:
    ...
    receipt = {
        ...
        "repository_snapshot": {"status": "FROZEN_R009"},
        "generated_at": "2026-08-26T20:00:01+09:00",
    }
    ...
    return start.GateEvidence(
        ...
        event_occurred_at="2026-08-26T20:00:02+09:00",
    )
...
def _projection() -> tuple[dict[str, object], start.GateEvidence, dict[str, object]]:
    ...
    projected, _event = start.project_seq98(
        ROOT,
        source,
        evidence,
...
def _prepared_for_write(tmp_path: Path) -> start.PreparedProjection:
    ...
    source_read = start.frozen_start.seq90._stable_read(
...
def test_seq98_starts_only_fp048_and_preserves_zero_credit() -> None:
    source, evidence, projected = _projection()
...
def test_source_requires_exact_integer_seq97_correction() -> None:
    source = _source()
    source["goal_execution"]["transition_history"][-1]["sequence"] = 97.0
    with pytest.raises(start.StartApplyError, match="seq97 snapshot-hygiene"):
...
def test_private_r009_receipt_and_chronology_are_fail_closed() -> None:
    source = _source()
    evidence = _evidence(source)
    ...
    with pytest.raises(start.StartApplyError, match="private R009 PASS"):
        start.project_seq98(
            ROOT,
            source,
            forged,
    ...
    forged = start.GateEvidence(
        ...
        receipt_binding=evidence.receipt_binding,
        event_occurred_at="2026-08-26T20:00:01+09:00",
    )
    with pytest.raises(start.StartApplyError, match="seq98 chronology"):
        start.project_seq98(
            ROOT,
            source,
            forged,
...
def test_projection_rejects_release_or_unrelated_goal_mutation() -> None:
    ...
    with pytest.raises(start.StartApplyError, match="seq98 projection differs"):
        start.validate_projection(ROOT, source, projected, evidence)


def test_public_seq97_inverse_and_started_validator_are_byte_exact(
    ...
    assert start.reconstructed_seq97_checkpoint_bytes(ROOT, projected) == (
...
def test_seq98_checks_historical_seq97_before_detached_live_r009(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []
    source = _inverse_source()
    ...
    class HistoricalCorrection(_SyntheticCorrection):
        ...
        def require_snapshot_hygiene_corrected_checkpoint(
            ...
            *,
            ...
            assert require_live_snapshot is False
            assert run_external_validators is False
            calls.append("historical_seq97")

        @staticmethod
        def canonical_seq97_checkpoint_bytes(
            _root: Path, checkpoint: dict[str, object]
        ) -> bytes:
            calls.append("canonical_seq97")
            return start.checkpoint_bytes(checkpoint)
    ...
    def published_gate(*_args: object, **_kwargs: object) -> start.GateEvidence:
        calls.append("live_r009_receipt")
        return evidence
...
def test_public_seq97_inverse_rejects_tampered_snapshot_preimage(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = _inverse_source()
    evidence = _evidence(source)
    ...
    with pytest.raises(start.StartApplyError, match="snapshot seal"):
        start.reconstructed_seq97_checkpoint_bytes(ROOT, projected)


@pytest.mark.parametrize("tampered_name", ["correction.py", "review.json"])
def test_seq97_physical_authority_rejects_producer_or_review_tamper(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    tampered_name: str,
) -> None:
    ...
    with pytest.raises(start.StartApplyError, match="physical authority"):
        start.reconstructed_seq97_checkpoint_bytes(tmp_path, projected)
...
def test_stale_same_bytes_new_inode_is_rejected_before_writer(
    ...
    with pytest.raises(start.StartApplyError, match="seq97 source changed"):
        start.write_projection(prepared, writer=writer)
...
def test_terminal_failure_after_replace_is_postcommit_uncertain(
    ...
    with pytest.raises(start.frozen_start.seq90.PostcommitUncertain):
        start.write_projection(prepared, writer=writer)
