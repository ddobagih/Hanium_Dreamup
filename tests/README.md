...
| `test_walksafe_project_continuation_v2_4.py` | v2.3·v2.4 선행 경계부터 현재 NPC seq60 `GOAL_STARTED`/`IN_PROGRESS`까지의 transition lifecycle을 검증하는 현재 singleton. `active-session-control`은 이 테스트 전에 strict continuation checker로 현재 checkpoint와 managed bytes를 먼저 확인함 |
| `test_walksafe_npc_single_admin_recovery_goal_seq56_57_20260810.py`, `test_apply_walksafe_npc_single_admin_recovery_goal_seq56_57_20260810.py`, `test_walksafe_npc_single_admin_recovery_start_gate_contract_r002_20260812.py`, `test_apply_walksafe_npc_goal_start_control_reanchor_seq58_20260812.py`, `test_run_walksafe_npc_single_admin_recovery_goal_start_gate_20260812.py`, `test_apply_walksafe_npc_goal_start_control_correction_seq59_20260812.py`, `test_apply_walksafe_npc_single_admin_recovery_goal_started_seq60_20260812.py` | NPC materialization·R002·zero-credit reanchor/correction·8-check gate·seq60 start를 fixture 기반으로 검증하는 현재 단위 계층. 실제 저장소 write, 제품 구현 완료, 정식 시험·기기·외부·출시 credit를 만들지 않음 |
| `test_apply_walksafe_npc_single_admin_recovery_goal_started_seq59_20260812.py` | add-only correction 전에 투영됐으나 publication 전에 seq60으로 대체된 seq59 start 경계를 재현하는 역사 제어 검사. 현재 checkpoint 전이로 사용하지 않음 |
| `test_run_walksafe_npc_single_admin_recovery_verification_20260813.py`, `test_build_walksafe_npc_single_admin_recovery_trace_20260812.py`, `test_build_walksafe_npc_single_admin_recovery_gap_backlog_r026_20260812.py`, `test_build_walksafe_npc_single_admin_recovery_gap_backlog_r027_20260813.py`, `test_build_walksafe_npc_single_admin_recovery_artifact_trace_successor_20260812.py`, `test_build_walksafe_npc_single_admin_recovery_strict_review_gate_20260812.py`, `test_build_walksafe_phase1_exact257_successor_r015_20260812.py`, `test_apply_walksafe_npc_single_admin_recovery_goal_completed_seq61_62_20260812.py` | NPC 4-lane observation과 completion·R027 보정 도구의 결정적 projection, R026 exact-byte add-only 선행자, 거부 R001·v2 result 결속, zero-credit/fail-closed 조건을 검증한다. 실제 DB·Gradle 제품 observation과 별도 검수 없이 `GOAL_COMPLETED`를 주장하지 않음 |
...
| `test_walksafe_android_gateway_ingress_current.py` | 현행 Gateway OpenAPI·제품 경계·nginx의 8개 API와 권리 안내 경로 일치, 계정 삭제 request ID·본문 상한, catch-all 404·no-store·접근 로그 차단을 검증 |
