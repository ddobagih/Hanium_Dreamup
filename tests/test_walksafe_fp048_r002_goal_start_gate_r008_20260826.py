...
from scripts import run_walksafe_fp048_r002_goal_start_gate_r007_20260826 as r007
...
def _fixture(
    ...
    for sequence in range(90, r007.SOURCE_SEQUENCE):
        event = _sealed_event(sequence, previous)
        history.append(event)
        previous = event["event_sha256"]

    predecessor = {
        "sequence": r007.SOURCE_SEQUENCE,
        "event_id": r007.CORRECTION_EVENT_ID,
        "event_type": r007.CORRECTION_EVENT_TYPE,
    ...
    contract_binding = gate.expected_r008_binding()
    correction_event = {
    ...
    fake = SimpleNamespace(
        ...
        SOURCE_SEQUENCE=r007.SOURCE_SEQUENCE,
        ...
        load_exact_seq95_source=lambda *_args, **_kwargs: None,
        load_r008_contract=lambda _root: (
            copy.deepcopy(contract_value),
            copy.deepcopy(contract_binding),
        ),
        r008_contract_binding=lambda _root: copy.deepcopy(contract_binding),
        project_seq96=lambda *_args, **_kwargs: None,
...
def test_r008_contract_is_canonical_exact_successor_of_r007() -> None:
    raw, contract = _contract()
    predecessor_raw = (ROOT / r007.CONTRACT_RELATIVE).read_bytes()
    ...
    assert contract["supersedes"] == {
        ...
        "canonical_sha256": r007.canonical_sha256(predecessor),
        ...
        "path": r007.CONTRACT_RELATIVE.as_posix(),
...
def test_r008_contract_has_exact_five_post_seq95_checks() -> None:
    ...
    required = (
        gate.POST_SEQ95_REGRESSION_TEST_RELATIVE,
    ...
    forbidden = (
        r007.POST_SEQ94_REGRESSION_TEST_RELATIVE,
        r007.CORRECTION_TEST_RELATIVE,
        r007.RUNNER_TEST_RELATIVE,
        r007.START_APPLY_TEST_RELATIVE,
    )
    assert all(path.as_posix() not in command for path in forbidden)


def test_r008_reuses_only_unused_004_for_seq97() -> None:
    assert gate.SOURCE_SEQUENCE == 96
    assert gate.EVENT_SEQUENCE == 97
    assert gate.STARTED_EVENT_ID == r007.STARTED_EVENT_ID
    assert gate._document_id(gate.STARTED_EVENT_ID).endswith("20260826-004")
    with pytest.raises(gate.GateError, match="unused post-seq95 start ID"):
...
def test_stage_aware_parser_ignores_exact_nonauthority_receipts_only() -> None:
    ...
    checkpoint = {
        ...
        "r007": gate.expected_preflight_attempt_004_binding(),
...
def test_stage_aware_parser_rejects_forged_or_partial_nonauthority_metadata(
    ...
    elif mutation == "float_exit_code":
        metadata["exit_code"] = 1.0
...
def test_r007_preview_parser_failure_is_repaired_without_losing_evidence() -> None:
    sealed = (
        gate.GATE_ROOT_RELATIVE
        / "WS-GOAL-GRAPH-V2-4-GOAL-STARTED-FP048-R002-20260826-003"
        / gate.RECEIPT_NAME
    ...
    with pytest.raises(r007.GateError, match="metadata differs"):
        r007._impl._checkpoint_bound_gate_event_directories(raw)
...
def test_seq96_authority_and_context_bind_r008_and_three_preflight_records(
    ...
    assert authority.contract_binding == gate.expected_r008_binding()
    assert authority.preflight_attempt_binding == (
        gate.expected_preflight_attempt_004_binding()
    )
    assert authority.r006_preflight_attempt_binding == (
        gate.expected_r006_preflight_attempt_004_binding()
    )
    ...
    assert context.contract_binding == gate.expected_r008_binding()


def test_seq96_authority_rejects_current_preflight_metadata_drift(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_pre_seq96_source_fails_closed_without_004_namespace(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root, source, fake, _ready, _event = _fixture(tmp_path)
    ...
    checkpoint_path = root / gate.CHECKPOINT_RELATIVE
    checkpoint_path.write_bytes(_json_bytes(source))
    fake.require_contract_corrected_checkpoint = lambda *_args, **_kwargs: None
    monkeypatch.setattr(gate, "_correction", lambda: fake)
    directory = root / gate.GATE_ROOT_RELATIVE / gate.STARTED_EVENT_ID
    assert not directory.exists()
    with pytest.raises(gate.GateError, match="not exact seq96"):
        gate.bind_contract_corrected_source(root, source)
    assert not directory.exists()
...
fake = SimpleNamespace(
    ...
    STARTED_SEQUENCE=gate.EVENT_SEQUENCE,
    STARTED_EVENT_ID=gate.STARTED_EVENT_ID,
    load_exact_seq95_source=lambda *args, **kwargs: None,
    load_r008_contract=lambda *args, **kwargs: None,
    r008_contract_binding=lambda *args, **kwargs: None,
    project_seq96=lambda *args, **kwargs: None,
...
assert gate.base.base.base.base.base.base._sync_runtime_paths is gate._sync_runtime_paths
assert private[5] is gate._PRIVATE_R002_SYNC_RUNTIME_PATHS
for adapter in (
    gate.base,
    gate.base.base,
    gate.base.base.base,
    gate.base.base.base.base,
    gate.base.base.base.base.base,
):
