...
from scripts import apply_walksafe_fp048_r002_goal_started_seq93_20260826 as start
from scripts import (
...
def _source() -> dict[str, Any]:
    ...
    binding = {
        "schema_version": "1.1",
        "document_id": "WS-FP048-R002-INITIAL-START-GATE-CONTRACT-20260826-004",
    ...
    history.extend((ready, seq90, seq91, correction_event))
    return {
...
def _evidence(root: Path, source: dict[str, Any]) -> start.GateEvidence:
        ...
        runs.append(
            {
                ...
                "exit_code": 0,
                "executed_at": f"2026-08-26T01:32:0{index + 1}.100000+09:00",
            }
        )
    snapshot = {
    ...
    receipt = {
        ...
        "runtime_bindings": [],
        "execution_window": {
            "started_at": "2026-08-26T01:32:01+09:00",
            "ended_at": "2026-08-26T01:32:07+09:00",
        },
        "check_runs": runs,
        "repository_snapshot": snapshot,
        "generated_at": "2026-08-26T01:32:08+09:00",
    }
    ...
    return start.GateEvidence(
        ...
        repository_payload={},
        event_occurred_at="2026-08-26T01:32:09+09:00",
    )
...
def _patch_runtime(
    ...
    correction_event = source["goal_execution"]["transition_history"][91]
    ...
    correction_authority = SimpleNamespace(
        ...
        r004_contract_binding=lambda _root: copy.deepcopy(binding),
        canonical_seq92_checkpoint_bytes=lambda _root, value: (
            start.json_bytes(value)
            if is_source(value)
            else (_ for _ in ()).throw(ValueError("wrong checkpoint"))
        ),
        reconstructed_seq92_checkpoint_bytes=lambda _root, value: (
            start.json_bytes(value)
            if is_source(value)
            else (_ for _ in ()).throw(ValueError("wrong checkpoint"))
        ),
...
def test_seq93_projection_is_exact_one_ready_to_in_progress_zero_credit_change(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    projected, event = start.project_seq93(
        tmp_path,
        source,
    ...
    assert projected["goal_execution"]["transition_history"][:92] == source["goal_execution"]["transition_history"]
    ...
    assert start.reconstructed_seq92_checkpoint_bytes(tmp_path, projected) == (
...
def test_seq93_suffix_rejects_each_forged_inverse_delta_field(
    tmp_path: Path,
...
def test_live_checkpoint_is_exact_seq92_or_seq93_stage() -> None:
    ...
    if len(history) < 92:
        pytest.skip("seq92 gate-contract correction is not published yet")
    if len(history) == 92:
        ...
        assert read.raw == correction.canonical_seq92_checkpoint_bytes(
            start.ROOT,
            checkpoint,
        )
    elif len(history) == 93:
        start.require_exact_seq93_projection(
            start.ROOT,
            checkpoint,
            require_live_snapshot=True,
        )
...
def test_seq93_test_authority_overrides_a_shadowed_gate_goal_sha256(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = _source()
...
def test_seq93_rejects_burned_event_id_and_changed_seq92_authority(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    ...
    with pytest.raises(start.StartApplyError, match="seq92"):
        start.require_exact_source(tmp_path, source)


def test_seq93_post_validator_rejects_resealed_wrong_source_checkpoint_sha256(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    evidence = _evidence(tmp_path, source)
    projected, _ = start.project_seq93(
        tmp_path,
        source,
        evidence,
        event_id=gate.STARTED_EVENT_ID,
    ...
    receipt_path.chmod(0o600)
    event = projected["goal_execution"]["transition_history"][92]
...
def test_failed_preflight_never_calls_publication(
    ...
    def fail(*_args: object, **_kwargs: object) -> Any:
        raise start.StartApplyError("synthetic missing fresh R004 PASS receipt")
...
def test_seq93_import_never_loads_legacy_starter_runtimes() -> None:
    program = textwrap.dedent(
        ...
        assert start.EVENT_SEQUENCE == 93
        assert start.write_projection.__globals__["EVENT_SEQUENCE"] == 93
...
def test_seq93_runtime_closure_loads_only_after_lazy_goal_graph_validator() -> None:
    program = textwrap.dedent(
        """
        from pathlib import Path
        import sys
...
def test_seq93_uses_local_prepared_projection_and_snapshot_digest() -> None:
    transport = SimpleNamespace(
        root=Path("/synthetic"),
        source_raw=b"source",
        source={"sequence": 92},
        projected={"sequence": 93},
    ...
    assert prepared.event == {"sequence": 93}
...
def test_seq93_runtime_trust_closure_is_always_managed() -> None:
    expected = (
...
def test_default_writer_receives_exact_seq93_commit_guard(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []
    prepared = start.PreparedProjection(
...
def test_seq93_cli_keeps_only_preflight_and_write_modes() -> None:
