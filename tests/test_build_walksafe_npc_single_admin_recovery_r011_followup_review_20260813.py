...
from scripts import build_walksafe_npc_single_admin_recovery_r011_followup_review_20260813 as subject
...
def assignment(context: subject.ReviewContext) -> dict:
    return {
        "schema_version": "1.0",
        "evidence_type": "NPC_R011_FOLLOWUP_REVIEW_ASSIGNMENT",
        "goal_id": subject.trace.GOAL_ID,
        "round_id": subject.ROUND_ID,
        "assigned_at": "2026-08-13T20:50:00+09:00",
        "assigner": identity("codex-r011-assigner", "/root", "INTERNAL_REVIEW_ASSIGNER"),
        "executor": identity("codex-r011-executor", "/root", "INTERNAL_IMPLEMENTATION_EXECUTOR"),
        "reviewer": identity("codex-r011-reviewer", "/root/r011_reviewer", "SEPARATE_INTERNAL_REVIEWER"),
...
def result(context: subject.ReviewContext, assigned: dict, raw: bytes) -> dict:
    return {
        "schema_version": "1.0",
        "evidence_type": "NPC_R011_REVIEWER_AUTHORED_RESULT",
        "goal_id": subject.trace.GOAL_ID,
        "round_id": subject.ROUND_ID,
        "assignment_binding": {
...
def test_frozen_r010_and_superseded_assignments_are_exact() -> None:
    assert subject.prepare_frozen_r010_context(ROOT).control_code_cohort_sha256
    context = subject.prepare_review_context(ROOT)
    assert len(context.predecessor_bindings) == 9
