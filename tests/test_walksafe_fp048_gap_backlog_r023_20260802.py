...
class WalkSafeFp048GapBacklogR023Test(unittest.TestCase):
    ...
    def test_fast_path_cleanup_attempts_all_closes_and_preserves_primary_error(self) -> None:
        outputs = builder.build_outputs(self.root)
        builder.write_or_check(self.root, outputs, write=True)
        output_names = {relative.name for relative in builder.OUTPUT_PATHS}

        for write in (False, True):
            for primary_error in (False, True):
                with self.subTest(write=write, primary_error=primary_error):
                    real_close = builder._OS_CLOSE
                    closed_outputs: list[str] = []
                    patcher: mock._patch | None = None

                    def close_then_raise(descriptor: int) -> None:
                        target = Path(os.readlink(f"/proc/self/fd/{descriptor}"))
                        if target.name in output_names:
                            closed_outputs.append(target.name)
                        real_close(descriptor)
                        raise OSError(errno.EIO, "secondary cohort close failure")
                    ...
                    def install_close_probe() -> None:
                        ...
                        patcher.start()
                        if primary_error:
                            raise RuntimeError("primary fast-path failure")

                    expected_error = RuntimeError if primary_error else OSError
                    expected_message = (
                        "primary fast-path failure"
                        if primary_error
                        else "secondary cohort close failure"
                    )
                    try:
                        with self.assertRaisesRegex(expected_error, expected_message):
                            builder.write_or_check(
                                ...
                                outputs,
                                write=write,
                                before_cohort_cleanup_hook=install_close_probe,
                            )
                    finally:
                        if patcher is not None:
                            patcher.stop()
                    self.assertCountEqual(closed_outputs, output_names)
    ...
    def test_snapshot_initialization_preserves_primary_error_and_attempts_all_closes(self) -> None:
        real_fstat = builder._OS_FSTAT
        real_close = builder._OS_CLOSE
        fstat_calls = 0
        closed: list[int] = []
        ...
        def fail_final_root(descriptor: int) -> os.stat_result:
            ...
            fstat_calls += 1
            if fstat_calls == final_root_call:
                raise OSError(errno.EIO, "primary root fstat failure")
            return real_fstat(descriptor)
        ...
        with (
            mock.patch.object(builder, "_OS_FSTAT", side_effect=fail_final_root),
            mock.patch.object(builder, "_OS_CLOSE", side_effect=close_then_raise),
        ):
            with self.assertRaisesRegex(OSError, "primary root fstat failure"):
                builder.RepositorySnapshot(self.root)
        self.assertEqual(len(closed), len(self.root.parts))

    def test_snapshot_exit_preserves_primary_error_and_attempts_all_closes(self) -> None:
        snapshot = builder.RepositorySnapshot(self.root)
        snapshot.read(builder.R021_GAP_REL)
        expected_closes = (
            len(snapshot._files)
            + len(snapshot._directories)
            + len(snapshot._outer_descriptors)
        )
        real_close = builder._OS_CLOSE
        closed: list[int] = []
        ...
        with mock.patch.object(builder, "_OS_CLOSE", side_effect=close_then_raise):
            with self.assertRaisesRegex(RuntimeError, "primary snapshot failure"):
                with snapshot:
                    raise RuntimeError("primary snapshot failure")
        self.assertEqual(len(closed), expected_closes)

    def test_opened_directory_preserves_primary_error_and_attempts_all_closes(self) -> None:
        real_close = builder._OS_CLOSE
        closed: list[int] = []
        patcher: mock._patch | None = None
        ...
        try:
            with self.assertRaisesRegex(RuntimeError, "primary directory failure"):
                with builder._opened_directory(self.root, Path("docs/control")):
                    patcher = mock.patch.object(
                        builder,
                        "_OS_CLOSE",
                        side_effect=close_then_raise,
                    ...
                    patcher.start()
                    raise RuntimeError("primary directory failure")
        finally:
            if patcher is not None:
                patcher.stop()
    ...
    def test_single_descriptor_cleanup_preserves_primary_and_propagates_without_one(self) -> None:
        class FailingList(list):
            def append(self, value: object) -> None:
                raise MemoryError("primary descriptor ownership failure")
        ...
        def close_then_raise(descriptor: int) -> None:
            real_close(descriptor)
            closed.append(descriptor)
            raise OSError(errno.EIO, "secondary single close failure")

        descriptor = os.open(self.root, os.O_RDONLY | os.O_DIRECTORY)
        with mock.patch.object(builder, "_OS_CLOSE", side_effect=close_then_raise):
            with self.assertRaisesRegex(
                MemoryError,
                "primary descriptor ownership failure",
            ):
                builder._append_owned_descriptor(
                    FailingList(),
                    descriptor,
                    descriptor,
                )
        ...
        descriptor = os.open(self.root, os.O_RDONLY | os.O_DIRECTORY)
        with mock.patch.object(builder, "_OS_CLOSE", side_effect=close_then_raise):
            with self.assertRaisesRegex(OSError, "secondary single close failure"):
                builder._close_descriptor_preserving_primary(descriptor)
        self.assertEqual(closed[-1], descriptor)

    def test_direct_os_close_is_centralized_in_close_sequence(self) -> None:
        source = (self.root / builder.BUILDER_REL).read_text(encoding="utf-8")
        self.assertEqual(source.count("_OS_CLOSE("), 1)
    ...
    def test_precommit_primary_error_is_not_masked_by_cleanup_close_errors(self) -> None:
        outputs = builder.build_outputs(self.root)
        real_close = builder._OS_CLOSE
        closed: list[int] = []
        patcher: mock._patch | None = None
        ...
        def fail_precommit() -> None:
            ...
            patcher.start()
            raise RuntimeError("primary precommit failure")

        try:
            with self.assertRaisesRegex(RuntimeError, "primary precommit failure"):
                builder.write_or_check(
                    self.root,
                    outputs,
                    write=True,
                    before_journal_commit_hook=fail_precommit,
                )
        finally:
            if patcher is not None:
                patcher.stop()
        self.assertGreater(len(closed), len(builder.OUTPUT_PATHS))
