...
from scripts import build_walksafe_fp016_camera_centered_buttonless_screen_trace_20260726 as builder
...
EXPECTED_OUTPUT_PATHS = tuple(
    TEST_ROOT / path
    for path in (
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-016-R001/implementation-record.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-016-R001/verification-result.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-016-R001/successor-trace.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-016-R001/review-subject.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-016-R001/independent-review.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-016-R001/completion-receipt.json",
        "docs/control/audits/walksafe-implementation-gap-analysis-20260726-r019.json",
        "docs/control/audits/walksafe-implementation-gap-analysis-20260726-r019.md",
        "docs/control/audits/walksafe-implementation-remediation-backlog-20260726-r019.json",
        "docs/control/audits/walksafe-implementation-remediation-backlog-20260726-r019.md",
        "docs/control/execution/walksafe-epic-02-fp016-camera-centered-buttonless-screen-active-ledger-overlay-20260726-r001.json",
        "docs/control/execution/walksafe-epic-02-fp016-camera-centered-buttonless-screen-active-ledger-overlay-20260726-r001.md",
    )
)
...
def write_logs(root: Path) -> dict:
    ...
    def successful_log(body: str, command: str, run_id: str) -> str:
        ...
        return (
            "WALKSAFE_EXECUTION_EVENT_SEQUENCE=24\n"
            "WALKSAFE_EXECUTION_EVENT_ID=WS-GOAL-GRAPH-V2-4-GOAL-STARTED-FP016-20260726-001\n"
            "WALKSAFE_EXECUTION_EVENT_SHA256=4939c7612a7b1b452df2273977305f057ff856e7f95633f5b2922d7052bc71f6\n"
...
def test_outputs_are_deterministic_and_stageable(tmp_path: Path) -> None:
    assert builder.FOCUSED_COMMAND.count("--tests ") == 12
    for pattern in (
        "*MainActivityFp016StaticTest",
        ...
        "*MainActivityStartupCapabilityStaticTest",
        "*WalkSafeStartupCapabilityTest",
        "*WalkSessionLifecycleTest",
        "*AndroidVoiceCommandTest",
    ):
    ...
    assert len(EXPECTED_SCOPE_PATHS) == 13
...
def test_gap_and_backlog_reassess_only_fp016(tmp_path: Path) -> None:
    outputs = builder.build_outputs(**write_logs(tmp_path))
    gap = parsed(outputs, builder.GAP_R019_JSON)
    backlog = parsed(outputs, builder.BACKLOG_R019_JSON)
    assessment = next(
        item for item in gap["assessments"] if item["gap_id"] == "GAP-025"
    )
    ...
    assert gap["summary"]["status_counts"]["PARTIAL"] == 30
    assert gap["summary"]["status_counts"]["IMPLEMENTED"] == 0
    assert gap["reassessment_scope"]["directly_reassessed_gap_ids"] == ["GAP-025"]
    assert gap["reassessment_scope"]["carried_forward_gap_count"] == 67
    assert assessment["status"] == "PARTIAL"
    assert assessment["formal_test_status"] == "NOT_RUN"
    assert assessment["fp016_reassessment"]["release_status"] == "NOT_ELIGIBLE"
    gap_markdown = outputs[builder.GAP_R019_MD]
    assert gap_markdown.startswith("# WalkSafe 구현 Gap 분석 r019\n")
    assert "`CONFLICTING → PARTIAL`" in gap_markdown
    assert "TC-FP-016-01~04" in gap_markdown
    assert "r018" not in gap_markdown
    assert "MISSING → PARTIAL" not in gap_markdown
    assert "TC-FP-016-01~05" not in gap_markdown
    assert backlog["next_single_action"]["source_policy_id"] == "FP-012"
    assert backlog["next_single_action"]["gap_id"] == "GAP-021"
...
def test_receipt_preserves_internal_only_boundary(tmp_path: Path) -> None:
    ...
    assert len(implementation["changed_artifacts"]) == 13
    changed = {
...
def test_canonical_log_cannot_predate_sequence_24(tmp_path: Path) -> None:
    logs = write_logs(tmp_path)
    ...
    with pytest.raises(builder.BuildError, match="starts before the FP-016 execution event"):
        builder.build_outputs(**logs)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "reviewer_id",
            "WS-FP016-INDEPENDENT-REVIEWER-001 ",
            "identities must differ",
        ),
        (
            "reviewer_id",
            "WS-FP016-INDEPENDENT-REVIEWER-ALIAS",
            "identities must differ",
        ),
        (
            "reviewer_task",
            "/root/fp016_final_evidence_review ",
            "not separate from the executor",
        ),
        (
            "reviewer_task",
            "/root/fp016_alias_review",
            "not separate from the executor",
        ),
    ],
)
...
def test_staged_output_tamper_or_delete_fails_closed(
    ...
    target = builder.output_destination(builder.FP016_OVERLAY_MD, stage)
    if mutation == "tamper":
