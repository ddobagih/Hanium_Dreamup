...
from scripts import apply_walksafe_fp048_r002_goal_started_seq94_20260826 as start
from scripts import (
...
SYNTHETIC_MANAGED_PATHS = sorted(
    (
        "synthetic/source.txt",
        start.SCRIPT_RELATIVE.as_posix(),
        start.TEST_RELATIVE.as_posix(),
    )
)
...
def _source() -> dict[str, Any]:
    ...
    seq91 = {
        "sequence": 91,
        "event_id": "SYNTHETIC-FP048-R002-SEQ91",
        "event_type": "GOAL_START_CONTROL_REANCHORED",
    ...
    seq91["event_sha256"] = start.contract.event_sha256(seq91)
    binding = {
        "schema_version": "1.2",
    ...
    correction_event = {
        "sequence": 92,
        "event_id": "SYNTHETIC-FP048-R002-SEQ92",
        "event_type": "GOAL_START_GATE_CONTRACT_CORRECTED",
        "occurred_at": "2026-08-26T01:32:00+09:00",
        "subject_goal_id": gate.TARGET_GOAL_ID,
        "from_status": "READY",
        "to_status": "READY",
        "status_changes": {},
    ...
    reanchor_event = {
        "sequence": 93,
        "event_id": start.reanchor.REANCHOR_EVENT_ID,
        "event_type": "GOAL_START_CONTROL_REANCHORED",
        "occurred_at": "2026-08-26T01:33:00+09:00",
        "subject_goal_id": gate.TARGET_GOAL_ID,
        "from_status": "READY",
        "to_status": "READY",
        "status_changes": {},
        ...
        "repository_context_reanchor": {
            "after": {
                "scope": "synthetic seq93",
                "managed_changed_paths": copy.deepcopy(SYNTHETIC_MANAGED_PATHS),
                "managed_changed_path_count": len(SYNTHETIC_MANAGED_PATHS),
                "path_set_sha256": "d" * 64,
                "content_set_sha256": "e" * 64,
            }
        },
    ...
    reanchor_event["event_sha256"] = start.contract.event_sha256(reanchor_event)
    history.extend((ready, seq90, seq91, correction_event, reanchor_event))
    return {
        ...
        "goal_execution": {
            ...
            "validation_cutoff_at": reanchor_event["occurred_at"],
            "goal_status": "READY",
            "status_by_goal": {
        ...
        "session_handoff": {
            "changed_files": copy.deepcopy(SYNTHETIC_MANAGED_PATHS),
            "source_commit_or_snapshot": {
                "file_count": len(SYNTHETIC_MANAGED_PATHS),
                "path_set_sha256": "d" * 64,
                "content_set_sha256": "e" * 64,
            },
...
def _evidence(root: Path, source: dict[str, Any]) -> start.GateEvidence:
        ...
        runs.append(
            {
                ...
                "exit_code": 0,
                "executed_at": f"2026-08-26T01:33:0{index + 1}.100000+09:00",
            }
        )
    snapshot = {
    ...
    receipt = {
        ...
        "runtime_bindings": [],
        "execution_window": {
            "started_at": "2026-08-26T01:33:01+09:00",
            "ended_at": "2026-08-26T01:33:07+09:00",
        },
        "check_runs": runs,
        "repository_snapshot": snapshot,
        "generated_at": "2026-08-26T01:33:08+09:00",
    }
    ...
    return start.GateEvidence(
        ...
        repository_payload={},
        event_occurred_at="2026-08-26T01:33:09+09:00",
    )
...
def _patch_runtime(
    ...
    reanchor_event = source["goal_execution"]["transition_history"][92]
    binding = reanchor_event["contract_supersession"]["replacement_contract_binding"]
    ...
    reanchor_authority = SimpleNamespace(
        ...
        require_branch_semantics_reanchored_checkpoint=require_source,
        r005_contract_binding=lambda _root: copy.deepcopy(binding),
        canonical_seq93_checkpoint_bytes=lambda _root, value: (
            start.json_bytes(value)
            if is_source(value)
            else (_ for _ in ()).throw(ValueError("wrong checkpoint"))
        ),
        reconstructed_seq93_checkpoint_bytes=lambda _root, value: (
            start.json_bytes(value)
            if is_source(value)
            else (_ for _ in ()).throw(ValueError("wrong checkpoint"))
        ),
    ...
    authority = SimpleNamespace(
        ...
        reanchor_event_sha256=reanchor_event["event_sha256"],
        correction_event_sha256=reanchor_event["event_sha256"],
        contract_binding=binding,
    )
    monkeypatch.setattr(start, "_correction", lambda: reanchor_authority)
    monkeypatch.setattr(gate, "bind_branch_reanchored_source", lambda _root, value: (
        authority
        if is_source(value)
        else (_ for _ in ()).throw(ValueError("wrong checkpoint"))
    ))
...
def test_seq94_projection_is_exact_one_ready_to_in_progress_zero_credit_change(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    projected, event = start.project_seq94(
        tmp_path,
        source,
    ...
    assert projected["goal_execution"]["transition_history"][:93] == source["goal_execution"]["transition_history"]
    ...
    assert start.reconstructed_seq93_checkpoint_bytes(tmp_path, projected) == (
...
def test_seq94_suffix_rejects_each_forged_inverse_delta_field(
    tmp_path: Path,
...
def test_live_checkpoint_is_exact_seq93_or_seq94_stage() -> None:
    ...
    if len(history) < 93:
        pytest.skip("seq93 branch-semantics reanchor is not published yet")
    if len(history) == 93:
        ...
        assert read.raw == reanchor.canonical_seq93_checkpoint_bytes(
            start.ROOT,
            checkpoint,
        )
    elif len(history) == 94:
        start.require_exact_seq94_projection(
            start.ROOT,
            checkpoint,
            require_live_snapshot=True,
        )
...
def test_seq94_test_authority_overrides_a_shadowed_gate_goal_sha256(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = _source()
...
def test_seq94_rejects_burned_event_id_and_changed_seq93_authority(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    ...
    with pytest.raises(start.StartApplyError, match="seq93"):
        start.require_exact_source(tmp_path, source)


def test_seq94_rejects_burned_003_receipt_binding(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    evidence = _evidence(tmp_path, source)
    burned_id = "WS-GOAL-GRAPH-V2-4-GOAL-STARTED-FP048-R002-20260826-003"
    forged = start.GateEvidence(
        ...
        receipt_binding={
            ...
            "path": (
                gate.GATE_ROOT_RELATIVE / burned_id / gate.RECEIPT_NAME
            ).as_posix(),
            "file_sha256": evidence.receipt_binding["file_sha256"],
        },
        repository_payload=evidence.repository_payload,
        event_occurred_at=evidence.event_occurred_at,
    )
    with pytest.raises(start.StartApplyError, match="seq94 gate binding differs"):
        start.project_seq94(
            tmp_path,
            source,
            forged,
...
def test_seq94_post_validator_rejects_resealed_wrong_source_checkpoint_sha256(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    evidence = _evidence(tmp_path, source)
    projected, _ = start.project_seq94(
        tmp_path,
        source,
        evidence,
        event_id=gate.STARTED_EVENT_ID,
    ...
    receipt_path.chmod(0o600)
    event = projected["goal_execution"]["transition_history"][93]
...
def test_failed_preflight_never_calls_publication(
    ...
    def fail(*_args: object, **_kwargs: object) -> Any:
        raise start.StartApplyError("synthetic missing fresh R005 PASS receipt")
...
def test_seq94_import_never_loads_legacy_starter_runtimes() -> None:
    program = textwrap.dedent(
        ...
        assert start.EVENT_SEQUENCE == 94
        assert start.write_projection.__globals__["EVENT_SEQUENCE"] == 94
...
def test_seq94_runtime_closure_loads_only_after_lazy_goal_graph_validator() -> None:
    program = textwrap.dedent(
        """
        from pathlib import Path
        import sys
...
def test_seq94_uses_local_prepared_projection_and_snapshot_digest() -> None:
    transport = SimpleNamespace(
        root=Path("/synthetic"),
        source_raw=b"source",
        source={"sequence": 93},
        projected={"sequence": 94},
    ...
    assert prepared.event == {"sequence": 94}
...
def test_seq94_runtime_trust_closure_is_always_managed() -> None:
    expected = (
...
def test_default_writer_receives_exact_seq94_commit_guard(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []
    prepared = start.PreparedProjection(
...
def test_seq94_cli_keeps_only_preflight_and_write_modes() -> None:
...
def test_gate_evidence_rejects_reordered_pretty_receipt(
        ...
        runs.append(
            {
                ...
                "output_sha256": start.sha256_bytes(output),
                "exit_code": 0,
                    "executed_at": f"2026-08-26T01:33:0{index}+09:00",
            }
        )
    ...
    receipt = {
        ...
        "runtime_bindings": [],
        "execution_window": {
                "started_at": "2026-08-26T01:33:00+09:00",
                "ended_at": "2026-08-26T01:33:06+09:00",
        },
        "check_runs": runs,
        "repository_snapshot": snapshot,
            "generated_at": "2026-08-26T01:33:07+09:00",
    }
    if reordered == "top":
