...
from scripts import apply_walksafe_fp048_r002_goal_started_seq96_20260826 as start
from scripts import (
...
def _source() -> dict[str, Any]:
    ...
    r006_binding = copy.deepcopy(correction.R006_CONTRACT_BINDING)
    ...
    correction_event = {
        ...
        "occurred_at": "2026-08-26T01:35:00+09:00",
        "subject_goal_id": gate.TARGET_GOAL_ID,
        "from_status": "READY",
        "to_status": "READY",
        "status_changes": {},
        ...
        "contract_supersession": {
            "previous_contract_binding": r006_binding,
            "replacement_contract_binding": binding,
        },
        "start_gate_runner_binding": {"status": "FROZEN_R007"},
        "repository_context_reanchor": {
            "after": {
                "scope": "synthetic seq95",
...
def _evidence(root: Path, source: dict[str, Any]) -> start.GateEvidence:
        ...
        runs.append(
            {
                ...
                "exit_code": 0,
                "executed_at": f"2026-08-26T01:35:0{index + 1}.100000+09:00",
            }
        )
    snapshot = {
    ...
    receipt = {
        ...
        "runtime_bindings": [],
        "execution_window": {
            "started_at": "2026-08-26T01:35:01+09:00",
            "ended_at": "2026-08-26T01:35:07+09:00",
        },
        "check_runs": runs,
        "repository_snapshot": snapshot,
        "generated_at": "2026-08-26T01:35:08+09:00",
    }
    ...
    return start.GateEvidence(
        ...
        repository_payload={},
        event_occurred_at="2026-08-26T01:35:09+09:00",
    )
...
def _patch_runtime(
    ...
    correction_event = source["goal_execution"]["transition_history"][94]
    ...
    correction_authority = SimpleNamespace(
        ...
        r007_contract_binding=lambda _root: copy.deepcopy(binding),
        canonical_seq95_checkpoint_bytes=lambda _root, value: (
            start.json_bytes(value)
            if is_source(value)
            else (_ for _ in ()).throw(ValueError("wrong checkpoint"))
        ),
        reconstructed_seq95_checkpoint_bytes=lambda _root, value: (
            start.json_bytes(value)
            if is_source(value)
            else (_ for _ in ()).throw(ValueError("wrong checkpoint"))
        ),
...
def test_seq96_projection_is_exact_one_ready_to_in_progress_zero_credit_change(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    evidence = _evidence(tmp_path, source)
    projected, event = start.project_seq96(
        tmp_path,
        source,
        evidence,
    ...
    assert projected["goal_execution"]["transition_history"][:95] == source["goal_execution"]["transition_history"]
    source_binding = projected["goal_execution"]["transition_history"][94][
        "source_checkpoint_binding"
    ]
    ...
    assert projected["goal_execution"]["transition_history"][94][
        "transition_control_review_binding"
    ]["round_id"] == correction.ROUND_ID
    assert projected["goal_execution"]["transition_history"][94][
        "contract_supersession"
        ...
        "transition_history"
    ][94]["contract_supersession"]["previous_contract_binding"]
    ...
    assert start.reconstructed_seq95_checkpoint_bytes(tmp_path, projected) == (
...
def test_public_seq95_r007_gate_authority_distinguishes_absent_and_exact_pass(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    _write_source_checkpoint(tmp_path, source)
    assert start.require_published_r007_gate_for_seq95(tmp_path, source) is None
    ...
    binding = start.require_published_r007_gate_for_seq95(tmp_path, source)
...
def test_public_seq95_r007_gate_authority_rejects_partial_or_forged_namespace(
    tmp_path: Path,
    ...
    with pytest.raises(start.StartApplyError):
        start.require_published_r007_gate_for_seq95(tmp_path, source)


@pytest.mark.parametrize("mutation", ("source", "receipt"))
def test_public_seq95_r007_gate_authority_reseals_source_and_receipt(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
) -> None:
...
def test_seq96_suffix_rejects_each_forged_inverse_delta_field(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
) -> None:
...
def test_live_checkpoint_is_exact_seq95_or_seq96_stage() -> None:
    ...
    if len(history) < 95:
        pytest.skip("seq95 gate-contract correction is not published yet")
    if len(history) == 95:
        ...
        assert read.raw == correction.canonical_seq95_checkpoint_bytes(
            start.ROOT,
            checkpoint,
        )
    elif len(history) == 96:
        start.require_exact_seq96_projection(
            start.ROOT,
            checkpoint,
            require_live_snapshot=True,
        )
...
def test_require_started_checkpoint_controls_external_consumers(
    ...
    monkeypatch.setattr(start, "require_exact_seq96_projection", require_exact)
...
def test_seq96_test_authority_overrides_a_shadowed_gate_goal_sha256(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = _source()
...
def test_seq96_rejects_reserved_005_event_id_and_changed_seq95_authority(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    ...
    with pytest.raises(start.StartApplyError, match="seq95"):
        start.require_exact_source(tmp_path, source)


def test_seq96_post_validator_rejects_resealed_wrong_source_checkpoint_sha256(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    evidence = _evidence(tmp_path, source)
    projected, _ = start.project_seq96(
        tmp_path,
        source,
        evidence,
        event_id=gate.STARTED_EVENT_ID,
    ...
    receipt_path.chmod(0o600)
    event = projected["goal_execution"]["transition_history"][95]
...
def test_seq96_post_validator_rejects_resealed_receipt_timestamp_tamper(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    source = _source()
    _patch_runtime(source, monkeypatch)
    ...
    tampered_receipt = copy.deepcopy(evidence.receipt)
    tampered_receipt["generated_at"] = "2026-08-26T01:35:00+09:00"
    tampered_bytes = start.json_bytes(tampered_receipt)
    receipt_path = tmp_path / evidence.receipt_binding["path"]
    receipt_path.write_bytes(tampered_bytes)
    receipt_path.chmod(0o600)
...
def test_failed_preflight_never_calls_publication(
    ...
    def fail(*_args: object, **_kwargs: object) -> Any:
        raise start.StartApplyError("synthetic missing fresh R007 PASS receipt")
...
def test_seq96_import_never_loads_legacy_starter_runtimes() -> None:
    program = textwrap.dedent(
        ...
        assert start.SOURCE_SEQUENCE == 95
        assert start.EVENT_SEQUENCE == 96
        assert start.STARTED_EVENT_ID.endswith("-004")
        assert start.write_projection.__globals__["EVENT_SEQUENCE"] == 96
...
def test_seq96_runtime_closure_loads_only_after_lazy_goal_graph_validator() -> None:
    program = textwrap.dedent(
        """
        from pathlib import Path
        import sys
...
def test_seq96_uses_local_prepared_projection_and_snapshot_digest() -> None:
    transport = SimpleNamespace(
        root=Path("/synthetic"),
        source_raw=b"source",
        source={"sequence": 95},
        projected={"sequence": 96},
    ...
    assert prepared.event == {"sequence": 96}
...
def test_seq96_runtime_trust_closure_is_always_managed() -> None:
    expected = (
...
def test_default_writer_receives_exact_seq96_commit_guard(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[str] = []
    prepared = start.PreparedProjection(
...
def test_seq96_cli_keeps_only_preflight_and_write_modes() -> None:
...
def test_gate_evidence_rejects_reordered_pretty_receipt(
        ...
        runs.append(
            {
                ...
                "output_sha256": start.sha256_bytes(output),
                "exit_code": 0,
                    "executed_at": f"2026-08-26T01:35:0{index}+09:00",
            }
        )
    ...
    receipt = {
        ...
        "runtime_bindings": [],
        "execution_window": {
            "started_at": "2026-08-26T01:35:00+09:00",
            "ended_at": "2026-08-26T01:35:06+09:00",
        },
        "check_runs": runs,
        "repository_snapshot": snapshot,
        "generated_at": "2026-08-26T01:35:07+09:00",
    }
    if reordered == "top":
