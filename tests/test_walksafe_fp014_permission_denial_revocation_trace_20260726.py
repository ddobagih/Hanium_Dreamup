...
from scripts import build_walksafe_fp014_permission_denial_revocation_trace_20260726 as builder
...
EXPECTED_OUTPUT_PATHS = tuple(
    TEST_ROOT / path
    for path in (
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-014-R001/implementation-record.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-014-R001/verification-result.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-014-R001/successor-trace.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-014-R001/review-subject.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-014-R001/independent-review.json",
        "docs/control/execution/goal-results/WS-GOAL-EPIC-02-FP-014-R001/completion-receipt.json",
        ...
        "docs/control/audits/walksafe-implementation-remediation-backlog-20260726-r018.json",
        "docs/control/audits/walksafe-implementation-remediation-backlog-20260726-r018.md",
        "docs/control/execution/walksafe-epic-02-fp014-permission-denial-revocation-active-ledger-overlay-20260726-r001.json",
        "docs/control/execution/walksafe-epic-02-fp014-permission-denial-revocation-active-ledger-overlay-20260726-r001.md",
    )
)
...
def write_logs(root: Path) -> dict:
    ...
    def successful_log(body: str, command: str, run_id: str) -> str:
        if command in (builder.FOCUSED_COMMAND, builder.FULL_ANDROID_COMMAND):
            tests = 665 if command == builder.FULL_ANDROID_COMMAND else 23
            body = (
                f"{body}\n"
        ...
        return (
            ...
            "WALKSAFE_EXECUTION_EVENT_ID=WS-GOAL-GRAPH-V2-4-GOAL-STARTED-FP014-20260726-001\n"
            "WALKSAFE_EXECUTION_EVENT_SHA256=2158aa372a00af3024f2a80eeb6e98471fd07b97ec570c07edd5b5ce07c3551e\n"
...
def test_outputs_are_deterministic_and_stageable(tmp_path: Path) -> None:
    assert builder.FOCUSED_COMMAND.count("--tests ") == 5
    for pattern in (
    ...
    assert len(EXPECTED_IMPLEMENTATION_PATHS) == 11
...
def test_gap_and_backlog_reassess_only_fp014(tmp_path: Path) -> None:
    outputs = builder.build_outputs(**write_logs(tmp_path))
    gap = parsed(outputs, builder.GAP_R018_JSON)
    backlog = parsed(outputs, builder.BACKLOG_R018_JSON)
    assessment = next(
        item for item in gap["assessments"] if item["gap_id"] == "GAP-023"
    )
    ...
    assert gap["reassessment_scope"]["directly_reassessed_gap_ids"] == ["GAP-023"]
    assert gap["reassessment_scope"]["carried_forward_gap_count"] == 67
    assert assessment["status"] == "PARTIAL"
    assert assessment["formal_test_status"] == "NOT_RUN"
    assert assessment["fp014_reassessment"]["release_status"] == "NOT_ELIGIBLE"
    gap_markdown = outputs[builder.GAP_R018_MD]
    assert gap_markdown.startswith("# WalkSafe 구현 Gap 분석 r018\n")
    assert "`PARTIAL → PARTIAL`" in gap_markdown
    assert "TC-FP-014-01~03" in gap_markdown
    assert "r017" not in gap_markdown
    assert "MISSING → PARTIAL" not in gap_markdown
    assert "TC-FP-014-01~05" not in gap_markdown
    assert backlog["next_single_action"]["source_policy_id"] == "FP-016"
    assert backlog["next_single_action"]["gap_id"] == "GAP-025"
...
def test_receipt_preserves_internal_only_boundary(tmp_path: Path) -> None:
    ...
    assert len(implementation["changed_artifacts"]) == 11
    changed = {
        item["path"]: item for item in implementation["changed_artifacts"]
    }
    assert tuple(changed) == EXPECTED_IMPLEMENTATION_PATHS
    assert all(
...
def test_actual_gate_rows_supply_modified_before_hashes() -> None:
    ...
    assert tuple(files) == builder.IMPLEMENTATION_PATHS
...
@pytest.mark.parametrize(
    ...
    [
        ("WALKSAFE_JUNIT_TESTS=665", "", "marker count differs"),
        (
            "WALKSAFE_JUNIT_TESTS=665",
            "WALKSAFE_JUNIT_TESTS=665\nWALKSAFE_JUNIT_TESTS=665",
...
@pytest.mark.parametrize("tests", [665, 666, 700])
...
def test_dirty_absent_or_deleted_precedes_pinned_head(
    ...
    path = EXPECTED_IMPLEMENTATION_PATHS[0]
...
def test_canonical_log_cannot_predate_sequence_19(tmp_path: Path) -> None:
    logs = write_logs(tmp_path)
    content = logs["control_plane_log"].read_text(encoding="utf-8")
    logs["control_plane_log"].write_text(
        content.replace(
            "WALKSAFE_COMMAND_STARTED_AT=2026-07-26T15:00:00+09:00",
            "WALKSAFE_COMMAND_STARTED_AT=2026-07-26T03:03:18+09:00",
        ),
        encoding="utf-8",
    )

    with pytest.raises(builder.BuildError, match="starts before the FP-014 execution event"):
        builder.build_outputs(**logs)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "reviewer_id",
            "WS-FP014-INDEPENDENT-REVIEWER-001 ",
            "identities must differ",
        ),
        (
            "reviewer_id",
            "WS-FP014-INDEPENDENT-REVIEWER-ALIAS",
            "identities must differ",
        ),
        (
            "reviewer_task",
            "/root/fp014_final_evidence_review ",
            "not separate from the executor",
        ),
        (
            "reviewer_task",
            "/root/fp014_alias_review",
            "not separate from the executor",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("actual_talkback_user_tests_remain_not_run", False),
        ("actual_device_tests_remain_not_run", False),
        ("android_platform_review_remains_not_run", False),
        ("operational_permission_profile_approval_remains_not_run", False),
...
def test_staged_output_tamper_or_delete_fails_closed(
    ...
    target = builder.output_destination(builder.FP014_OVERLAY_MD, stage)
    if mutation == "tamper":
