...
function testMultipleCandidatesBlockAutoNavigationAndSummarizeThree() {
  ...
  assert(policy.speechMessage.includes("1번 서울역, 서울 중구 한강대로 405, 120m"), "speech should identify candidates by name, address and distance");
  assert(policy.speechMessage.includes("나머지 1개는 화면의 더 보기"), "speech should disclose hidden candidates");
  assert(!policy.speechMessage.includes("서울역 서부,"), "speech should stay bounded to the visible top three");
}
...
function testSpeechLabelUsesExplicitMissingMetadataFallbacks() {
  const policy = resolveNavigationDestinationPolicy(
    response([
      result({ id: "missing-1", name: "시청", road_address: null, address: null, distance_m: null }),
      result({ id: "missing-2", name: "시청 별관", road_address: null, address: null, distance_m: null })
    ])
  );

  assert(policy.speechMessage.includes("1번 시청, 주소 미상, 거리 미상"), "voice-only selection must state missing address and distance");
}
...
function testUnknownDistanceCandidateRequiresExplicitSelection() {
  const policy = resolveNavigationDestinationPolicy(response([result({ id: "unknown-distance", distance_m: null })]), {
    maxCandidateDistanceM: 30000
  });

  assert(policy.status === "distance_unknown", "missing distance should remain an explicit candidate-selection state");
  assert(policy.autoNavigationAllowed === false, "missing distance must never auto-select a destination");
  assert(policy.selectedCandidate === null, "missing distance must not silently select a candidate");
  assert(policy.canSelectCandidate, "the user may explicitly confirm the uncertain candidate");
  assert(policy.unknownDistanceCount === 1, "unknown distance count should remain visible to callers");
}
...
function main() {
  ...
  testSpeechLabelUsesExplicitMissingMetadataFallbacks();
  testOutOfRangeCandidateBlocksSelection();
  testUnknownDistanceCandidateRequiresExplicitSelection();
