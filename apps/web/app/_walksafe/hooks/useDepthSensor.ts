...
/**
 * Owns the user-started WebXR depth session and exposes the latest CPU depth frame for bbox sampling.
 * Capability checks do not start a session, and session/render-layer failures must release XR resources.
 */
import { useCallback, useEffect, useRef, useState } from "react";
import type { MutableRefObject } from "react";
import type { TwoModelDetection } from "@/types/inference-v2";
import type { DepthEstimateResult, DepthFrameSampler } from "../depth-estimator";
...
export function useDepthSensor({ samplePoint }: UseDepthSensorOptions = {}): UseDepthSensorResult {
  ...
  const stopDepthSensor = useCallback(async () => {
    ...
    setState((current) => ({
      ...current,
      status: "stopping",
      message: "WebXR depth 세션 중지 중입니다.",
      frameCount: 0,
      lastFrameAtMs: null,
      latestSample: null
    }));
    ...
    try {
      await session.end();
      if (sessionGenerationRef.current !== stopGeneration) {
        return;
      }
      setState((current) => ({
        ...current,
        status: "stopped",
        message: "WebXR depth 세션이 중지됐습니다.",
        frameCount: 0,
        lastFrameAtMs: null,
        latestSample: null,
        error: null
  ...
  const checkDepthSupport = useCallback(async (): Promise<boolean> => {
    const checkGeneration = sessionGenerationRef.current;
    ...
    try {
      ...
      if (sessionGenerationRef.current !== checkGeneration) {
        return false;
      }
      setState((current) => ({
        ...current,
  ...
  const startDepthSensor = useCallback(async (): Promise<boolean> => {
    if (sessionRef.current) {
      setState((current) => ({
        ...current,
        status: "running",
        message: "WebXR depth 실험 수신 중 · camera/depth 좌표 정합 검증 전이므로 위험 판정에는 사용하지 않음",
        error: null
      }));
      return true;
    }
    ...
    try {
      ...
      const handleEnded: EventListener = () => {
        if (sessionGenerationRef.current !== requestGeneration || sessionRef.current !== requestedSession) {
          return;
        }
        clearActiveSession();
        setState((current) => ({
          ...current,
          status: "stopped",
          message: "WebXR depth 세션이 종료됐습니다.",
          frameCount: 0,
          lastFrameAtMs: null,
          latestSample: null
        }));
            ...
            setState((current) => ({
              ...
              message:
                sample.distanceM === null
                  ? "WebXR depth 실험 프레임 수신 중 · 위험 판정 미사용"
                  : `WebXR depth 실험 표시 · 중앙 ${sample.distanceM.toFixed(2)}m · 위험 판정 미사용`,
              supported: true,
  ...
  const estimateDepthForDetection = useCallback((): DepthEstimateResult | null => null, []);
