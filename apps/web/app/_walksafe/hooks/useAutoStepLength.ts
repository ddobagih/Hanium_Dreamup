...
/**
 * Collects bounded GPS and motion windows for step-length calibration after browser permission gates pass.
 * Local storage is optional; failure or expiry falls back to live evidence and then the product default.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import type { GpsFix } from "@/types/inference";
import { WALKSAFE_DEFAULT_STEP_LENGTH_M } from "../config";
import { estimateRecentStepSpeedMps } from "../motion-projection";
import {
...
export type AutoStepLengthState = StepLengthEstimate & {
  ...
  shakeScore: number;
  recentStepSpeedMps: number | null;
...
const MOTION_UI_PUBLISH_INTERVAL_MS = 250;
...
export function useAutoStepLength(gps: GpsFix | null, enabled = true): AutoStepLengthState {
  ...
  const lastMotionPublishAtMsRef = useRef(0);
  ...
  const [recentStepSpeedMps, setRecentStepSpeedMps] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    refreshEstimate();
  }, [enabled, gps, refreshEstimate]);

  useEffect(() => {
    if (!enabled || typeof window === "undefined" || !getDeviceMotionPermissionEvent()) {
      return;
    }
    ...
    const onMotion = (event: DeviceMotionEvent) => {
      ...
      const stepDetected = motionDelta >= STEP_PEAK_THRESHOLD && wasBelowReset && enoughInterval;
      ...
      if (stepDetected || nowMs - lastMotionPublishAtMsRef.current >= MOTION_UI_PUBLISH_INTERVAL_MS) {
        lastMotionPublishAtMsRef.current = nowMs;
        ...
        setRecentStepSpeedMps(estimateRecentStepSpeedMps(stepEventsRef.current, estimate.stepLengthM, nowMs));
      }
    };

    window.addEventListener("devicemotion", onMotion);
    return () => window.removeEventListener("devicemotion", onMotion);
  }, [enabled, estimate.stepLengthM, motionPermissionStatus, refreshEstimate]);
