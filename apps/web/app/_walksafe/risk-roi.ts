/**
 * Geometric ROI heuristic for general objects. A bounded 3-5 second route
 * projection may shift the screen corridor, but temporal history must still
 * prove stability before this context can produce an alert.
 */
...
export type RoiRiskResult = {
  ...
  debug?: {
    ...
    predictionHorizonS: number | null;
...
function resolveMotionShiftX(
  ...
  if (
    typeof motion.future_shift_x === "number" &&
    Number.isFinite(motion.future_shift_x) &&
    typeof motion.confidence === "number" &&
    motion.confidence >= 0.35
  ) {
    return {
      shiftX: Math.max(-maxMotionShiftX, Math.min(maxMotionShiftX, motion.future_shift_x)),
      headingKnown: true,
      speedKnown: true
    };
  }
...
function resultDebug(
  ...
  return {
    ...
    excludedByClass,
    predictionHorizonS:
      typeof options.motion.prediction_horizon_s === "number" && Number.isFinite(options.motion.prediction_horizon_s)
        ? options.motion.prediction_horizon_s
        : null,
    projectedDistanceM:
      typeof options.motion.projected_distance_m === "number" && Number.isFinite(options.motion.projected_distance_m)
        ? options.motion.projected_distance_m
        : null,
    motionConfidence:
      typeof options.motion.confidence === "number" && Number.isFinite(options.motion.confidence)
        ? options.motion.confidence
        : null
  };
}
