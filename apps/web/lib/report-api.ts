/**
 * Provides the user-report and administrator query/export client contracts over the backend API.
 * Online checks fail fast, while list, summary and export filters share one query vocabulary.
 */
...
const REPORT_IMAGE_PATH = /^\/uploads\/[A-Za-z0-9][A-Za-z0-9._-]{0,127}\.(?:jpe?g|png|webp)$/i;
const REPORT_API_TIMEOUT_MS = 18_000;
...
export type ReportExportOptions = {
  redacted?: boolean;
  profile?: "internal" | "minimum" | "agency";
  bom?: boolean;
  manifest?: boolean;
...
export function agencyExportFilters(filters: ReportListParams): ReportListParams {
  return {
    ...filters,
    status: "reviewed",
...
export function reportImageUrl(report: ReportResponse): string {
  return REPORT_IMAGE_PATH.test(report.image_path)
    ? `${API_BASE_URL}${report.image_path}`
    : `${API_BASE_URL}/uploads/invalid`;
}
...
function buildReportSearchParams(filters: ReportListParams, includeLimit: boolean) {
  ...
  appendParam(params, "performance_excluded", filters.performance_excluded);
...
export function reportExportUrl(
  ...
  appendParam(params, "profile", options.profile);
  ...
  appendParam(params, "audit_id", options.auditId);
...
async function fetchReportApi(input: string, init: RequestInit = {}): Promise<Response> {
  const abortController = new AbortController();
  const timeoutId = globalThis.setTimeout(() => abortController.abort(), REPORT_API_TIMEOUT_MS);
  try {
...
export async function submitReport(detection: DetectionEvent, image: Blob): Promise<ReportResponse> {
  ...
  const response = await fetchReportApi(`${API_BASE_URL}/reports`, {
    method: "POST",
    body
  });
...
export async function checkDuplicateReports(detection: DetectionEvent): Promise<DuplicateCheckResponse | null> {
  ...
  const response = await fetchReportApi(`${API_BASE_URL}/reports/duplicate-check?${params.toString()}`, {
    cache: "no-store"
  });
...
export async function listReports(filters: ReportListParams = {}): Promise<ReportResponse[]> {
  ...
  const response = await fetchReportApi(`${API_BASE_URL}/reports?${params.toString()}`, {
    cache: "no-store"
  });
...
export async function getReportSummary(filters: ReportListParams = {}): Promise<AdminReportSummary> {
  ...
  const response = await fetchReportApi(`${API_BASE_URL}/reports/summary?${params.toString()}`, {
    cache: "no-store"
  });
...
export async function getDetectV2Health(): Promise<DetectV2HealthResponse> {
  ...
  const response = await fetchReportApi(`${API_BASE_URL}/detect/v2/health`, {
    cache: "no-store"
  });
...
export async function updateReportStatus(reportId: string, patch: ReportStatus | ReportStatusPatch): Promise<ReportResponse> {
  ...
  const response = await fetchReportApi(`${API_BASE_URL}/reports/${reportId}/status`, {
    method: "PATCH",
    headers: {
