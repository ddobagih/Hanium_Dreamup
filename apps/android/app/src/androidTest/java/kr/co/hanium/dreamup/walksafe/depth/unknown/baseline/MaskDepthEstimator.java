package kr.co.hanium.dreamup.walksafe.depth.unknown.baseline;
...
public final class MaskDepthEstimator {
    ...
    public static final class DepthFrame {
        ...
        private DepthFrame(FrameIdentity frame,long timestamp,String clock,int[] mm,double[] meters,
                           byte[] confidence,Calibration calibration,FrameIdentity confidenceFrame,boolean reference) {
            ...
            this.source=reference?"REGISTERED_RGBD_REFERENCE":"ARCORE_RAW_DEPTH";
        }
        public static DepthFrame raw(FrameIdentity frame,long timestamp,String clock,int[] mm,byte[] confidence,
            ...
            return new DepthFrame(frame,timestamp,clock,mm,null,confidence,calibration,confidenceFrame,false);
        }
        ...
        public static DepthFrame registered(FrameIdentity frame,double[] meters,Calibration calibration) {
            return new DepthFrame(frame,0,null,null,meters,null,calibration,null,true);
        }
        ...
        public static DepthFrame registeredMillimeters(FrameIdentity frame,int[] mm,Calibration calibration) {
            return new DepthFrame(frame,0,null,mm,null,null,calibration,null,true);
        }
    ...
    public List<Result> associate(List<Detection> detections,DepthFrame depth){
        ...
        for(Builder b:builders){
            ...
            List<int[]> components=raster.components();
    ...
    private static final class Raster {
        ...
        List<int[]> components(){
            List<int[]> out=new ArrayList<>();int[] queue=new int[bits.length];
    ...
    private Sample sample(int id,int[] pixels,Raster raster,DepthFrame d,Grid g){
        ...
        int[] valid=new int[pixels.length];int validCount=0,n=0;
        for(int i:pixels)if(raster.interior(i,p.erosionPixels)){
            n++;double z=d.z(i);
            if(Double.isFinite(z)&&z>=p.minDepthM&&z<=p.maxDepthM&&(d.reference||(d.confidence[i]&255)>=p.minRawConfidence))valid[validCount++]=i;
        }
        ...
        for(int k=1;k<=sorted.length;k++){
            if(k==sorted.length||sorted[k]-sorted[k-1]>Math.max(p.clusterGapM,p.clusterGapRelative*(sorted[k]+sorted[k-1])/2)){
        ...
        Double conf=null;if(!d.reference){double[] values=new double[kept.length];for(int k=0;k<kept.length;k++)values[k]=d.confidence[kept[k]]&255;conf=median(values);}
        return new Sample(new Component(id,pixels,n,validCount,kept.length,Status.KNOWN,"dominant_interior_layer",
            quantileSorted(sorted,robustStart,robustEnd,.5),medianRange(d,g,kept),quantileSorted(sorted,robustStart,robustEnd,.2),quantileSorted(sorted,robustStart,robustEnd,.8),mad,conf),kept);
    }
    ...
    private double[] sortedDepths(DepthFrame d,int[] indices,int count){
        ...
        Arrays.sort(sorted);return sorted;
    }
