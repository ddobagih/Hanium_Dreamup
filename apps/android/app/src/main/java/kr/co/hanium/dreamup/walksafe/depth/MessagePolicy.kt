...
class MessagePolicy(
    ...
    fun evaluate(result: MetricDepthDecision, nowMs: Long = System.currentTimeMillis()): MessagePolicyDecision {
        val candidate = when {
            result.className.equals("crosswalk", ignoreCase = true) -> crosswalkGuidanceMessage(result)
    ...
    private fun tactileMessage(result: MetricDepthDecision): MessagePolicyDecision {
        return when (result.className.lowercase()) {
            "normal_tactile_block" -> pathGuidanceMessage(result)
            ...
            else -> pathGuidanceMessage(result)
        }
    }

    private fun pathGuidanceMessage(result: MetricDepthDecision): MessagePolicyDecision {
        if (result.confidenceFinal < config.pathGuidanceMinConfidence) {
            return none("path guidance confidence below threshold")
        }
        val steps = if (hasReliableMetricDistance(result)) {
            distanceMetersToSteps(result.riskDistanceM, stepLengthM)
        } else {
            null
        }
        val message = if (steps != null && steps <= 6) {
            "약 ${steps}보 앞 점자블록입니다. 점자블록을 따라 이동하세요."
        } else {
            "점자블록을 따라 이동하세요."
        }
        return MessagePolicyDecision(
            ...
            reason = "normal tactile block path guidance",
        )
    }
    ...
    private fun crosswalkGuidanceMessage(result: MetricDepthDecision): MessagePolicyDecision {
        ...
        val message = if (steps != null && steps <= 8) {
            "약 ${steps}보 앞 횡단보도입니다. 신호와 차량을 확인하세요."
        } else {
            "전방 횡단보도입니다. 신호와 차량을 확인하세요."
        }
        return MessagePolicyDecision(
            userFacing = UserFacingDepth(
                stepsAhead = steps,
            ...
            purpose = MessagePurpose.PATH_GUIDANCE,
            reason = "crosswalk path guidance",
        )
    }
