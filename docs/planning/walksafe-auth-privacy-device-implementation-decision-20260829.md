...
목적·안전 고지
  ...
  → [선택] raw v2 진단 metadata 처리 / 자동신고 / 비식별 AI 개선·학습
  → OTP 발급 요청
...
- 가입 화면: 서비스 약관, 개인정보 처리, 위치기반서비스 약관을 필수 3개로, raw v2 진단 metadata 처리·자동신고·비식별 학습 재사용을 선택 3개로 구분한다. 선택값은 기본 OFF이다. 직접 신고는 raw·자동신고 선택과 분리하고 매 신고 전 사진·정확 위치 전송을 명시적으로 다시 확인한다.
