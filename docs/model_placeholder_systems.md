...
YOLO/ONNX 모델이 아직 PWA에 연결되지 않았기 때문에, 병렬 개발을 위해 임시로 넣은 시스템을 따로 추적한다. 이 문서의 항목은 최종 모델 연결 전까지는 데모/통합 개발용으로만 사용한다.
...
| 서버 추론 placeholder | `backend/app/detector.py`, `backend/app/main.py` | `/detect` 요청에 `model_unavailable` 반환 | 실제 모델 어댑터가 `DetectionEvent` 목록을 `source: "server"`로 반환 |
...
7. `/detect/health`가 `ready`를 반환하도록 `backend/app/detector.py`를 교체한다.
