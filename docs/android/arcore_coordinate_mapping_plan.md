...
- 아래 단계 1~4의 코드 경로는 연결됐고, 단계 5 실기기 검증만 미완료다. 본문의 구현 제안은 현재 코드 설명이 아니라 설계 이력과 회귀 기준으로 읽는다.

## 배경

초기 Android 앱에는 detector의 normalized bbox를 preview view 좌표와 depth image 좌표에 그대로 대응시키는 identity 가정이 있었다. 현재는 frame-aware mapper 우선 경로가 연결됐지만 mapper 부재 시의 identity fallback과 실기기 정합 위험은 남아 있다. identity 가정은 다음 조건이 모두 맞을 때만 안전하다.
...
단계 1~4는 코드/metadata 경로가 연결됐다. 단계 5는 아직 Device PASS가 아니다.
