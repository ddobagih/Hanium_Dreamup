...
> Status: SNAPSHOT / PLATFORM SUPERSEDED. 모델 학습 완료 전이자 Android-primary 가정을 사용한 문서다. 2026-07-10 확정된 Web/PWA 주 앱 기준은 `../../status/implementation_audit_20260710.md`와 `../../status/current_status.md`를 우선한다.

작성 기준: 2026-07-01 KST, `/home/ddobagi/Code/hanium-dreamup`.

이 문서는 `docs/review/requirements/walksafe_implementation_plan_20260630.html`, `docs/evidence/feature_matrix.md`, `product/*`, `docs/android/*`, `docs/walksafe-v2/*`, `apps/android`, `backend`, `apps/web`, `model`, `scripts`, `configs`를 10개 조사 축으로 나눠 확인한 요구사항 추적표다.
...
| WS-REQ-029 | Backend Navigation | TMAP-only walking route proxy와 POI search를 제공한다. | `docs/walksafe-v2/navigation_integration_policy.md` | `backend/app/api/navigation.py`, `tmap_pedestrian.py` | `backend/tests/test_navigation_routes.py` PASS | DONE | live key/quota/실경로 smoke는 제외 |
...
| WS-REQ-039 | Admin List/Detail | Admin report list/detail/filter/status를 제공한다. | 6/30 HTML Admin/Ops | `apps/web/app/admin/page.tsx`, backend stable `limit`/`offset` list/status endpoints, Android source/unified model filter option | web typecheck/lint, admin policy script, backend pagination test PASS | DONE | 실제 조직 계정 lifecycle은 별도 운영 과제 |
...
2. live map SDK와 bulk workflow polish: 후속 UI/운영 lane. 정적 heatmap과 50건 단위 pagination은 구현됨.
