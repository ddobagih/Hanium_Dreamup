...
- 현재 backend `reports.source` enum은 `fake`/`onnx`/`server`/`android`다. `source=android`는 Android native upload를 admin/export에서 분리하기 위한 값이며, `reporter_user_id`와 gate status는 아직 서버 auth subject나 field PASS 근거가 아니다. fake를 성능 리포트에 포함하지 않는다.
