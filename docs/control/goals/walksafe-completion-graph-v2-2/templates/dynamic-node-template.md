...
USER/EXTERNAL blocker 요청은 사건 직전 ready frontier의 첫 Goal이면서 기존 runtime focus인 결정적 초점만 대상으로 한다. owner·request kind·구체 action·대상 Goal 경로/내용 hash/Work Item 유형·정규화한 해제 증거 role·권한 요구를 canonical logical basis로 만들고 그 SHA-256을 전역 고유 request key로 사용한다. 활성 요청은 다른 내부 ready branch를 선점하지 않으며 `pending_questions`는 활성 USER request와 일대일로 유지하되 내부 frontier가 남으면 `DEFERRED_INTERNAL_FRONTIER`로 전달을 유예한다.

`ARTIFACT_WORK`는 `ARTIFACT_REGISTER` SHA에 결속된 `artifact_work_queue`에서 materialize하고, `output_subject_ids_by_role`의 `ARTIFACT_REGISTER`·`ARTIFACT_CHANGE_LOG`에 같은 단일 `DLV-*` 대상을 고정한다. 같은 대상의 live Goal은 하나뿐이다. `artifact_work_reason`은 네 허용값 중 하나이며 `ACTIVE_EVENT_UPDATE`는 실제 trigger JSON의 `document_id/path/file_sha256` direct binding을 materialization event의 refs와 정확히 결속해야 한다. 일반 작업은 현재 queue의 허용 상태와 일치하는 `ACTIVE`, `APPLICABILITY_DECISION`만 `PENDING_EVALUATION` 대상을 허용한다. 일반 내부 ready frontier가 소진됐을 때 queue의 첫 assessment 대상은 필요한 reason의 Goal 또는 허용된 register 종결 disposition으로 소비해야 한다. 함께 생산하는 요구·설계·모듈·시험 role에는 실제 행 ID 범위를 추가한다. `CANONICAL_BINDINGS_UPDATED`의 생산자 출력 범위, 실제 register delta, append된 change-log affected 범위가 생성 시 선언과 정확히 같아야 하므로 실행 중 범위를 조용히 넓힐 수 없다. successor revision도 reason·trigger·`source_blocker_ids`·`output_subject_ids_by_role`을 포함한 의미 범위를 바꿀 수 없으며, 대상이나 출력 범위가 달라지면 새 Work Item으로 만든다.
...
- USER/EXTERNAL은 typed request와 전역 고유 canonical logical key를 기록하고, USER 질문은 내부 frontier가 소진될 때까지 전달을 유예
