...
FP-017/GAP-026은 Phase A 내부 slice 뒤 다음 정책을 선택하기 위한 단 하나의 bootstrap consumed pair로만 봉인하고 Workstream coverage 계산에만 사용한다. 이는 GAP-026의 `PARTIAL`, 정식 시험·기기시험 `NOT_RUN` 경계를 바꾸거나 실제 Work Item 완료를 뜻하지 않으며 다른 PARTIAL 정책을 완료로 추정하는 규칙도 아니다.

현재 `package_status=PREPARED_NOT_ACTIVATED`, `activation_status=READY_NOT_ACTIVATED`다. 사용자가 이 graph-v2-2 패키지의 활성화를 명시적으로 요청하기 전에는 새 실행 event나 기능 변경을 시작하지 않는다. 활성화 요청 뒤에는 사용자 승인 원본과 별도 quick-gate receipt를 결속한 `PACKAGE_ACTIVATED`를 먼저 기록하되 Goal 상태는 바꾸지 않는다. 제품 코드 작업 전에는 재개 runbook §10.1의 전체 구현 시작 검사를 추가로 통과해야 하며, 그 뒤 별도 `GOAL_STARTED`로 선택된 Work Item 하나만 `IN_PROGRESS`로 전환한다. 새 터미널에서 그 Work Item을 계속할 때는 변경된 통제 bytes를 checkpoint working snapshot에 한정해 reconcile하고 전체 검사를 다시 통과한 `WORK_SESSION_RESUMED`를 먼저 기록한다. 이전 실행 event의 pre-work snapshot은 역사 receipt로 보존한다.
...
2. fail-first 검증을 만들고 최소 변경을 구현한 뒤 실제 변경 hash를 가진 `IMPLEMENTATION_RECORD`를 확정한다.
3. targeted·구성요소·회귀 검증을 실행하고 명령·exit code·원출력 hash를 가진 `VERIFICATION_RESULT`를 확정한다.
4. 실제 사실에 맞게 Draft·Active 산출물과 Gap·Backlog successor bytes를 확정한다. 새 Gap의 evidence catalog에는 앞의 구현·검증 두 결과만 exact hash로 넣는다.
5. 새 Gap·Backlog hash가 정해진 뒤 resulting canonical binding을 가진 post-update `SUCCESSOR_TRACE`를 만들고, 세 결과를 독립 review한 뒤 그 review까지 결속한 `WORK_ITEM_EXECUTION_RECEIPT`를 남긴다.
6. staged snapshot에서 정책·Gap별 변화와 전역 변화를 계산해 `CANONICAL_BINDINGS_UPDATED`를 적용하고, 허용된 내부 생산자 Goal은 바로 다음 `GOAL_COMPLETED`에서 같은 receipt·증거를 결속해 완료한다.
...
8. 다음 Work Item이 필요하면 완료 전환 뒤 `GOAL_MATERIALIZED → GOAL_READY` 순서로 추가하고 준비되지 않은 Goal을 바로 시작하지 않는다.
...
- 산출물 queue에서 blocker와 upstream이 해소된 대상이 생기면 선언된 결정적 순서의 단일 `DLV-*` `ARTIFACT_WORK`를 만든다. 일반 ready frontier가 소진되면 첫 `WAITING_APPLICABILITY/WAITING_TRIGGER` 평가 대상을 적용성 결정·실제 trigger direct binding·허용 종결 disposition 중 하나로 처리하며 조용히 정지하지 않는다.
...
사용자 결정 질문과 외부 action/evidence 요청은 분리한다. 새 blocker 요청은 사건 직전 ready frontier의 첫 Goal이면서 기존 runtime focus인 결정적 초점만 대상으로 한다. request key는 owner·kind·action·대상 Goal 경로·hash·유형·정규화한 해제 증거 role·권한 요구를 canonical JSON으로 만든 SHA-256이므로 blocker ID만 바꾼 재발행을 허용하지 않는다. 활성 요청은 다른 내부 ready branch를 선점하지 않으며, USER 질문은 활성 request와 일대일로 기록하되 내부 frontier가 남으면 전달을 유예하고 그 branch를 계속 진행한다.

저장소 자율 범위를 모두 완료했다는 판정에는 내부 `PLANNED/BLOCKED` leaf와 비종결 artifact queue가 없다는 사실도 포함한다. 그 뒤 외부 action/evidence만 남으면 `COMPLETE_AWAITING_EXTERNAL` 파생 milestone과 대상·action·candidate·receipt role·authority를 결속한 event-scoped 실행 packet을 남기되 Master·package·출시를 완료로 표시하지 않는다. packet은 `ISSUED_NOT_EVIDENCE` 요청서이며 완료나 외부 attestation을 대신하지 않고 완료 evidence로 참조할 수도 없다. 내부 독립검토도 외부 법률·전문가·실사용자·실기기 검토를 대신하지 않는다.
