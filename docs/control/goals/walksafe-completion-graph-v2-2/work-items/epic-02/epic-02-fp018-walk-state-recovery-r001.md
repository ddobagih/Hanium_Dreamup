...
이 Goal의 완료수준은 저장소 내부 정책 정합 구현과 Gap 재평가다. 실제 기기와 `TC-FP-018-01~06` 정식 PASS는 정식 검증 Workstream의 동적 시험 Goal로 남긴다.
...
7. 실제 변경 경로·전후 hash를 가진 append-only `IMPLEMENTATION_RECORD`를 확정한다.
8. targeted JVM test와 MainActivity 정적/구성요소 검사, Android debug build·lint 회귀를 실행하고 명령·exit code·원출력 hash를 가진 `VERIFICATION_RESULT`를 확정한다.
9. Active overlay와 Gap·Backlog successor bytes를 실제 결과에 맞게 확정하고 새 Gap evidence catalog에는 구현·검증 두 결과만 exact hash로 넣는다.
10. 새 Gap·Backlog hash가 정해진 뒤 resulting canonical binding을 가진 post-update `SUCCESSOR_TRACE`를 만든다.
11. 독립 서브에이전트가 세 결과를 결속해 정책 누락·자동재개 경로·완료 과장을 재검토한 뒤 completion receipt를 만든다.
12. `CANONICAL_BINDINGS_UPDATED → GOAL_COMPLETED` 순서로 같은 receipt를 결속하고, 다음 정책 Work Item은 그 뒤 `GOAL_MATERIALIZED → GOAL_READY` 순서로만 준비한다.
...
1. append-only 구현기록·검증결과 뒤 Active overlay와 r008을 보존한 Gap·Backlog successor bytes를 확정한다.
2. 새 Gap·Backlog hash를 결속한 post-update successor trace, 독립 review, completion receipt를 순서대로 만든다.
3. 부모 [`../../workstreams/epic-02-safe-walk-state-and-permissions.md`](../../workstreams/epic-02-safe-walk-state-and-permissions.md)의 내부 완료수준과 내부 잔여작업을 재평가한다.
...
5. successor, 다음 Goal, 경로·내용 hash, `CANONICAL_BINDINGS_UPDATED → GOAL_COMPLETED → GOAL_MATERIALIZED → GOAL_READY` 전환 순서와 새 canonical head용 continuation snapshot 계약을 격리 사전검사한다.
...
7. 중간 단계가 실패하면 체크포인트를 바꾸지 않고 준비 파일을 폐기하거나 고친다. commit 뒤 일반 continuation·Goal 검사와 관련 회귀를 통과하면 사용자 질문 없이 checkpoint의 ready frontier 선택 규칙이 고른 다음 leaf를 진행한다.
