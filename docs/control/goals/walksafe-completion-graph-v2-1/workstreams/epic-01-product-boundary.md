...
다음은 정식 검증 Workstream의 실행 증거로 남아 있다.
...
현재 Goal은 `COMPLETE_AT_TARGET/IMPLEMENTATION_READY`다. 정식 시험이나 출시 완료로 승격하지 않는다. 정식 검증 Workstream은 위 외부 작업을 동일 release candidate에서 수행한다.
...
완료 뒤 checkpoint의 ready frontier를 다시 계산하고 선택 규칙이 고른 다음 leaf를 진행한다. 현재 canonical next action은 EPIC-02의 FP-018이지만 특정 Workstream을 고정 후속으로 자동 시작하지 않는다.
