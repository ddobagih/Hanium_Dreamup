...
class WalkSafeGoalGraphTest(unittest.TestCase):
    def setUp(self) -> None:
        self._anchors = (
            graph.EXPECTED_MANIFEST_SHA256,
    ...
    def test_policy_gap_work_item_binds_exactly_one_pair(self) -> None:
        ...
        self.assertEqual(node["source_policy_ids"], ["FP-018"])
        self.assertEqual(node["gap_ids"], ["GAP-027"])
    ...
    def test_snapshot_cutoff_can_advance_without_rewriting_initial_event(self) -> None:
        with self.fixture_root() as root:
            ...
            self.activate_package(root, checkpoint)
            state = checkpoint["goal_execution"]
            activation = state["transition_history"][-1]
            activation["occurred_on"] = "2026-07-24"
            activation["occurred_at"] = "2026-07-24T19:41:01+09:00"
