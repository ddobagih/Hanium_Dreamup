# WalkSafe v2.5 비효력 후보 correction 로드맵 R019

- 작성일: `2026-08-02`
- 상태: `PENDING_TWO_INTERNAL_REVIEWS`
- replaces: rejected R018
...
R018은 구조 검수와 회의적 검수에서 각각
`BLOCKING=1 MAJOR=0 MINOR=0`으로 거부됐다. 두 검수는 R017의 namespace,
exact-existing retry, failed-r001 physical preservation 결함은 모두 닫혔다고 확인했지만,
고정 `PREPARED_AT=2026-08-02T11:00:00+09:00`이 r001 실패와 R017/R018 실행
권위보다 앞서 physical candidate의 provenance를 역전시킨다고 판정했다.

R019는 R018의 closure를 모두 유지하면서, 새 dual review가 완료된 뒤 실제 P2 준비
시작 시각을 한 번 관측해 three-source에 봉인한다. r001은 물리적으로 보존하고 r002
candidate만 add-only로 준비·공개·검수한다. R016의 truthful delegation, zero-credit,
strict chronology, monotonic freshness와 pre-C1 no-resume 계약은 유지한다.

R016/R017/R018에서 예고한 activation·Goal·제품 단계는 R020 이후로 밀린다.
...
test preimage의 공개 전 absent·공개 후 present dual-state oracle은 유지한다. R019는
revision namespace, provenance, r001 preservation, publication retry semantics,
preparation timestamp와 그 negative oracle만 명시적으로 수정할 수 있다.
...
## 3. R019 epochs와 allowlist

1. P0 — 이 R019 문서 add-only
2. P1 — R019 plan review 두 개 add-only
3. P2 — preparation time 관측 뒤 three-file source correction
...
DELEGATION_REQUIREMENT_ID = WS-V25-R022-CURRENT-LIVE-SESSION-DELEGATION-20260802-R019-R002
...
### 4.3 observed truthful preparation timestamp

R019에는 미래 또는 임의의 고정 `PREPARED_AT`을 미리 쓰지 않는다. P1 dual PASS와
모든 P2 precondition을 재확인한 직후, 첫 source patch 전에 다음 알고리즘으로 한 번만
선택한다.

1. 다음 exact predecessor들의 no-follow `lstat`에서
   `max(st_mtime_ns, st_ctime_ns)`의 최댓값 `PREDECESSOR_MAX_NS`를 구한다:
   immutable r001 root+six, R017 roadmap+reviews, R018 roadmap+reviews,
   R019 roadmap+두 PASS reviews.
2. `Asia/Seoul` wall clock의 `time.time_ns()`를 관측하고 microsecond 경계로 내림한
   `SELECTED_NS = observed_ns - observed_ns % 1000`을 계산한다.
3. `SELECTED_NS > PREDECESSOR_MAX_NS`가 아니면 다시 관측한다. 성립한 첫 값을
   `YYYY-MM-DDTHH:MM:SS.ffffff+09:00` exact 형식으로 직렬화한다.
4. 그 한 값을 `P2_PREPARATION_STARTED_AT` 및 physical `PREPARED_AT`으로 사용하고,
   같은 `apply_patch` P2 epoch에서 core, builder, test에 exact 동일 literal로 봉인한다.
   `PREPARED_ON`은 그 값을 parse한 Asia/Seoul calendar date에서 파생한다.

관측값은 microsecond 내림이므로 선택 시점보다 미래가 아니다. source patch 뒤에는 다음
모두가 참이어야 한다.

```text
PREDECESSOR_MAX_NS < PREPARED_AT_NS
PREPARED_AT_NS <= min(each changed source st_mtime_ns, each changed source st_ctime_ns)
PREPARED_AT_NS <= immediate post-patch time.time_ns()
core_literal == builder_literal == test_literal
timezone == +09:00
fractional_precision == 6
```

R019 reviews가 끝나기 전 선택, 초 단위 반올림으로 미래값 선택, source 간 다른 literal,
source patch 뒤 timestamp 재선택·수정은 금지한다. 위 oracle이 하나라도 실패하면 P3는
열리지 않으며 r002는 쓰지 않고 새 roadmap/source revision이 필요하다.

synthetic fixture는 별도 calendar literal을 갖지 않고 이 base에서 exact offset으로
파생한다.

```text
synthetic core reviewed_at = base + 5 minutes
synthetic request generated_at = base + 10 minutes
synthetic receipt recorded_at = base + 15 minutes
synthetic quick started_at = base + 15 minutes 30 seconds
synthetic quick completed_at/checked_at = base + 16 minutes
synthetic quick valid_until = base + 26 minutes
synthetic seq3 occurred_at = base + 16 minutes + 1 microsecond
```

core의 하나의 timezone-aware helper가 offsets를 canonical RFC3339로 만든다. builder와
tests는 이 helper/동일 base를 사용하고 독립 날짜 literal을 복제하지 않는다. strict
`seq1 < seq2 < seq3`, exact `+1us`, expiry와 599/600/601 monotonic tests를 유지하며
timezone-invalid fixture도 base에서 파생한다. synthetic 미래 fixture는 명시적
`fixture_only`이고 live authorization으로 인정하지 않는다.

## 5. provenance and failed-r001 validator

### 5.1 provenance chain

P1 뒤 실제 R019 plan/review hashes만 source에 넣는다. R016은 accepted predecessor,
R017/R018과 각 reviews는 rejected history, R019와 두 PASS reviews는 current execution
authority로 구분한다.
...
r019_roadmap
r019_structural_review
r019_skeptical_review
candidate_structural_review
candidate_skeptical_review
control_core_review
```

R019 reviews는 r002 build 전에 존재하므로 physical pin 가능하다. r002 outputs,
...
## 8. R019 independent plan review

P1 exact files:

```text
docs/control/execution/artifact-closure/run-20260727-001/WALKSAFE-AUTONOMOUS-EXECUTION-ROADMAP-20260802-R019-independent-structural-review-r001.md
docs/control/execution/artifact-closure/run-20260727-001/WALKSAFE-AUTONOMOUS-EXECUTION-ROADMAP-20260802-R019-independent-skeptical-review-r001.md
```

둘은 같은 R019 SHA/bytes/lines와 다음을 검증한다.

- R017/R018 findings와 fixed-time provenance inversion closure
- timestamp selection의 strict-after/non-future/source-equality/one-shot oracle
- r001 content+lstat immutable seal
- r002 exact paths/IDs/events/gate-derived paths와 intentional historical literals
- R016/R017/R018/R019 provenance DAG와 cycle 0
...
authority_granted=R019_R002_CANDIDATE_CORRECTION_ONLY
```
...
R019 dual PASS 뒤 §4.3 값을 관측하고 three files만 한 `apply_patch` epoch로 수정한다.
tests는 다음을 같은 37개 universe에 고정한다.

- §4 paths/IDs/event IDs와 observed timestamp·derived offsets
- R016/R017/R018/R019 binding key·hash mutation negatives
...
1. C0/R002/R016/R017/R018/R019 physical pins와 r001 §2 content+lstat exact
2. r002 target absent/non-symlink
3. three source preimage CAS
4. §4.3 timestamp one-shot selection과 three-source patch
5. §4.3 post-patch physical time/equality oracle와 새 three source SHA capture
6. AST/duplicate literal-key/trailing-whitespace 검사
7. current-construction namespace residual scan
...
outputs에서 core와 두 wrappers를 검증한다. suite 뒤 source/r001/timestamp CAS를 다시
확인한다.

residual scan은 §4 current identifiers에 old r001/001 literal 0을 요구하되,
failed-r001 validator constants, PLAN_ROOT, reviewed R002/design/R016/R017/R018 history와
first review suffix처럼 열거한 historical roles만 허용한다.

## 10. P3 publication and post-build gate

P3 직전 r001/source/timestamp/C0/R002/provenance CAS와 r002 absence를 다시 확인한다.
write 결과는 정확히 `PUBLISHED_NEW`만 허용한다.

P3 직후 순서:

1. r001 §2 content+lstat, source/timestamp, r002 root+exact six CAS capture
...
PASS를 요구한다. 두 번의 read-only check 전후에 r002 root와 각 file의
`(dev,ino,mode,uid,gid,nlink,size,mtime_ns,ctime_ns,sha256)`를 비교한다. root에는
hash 대신 exact non-symlink directory type와 exact six-entry set을 적용한다.

모든 argv/exit/stdout/stderr SHA, source 시작/종료 SHA, observed timestamp와
r001/r002 hashes/bytes/physical tuples를 candidate review에 기록한다.

## 11. P4 r002 candidate reviews

두 독립 검수자는 exact r002 six outputs, output manifest, generator/source pins,
R016/R017/R018/R019 chain, observed timestamp selection oracle, pre/post suite,
namespace residual scan, publisher terminal과 두 candidate checkers를 재검증한다.
...
1. R020 — exact r002/reviews 기반 one-shot monotonic lock/CAS, resolved review,
   checkpoint-last v2.5+r022 activation
2. R021 — seq3 immutable prefix 뒤 Goal replay와 v2.5 full19/repository-state
3. R022 — FP008 materialized/ready, v2.5 full19, `GOAL_STARTED`
4. R023 — Android adminapp read-only 신고 목록/상세 최소 slice와 종료검수

R020 PASS 전 canonical/checkpoint write 0, R021 PASS 전 Goal write 0, R022 valid
...
```text
R019_DUAL_PLAN_REVIEW_PASS
AND OBSERVED_PREPARATION_TIMESTAMP_CAUSAL
