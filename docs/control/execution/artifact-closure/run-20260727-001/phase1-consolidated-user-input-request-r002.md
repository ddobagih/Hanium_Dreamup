...
- 선행본: `phase1-consolidated-user-input-request-r001.md`
...
각 ID의 attestation subject가 기준일 현재 사실과 일치하는지 두 역할이 순서대로 판정합니다. 먼저 `INDEPENDENT_QA_REVIEWER`가 `accept`, `reject`, `return` 중 하나를 기록하고, 이어서 `SERVICE_OWNER` 또는 `PRODUCT_OWNER`가 `approve`, `reject`, `return` 중 하나를 별도로 기록합니다.
...
- `approve`: service/product owner가 독립 QA 판정까지 포함된 exact attestation을 승인한다는 판정이다.
- 고정 `decision_item_manifest_sha256`: `0d3798d5f3dd0751393741ef54b755cc86bcb9bd1cf0867157a65b983e1a95cd`
...
| Owner | `approve` / `reject` / `return` |  |  |  | `SERVICE_OWNER` / `PRODUCT_OWNER` |  |  |
