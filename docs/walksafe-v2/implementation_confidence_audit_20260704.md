...
| PWA/Web 제품 표면 | PARTIAL | Android가 제품 앱인데 PWA manifest/install/offline UI가 제품처럼 보일 수 있다. |
...
1. 제품 표면 격리: PWA manifest/install/offline UI, Android debug UI, backend debug router, fake detector 기본값을 demo/dev로 더 강하게 분리한다.
