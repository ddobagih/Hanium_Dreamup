...
- 기준일: 2026-07-13 KST
- 2026-07-13 구현 정합 보정: readiness, role protection, bounded summary/export, inference worker 복구와 audit 좌표 redaction 반영
...
| `GET /ready` | 추가 | DB/Alembic·upload fsync·real detector warm-up·TMAP dependency readiness |
| `GET /navigation/walking/health`, `POST /navigation/walking` | 추가 | TMAP 보행 경로 proxy와 앱 전용 route schema |
...
`WALKSAFE_FIELD_TEST_SECURITY_ENABLED=true`가 기본이다. detect/report 생성/navigation/health/ready는 field 역할, report 조회·검수·summary/export·upload/debug/OpenAPI는 admin 역할로 분리하고, 보호된 operation은 named actor와 30초 HMAC assertion을 요구한다. 보안 비활성은 `development`/`test`와 명시적 insecure-local opt-in을 함께 사용한 경우만 허용되며 field/staging/production에서는 시작 단계에서 거부된다.
...
`trigger=auto`는 `auto_reported=true`, `trigger=voice`는 `false`여야 한다. `source_model`은 model identifier만 허용하며 제어문자, filesystem path와 URL은 거부한다.
...
- client `data_origin`, `runtime_mode`, provenance, actor와 performance 판정은 신뢰하지 않고 server 수신 source/session 값으로 덮어쓴다.
- v2 손상 점자블록 신고는 GPS가 있어야 저장한다. GPS가 없으면 422로 거부한다.
- image는 MIME·확장자·signature·size 확인 뒤 Pillow decode와 metadata-free 재인코딩을 필수로 한다. decode/sanitize 불가 입력이나 Pillow 미설정은 원본 보존 없이 거부한다.
- 중복 후보는 class/location/time 기준으로 기존 정책을 재사용한다.
- image 저장 뒤 duplicate/DB 작업이 실패하면 해당 요청에서 만든 파일을 보상 삭제한다.
...
관리자/운영자가 현재 필터 조건의 신고 목록을 검수하고, 외부 기관에 수동 신고할 CSV를 준비하기 위해 내보내는 endpoint다.
...
GET /reports/export?format=csv&profile=minimum
GET /reports/export?format=csv&profile=agency&status=reviewed&class_name=damaged_tactile_block
```

- 기본 포맷: CSV
- 선택 포맷: `format=json`, `format=geojson`
- GeoJSON은 `FeatureCollection`을 반환한다. 위치가 있는 신고는 Point Feature, 위치가 없는 신고는 `geometry: null`로 둔다.
- `aggregate=grid`는 외부 지도 SDK 없이 관리자 정확 위치 summary grid를 GeoJSON FeatureCollection으로 반환한다. `profile=internal`만 허용하며 redacted/minimum/agency 조합은 400이다.
...
- `profile=minimum`은 좌표를 소수 2자리로 낮추고 image path·신고자/trace/bbox/중복 ID/note/reason을 비운다.
- `profile=agency`는 named 관리자가 검수한 reviewed·damage·high≤15m·non-fake만 기관 신고 최소 필드와 정확 좌표로 남긴다. `performance_excluded`는 모델 성능 집계 경계로 유지하되 기관 human review 적격성과 분리한다. 손상 지점 식별에 정확 위치가 필요하므로 redacted와 함께 쓸 수 없다.
- `bom=true`는 CSV에 UTF-8 BOM을 붙이며 모든 text cell은 spreadsheet formula prefix를 escape한다.
- `manifest=true`는 JSON export를 manifest wrapper로 감싸 필터, 건수, `rows_sha256`를 함께 기록한다.
- export는 최대 10,000행이며 초과하면 truncate하지 않고 413으로 필터 축소를 요구한다. durable export audit에는 exact `lat`/`lng` query를 저장하지 않고 `exact_location_filter_redacted`만 남긴다.
- CSV fields:
  - `id`
  - `status`
  - `class_name`
...
현재 export는 기관 자동 API 전송이 아니다. 관리자가 검수·필터링한 뒤 CSV를 내려받아 외부 기관에 직접 수동 신고하는 운영 흐름이다.

## 6.1 `GET /reports/duplicate-check`

field 역할에서 class/time/location 기반 후보를 조회하지만 응답은 `duplicate_report_ids`와 `duplicate_count`만 반환한다. 신고 좌표, image path, review note나 전체 metadata는 반환하지 않는다.

## 7. `GET /reports/summary`

`GET /reports`와 같은 필터 조건으로 전체 개수, 상태/source count, 위치 있음/없음, 좌표 격자 Top cluster를 반환한다. Admin은 안정 정렬된 `limit`/`offset` 페이지를 사용하며, summary는 현재 페이지와 별개로 전체 filtered row를 요약한다. 최대 10,000행을 넘으면 truncate하지 않고 `413 report_summary_too_large`를 반환한다.

## 8. `GET /navigation/destinations/search`

목적지 이름을 앱 전용 후보 schema로 정규화한다. 제품 환경은 TMAP POI live 고정이며, `TMAP_POI_PROVIDER=mock`은 외부 호출 없이 로컬 mock/alias 후보만 반환하는 테스트 fixture 모드다.
...
응답 후보는 `id`, `name`, `point`, `address`, `road_address`, `category`, `result_type`, `distance_m`를 포함한다. `result_type`은 `poi`, `address`, `alias` 중 하나다. 한 응답 안의 비어 있지 않은 후보 ID는 고유해야 하며 중복이면 전체 provider 응답을 거부한다. Android도 normalized response query가 요청과 같은지, 요청 limit 이내인지, 모든 후보의 ID/type/좌표/필드를 확인하고 하나라도 malformed면 부분 후보를 사용하지 않는다. 검색 health는 key 설정 여부만 확인하고 실제 provider를 호출하지 않는다.

## 9. `POST /navigation/walking`

TMAP 보행자 경로안내 API를 서버에서 호출하고, 프론트가 쓰기 쉬운 앱 전용 schema로 정규화한다. TMAP appKey는 백엔드 env에만 둔다. 현재 제품 계약은 `WALKING_ROUTE_PROVIDER=tmap_pedestrian` 고정이며 다른 provider 설정은 지원 범위가 아니다.
...
| `radius_m` | int | route 요청 schema 호환 필드. 현재 TMAP 요청에는 사용하지 않음 |
...
`guide_points`는 TMAP Point feature의 안내 지점을 정규화한 배열이다. `description`이 `", 73m"`처럼 안내 의미가 없는 거리 문구뿐이면 `instruction`은 `null`이 될 수 있다. `bearing_deg`는 route polyline의 다음 segment 방향에서 계산한 0 이상 360 미만의 보조 heading이다.

Android route client는 provider/schema/result/`STAIR_AVOID`, 양수 summary, 요청 origin/destination과 polyline 양끝 100m 이내 정합, polyline geometry-summary distance 비율, guide index·route 투영·선언 거리의 단조 순서를 모두 검사한다. 하나라도 어긋나면 일부 geometry/guide를 사용하지 않고 전체 응답을 거부한다.
...
| env | 의미 |
|---|---|
| `WALKING_ROUTE_PROVIDER` | `tmap_pedestrian` 고정 |
...
- TMAP provider 응답을 실사용 안전 판단으로 과장하지 않는다. 위험 경고는 WalkSafe risk evaluator가 우선한다.
