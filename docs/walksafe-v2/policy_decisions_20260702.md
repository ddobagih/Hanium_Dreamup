...
작성 기준: 2026-07-02 KST, 2026-07-11 구현·기관 운영 정합 보정.
...
| ID | 항목 | 결정 |
|---|---|---|
| P-01 | 제품 1차 가치 | 점자블록 손상은 자동 신고 형태로 처리한다. 자동 결과는 aria-live와 haptic으로 알리되 TTS는 하지 않는다. 사용자가 명시적으로 "신고해줘"라고 요청한 경우에는 TTS도 낸다. |
...
| P-06 | 신고 이미지와 기본 정보 | report image는 사용자 기기에 저장하지 않고 서버에 저장한다. 위치, 신고자 식별자, 신고 시각 같은 기본 report 정보도 서버에 저장한다. 얼굴/차량번호 모자이크는 하지 않는다. Pillow decode·metadata-free 재인코딩을 필수로 하고 불가능하면 저장을 거부한다. 6개월 후 report row와 image를 자동 삭제한다. |
...
| P-08 | 위험 알림 민감도 | trusted distance/relative-motion TTC는 3초 이하 STOP, 3~10초 WARNING, 10~30초 AWARE로 둔다. bbox scale TTC는 물리 계측이 아닌 휴리스틱이므로 3초 이하라도 단독 STOP/high로 쓰지 않는다. AWARE는 내부 상태다. |
...
| P-13 | 기관 신고 | 공공기관 자동 API 제출은 제품 범위에 넣지 않는다. named 관리자가 검수한 reviewed·damage·high≤15m·non-fake의 정확 위치 최소 `agency` CSV/manifest를 내려받아 외부 기관에 수동 신고하고 receipt를 기록한다. `performance_excluded`는 모델 성능 집계 경계로 유지하되 기관 human review 적격성과 분리한다. 임의 선택·병합 기능은 없다. |
...
| P-17 | 운영 서비스 방향 | 기능 사용은 로그인 필수다. fail-closed named actor·역할 분리·파일 limiter·logout/audit와 retention apply 도구는 구현했으며, 실제 운영 account/storage/scheduler/backup·restore evidence는 release gate로 강제한다. |
| P-18 | 앱 플랫폼 | 주 사용자 앱은 Web/PWA다. Android native는 ARCore/depth/TFLite 실험·검증 보조 경로로 유지하며 두 경로의 evidence를 서로 대체하지 않는다. |
| P-19 | 13-class 역할 | 13개 class를 모두 핵심으로 유지한다. `damaged_tactile_block`만 자동 신고, `normal_tactile_block`은 활성 경로 보조, `traffic light`는 존재 표시만, 나머지 차량·보행자·자전거·노면·킥보드·횡단보도는 위험/경로 문맥에 사용한다. |
| P-20 | 자동 신고 안정성 | 손상 점자블록은 confidence 0.70 이상, GPS accuracy 15m 이내, 공간적으로 같은 후보가 1.8초 이내 3개 연속 프레임에서 확인돼야 자동 신고한다. 성공·실패는 aria-live+haptic, 자동 TTS 없음으로 처리한다. |
| P-21 | 현장 TTC | trusted TTC에 3/10/30초 STOP/WARNING/AWARE를 적용한다. AWARE는 내부 상태이고 high/STOP만 진동한다. bbox TTC·단안 거리는 휴리스틱/추정으로 표시하고 단독 STOP이나 센서 계측으로 표현하지 않는다. |
| P-22 | 점자블록 길안내 | 보행 provider를 TMAP `STAIR_AVOID`로 고정하고, 정상 점자블록이 활성 경로 방향·이동 상태·약 3~5초 camera future ROI와 맞을 때만 근거리 보조 안내한다. 점자블록을 실제 route graph로 만들거나 TMAP 경로가 점자블록 전체 망을 반영한다고 주장하지 않는다. |
| P-23 | 현장 데이터 | field 명시 동의 session의 allowlist JSONL은 약 1 Hz, server 날짜 7일 보존으로 둔다. raw/base64 camera/audio를 endpoint가 거부하고 수집 중지·현재 session 삭제를 제공한다. 스냅샷은 별도 동의 경로다. |
| P-24 | 현장 기준값 | class별 출시 recall·오탐률·지연시간 하한은 2026-07-11 field 관찰값으로 시작하고 실제 외출 로그를 본 뒤 확정한다. 미확정 상태를 출시 승인으로 해석하지 않는다. |
...
- Web은 `reporter_user_id`를 보내지 않고 Android local ID도 인증 subject가 아니다. gateway named actor는 ingestion/review/export audit에 남지만 외부 IdP identity를 뜻하지 않으며 실제 운영 account 구성은 release gate 대상이다.
...
- Release 이후에는 인증된 user id를 report subject로 남기는 것을 목표로 한다. 현재 optional `reporter_user_id`는 인증값이 아니다.
...
- fail-closed named actor, field/admin 역할 분리, 파일 login limiter, logout과 actor audit은 구현됐다. 운영 account/token/session secret, storage 보호와 append-only audit evidence는 release gate/후속 운영 범위다.
...
- report 삭제는 기본 dry-run이며 guarded apply가 DB lock·image quarantine·actor/audit manifest를 남긴다. telemetry는 7일 apply receipt를 남긴다.
- 실제 운영 scheduler, report apply manifest와 OpenPGP backup/restore evidence가 없으면 release gate가 FAIL한다.
...
| Q-07 | 빨간 신호등 STOP | 확정: 현재 MVP는 `traffic light` 존재만 탐지하며 색상·횡단 가능 판단은 하지 않는다 |
