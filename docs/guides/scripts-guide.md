...
| 백업·복구 drill | [`backup_walksafe_data_20260711.sh`](../../scripts/backup_walksafe_data_20260711.sh), [`restore_walksafe_backup_drill_20260711.sh`](../../scripts/restore_walksafe_backup_drill_20260711.sh) | `CURRENT / DB_DEVICE` | 문서화된 전용 0700 경로를 사용하고 source는 `DATABASE_URL`, target은 `TARGET_DATABASE_URL` 환경변수로만 전달한다. DB URL을 CLI 인자에 넣거나 shell로 우회 실행하지 않음 |
