...
절 4의 목표 수치는 **기기 1대에서 달성 가능함을 확인했고, 승인 게이트를 제외한 4개를 구현했다.**
확인한 것은 "이 기준이 정상 기기를 배제하지 않는다"이며, "이 기준이 보행 안전에 충분하다"가
아니다. 표본이 1대·1지점이므로 값을 확정하기 전에 다른 기종과 실외에서 재현해야 한다.

| 절 | 항목 | 상태 |
|---|---|---|
| 4.1 | GPS 15 m + 차단 승격 | **구현됨** |
| 4.1.1 | 실패 사유 분리 | **구현됨** |
| 4.2 | 카메라 10프레임 중 80 % | **구현됨** |
| 4.3 | 발열 SEVERE 측정 보류 | **구현됨** |
| 4.4 | TTS 재생시간 1,000 ms | **구현됨** |
| 4.6 | 승인 기기 게이트 | 미착수 — 절 6의 순서가 선행 |
| 5 | 추론 지연 상한 | `[NOT_APPROVED]` — 값 미정 |

조인 기준으로 SM-A716S가 `FULL`을 다시 받았다(2026-09-08). 자동 시험은 2,046건 중 12건 실패이며
전부 기존 macOS 기준선(`AndroidReportQueueStoreTest`)이다.
...
### 5.3 조인 기준은 저장된 통과를 무효화해야 한다

값을 모두 조이고 설치했더니 SM-A716S가 **옛 기준으로 받은 `FULL`을 그대로 복원했다.** 결속
지문이 어제와 동일했기 때문이며, 새 기준 검사는 한 번도 돌지 않았다. 단위 시험은 전부 통과하고
있었으므로 실기기가 아니었으면 드러나지 않았다.

`PostLoginDeviceCheckResultBinding`이 `probePolicyVersion`을 지문에 포함하는 것이 정확히 이
목적이다. `20260905-interactive-v1` → `20260908-tightened-v1`로 올리자 저장 결과가 버려지고
재점검이 요구됐다.

**앞으로 판정 기준을 바꿀 때마다 이 값을 함께 올린다.** 올리지 않으면 이미 통과한 기기는 새
기준을 영영 만나지 않는다.
