...
fun buildConfigString(value: String): String = "\"" + value
    ...
    .replace("\n", "\\n")
    .replace("\r", "\\r") + "\""
val debugGatewayOrigin =
    verifiedReleaseGatewayOrigin ?: "http://127.0.0.1:8081"
...
tasks.configureEach {
    ...
    if (name == "mergeDebugAssets" || name == "mergeReleaseAssets" || name == "mergeVisualTestAssets") {
...
android {
    ...
    defaultConfig {
        ...
        buildConfigField("boolean", "VISUAL_TEST_ENABLED", "false")
        buildConfigField("String", "WALKSAFE_TMAP_MAP_KEY", "\"\"")
    ...
    buildTypes {
        debug {
            if (developmentReportQueue && !reportQueueEnabled) {
                ...
                buildConfigField(
                    "String",
                    "WALKSAFE_REPORT_QUEUE_TEST_ORIGIN",
                    buildConfigString(debugGatewayOrigin),
                )
        ...
        create("visualTest") {
            ...
            applicationIdSuffix = ".visualtest"
            versionNameSuffix = "-visual-test"
            isDebuggable = true
            signingConfig = signingConfigs.getByName("debug")
            buildConfigField("boolean", "VISUAL_TEST_ENABLED", "true")
            buildConfigField("String", "WALKSAFE_TMAP_MAP_KEY", buildConfigString(
                providers.environmentVariable("WALKSAFE_TMAP_MAP_KEY").orNull.orEmpty(),
            ))
            buildConfigField("String", "WALKSAFE_BUILD_MARKER", "\"walksafe-visual-test-v1\"")
        }
        release {
            buildConfigField("String", "WALKSAFE_SOURCE_COMMIT", "\"${verifiedSourceCommit ?: "unverified"}\"")
    ...
    sourceSets {
        getByName("debug").kotlin.srcDir("src/standard/java")
        getByName("release").kotlin.srcDir("src/standard/java")
        getByName("visualTest") {
...
androidComponents {
    beforeVariants(selector().withBuildType("visualTest")) { variant ->
        requireNotNull(variant.hostTests[com.android.build.api.variant.HostTestBuilder.UNIT_TEST_TYPE]).enable = true
    }
}
