...
class RouteNavigator(
    ...
    @Synchronized
    fun visualTestRouteSnapshot(): WalkingRoute? = route?.let { currentRoute ->
        currentRoute.copy(
            polyline = currentRoute.polyline.toList(),
            guidePoints = currentRoute.guidePoints.toList(),
            steps = currentRoute.steps.map { step -> step.copy(points = step.points.toList()) },
        )
    }
