...
class AndroidPhoneMountingSensorProbe(context: Context) : Closeable {
    ...
    fun latestNow(
        ...
        // RMS preserves sustained motion without letting one brief peak dominate the window.
        val rmsShake = sqrt(freshGyro.sumOf {
            it.magnitudeRadiansPerSecond * it.magnitudeRadiansPerSecond
        } / freshGyro.size)
        PhoneMountingSensorMeasurement(
            angularShakeDegreesPerSecond = Math.toDegrees(rmsShake),
