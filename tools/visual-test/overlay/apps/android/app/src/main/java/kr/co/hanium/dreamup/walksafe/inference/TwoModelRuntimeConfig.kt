...
data class TwoModelRuntimeConfig(
    ...
    companion object {
        ...
        fun load(context: Context): TwoModelRuntimeConfig {
            DetectorLoadDiagnostics.record("config_start")
            val json = try {
                context.assets.open(CONFIG_ASSET).bufferedReader().use { it.readText() }
            } catch (error: Exception) {
                DetectorLoadDiagnostics.record("config_failed", "stage=asset_open", error)
                throw error
            }
            val config = try {
                parse(json)
            } catch (error: Exception) {
                DetectorLoadDiagnostics.record("config_failed", "stage=parse", error)
                throw error
            }
            DetectorLoadDiagnostics.record(
                "config_parsed",
                "primary=${config.primaryModelKey} fallback=${config.fallbackModelKey ?: "none"}" +
                    " unified_declared_enabled=${config.unifiedWalksafe?.enabled}",
            )
            ...
            DetectorLoadDiagnostics.record("config_ready", "unified_effective_enabled=${unified?.enabled}")
        ...
        internal fun assetMatches(context: Context, model: ModelRuntimeConfig): Boolean {
            val expectedSha256 = model.artifactSha256 ?: run {
                DetectorLoadDiagnostics.record("asset_hash_missing", "model=${model.key}")
                return false
            }
            DetectorLoadDiagnostics.record("asset_hash_start", "model=${model.key}")
            return runCatching {
                ...
                (actualSha256 == expectedSha256).also { matches ->
                    DetectorLoadDiagnostics.record("asset_hash_result", "model=${model.key} match=$matches")
                }
            }.onFailure { error ->
                DetectorLoadDiagnostics.record("asset_hash_failed", "model=${model.key}", error)
            }.getOrDefault(false)
        }
