...
> **문서 상태(2026-06-02): 과거 실행 계획 snapshot.** 현재 기준은 `docs/status/current_status.md`, `apps/android/README.md`, `docs/README.md`를 우선한다. APK hash와 unified-primary 전환 이후의 모델/좌표 상태는 최신 문서를 따른다.
...
- [단계] saved reports 기반 threshold tradeoff 표를 만든다.
  -> 검증: `reports/runs/evaluations/stage1_yolo26s_reviewed_test_presence_error_candidates_20260531/`와 `reports/runs/evaluations/predictions_manifest_presence_20260531/` 출처와 한계가 표 아래에 적힌다.
...
- 공공기관 자동 제출/API 연동.
- TMAP/Cloud STT/TTS secret 설정 또는 비용 발생 live API 호출.
