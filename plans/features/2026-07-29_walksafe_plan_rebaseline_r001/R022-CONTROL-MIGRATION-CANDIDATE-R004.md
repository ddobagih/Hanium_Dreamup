...
이 문서는 실패한 R003을 수정하지 않고 add-only successor 설계만 추가한다.
candidate, authorization request, canonical r022, Goal/event와 제품 코드를 만들거나
변경하지 않는다. R004 자체의 독립검수 findings가 모두 0이 되기 전에는 어느
transaction도 구현·발행·승인·적용하지 않는다.

## 1. frozen 입력과 현재 build blocker

R004는 다음 두 물리 문서를 선행 입력으로 결속한다.
...
모두 write 0으로 막는다. R004 설계, R003, 기존 dirty bytes 또는 편의를 위한
...
- `TP`: add-only P candidate root
  `plans/features/2026-07-29_walksafe_plan_rebaseline_r001/v2-4-selector-preflight-r004`
- `TM`: add-only M candidate root
  `plans/features/2026-07-29_walksafe_plan_rebaseline_r001/v2-5-control-candidate-r004`
- `GP`:
...
`A0`와 `R0`는 R004, candidate, repository, gate root, environment, CLI path,
...
parent ACL hash/revision, repository/checkpoint, R004/candidate/review, transaction,
...
R004는 deterministic bytes와 opaque bytes를 같은 규칙으로 다루지 않는다.
...
| `D_NAME_SWITCHED` | target exact after, adopted temp inode와 동일, temp absent | target parent `fsync` |
| `D_DONE` | parent-fsync confirmation과 physical binding exact | 완료 |
...
13. 각 raw spool을 정확히 한 번 `COPY_NOREPLACE` D-PUBLISH로 같은 inode의 final
    raw name에 승격하고 parent fsync; exact `D_NAME_SWITCHED`면 fsync만 resume
...
B append는 R004 publication primitive의 선행 trust primitive다. preinstalled
...
ACTIVE checker는 candidate root, R001~R004 설계/review와 fixture를 열지 않고
...
exact pivot receipt와 quick까지만 있고 application-consumption marker가 없으면
source authority는 seq39다. unclosed quick intent/spool/result/PASS receipt 또는
`APPLICATION_CONSUMPTION_PROBE_STARTED`부터 selector가
...
|---|---|---|
| unknown entry/fork/inode/bytes 또는 incident+receipt | `PHYSICAL_DIVERGENCE_FAIL_CLOSED` | repository write 0 |
...
| P external bootstrap receipt 없음 + bootstrap claim/partial GP, 또는 P external bootstrap receipt exact + pivot receipt 없음 + pivot intent absent/AGENTS before, 또는 pivot receipt 없음 + pivot intent/temp/AGENTS exact-after, 또는 pivot receipt exact + unclosed quick intent/spool/result/PASS receipt | `SELECTOR_PREFLIGHT_BOOTSTRAP_RECOVERY_REQUIRED` | P same-tx recovery capability만 |
...
| M external bootstrap receipt 없음 + bootstrap claim/partial GM, incident 없음 | `M_BOOTSTRAP_RECOVERY_REQUIRED` | M same-tx recovery capability만 |
| M post-commit receipt 없음 + application marker/attestation/claim temp·spool/progress/final prefix index 1~14, incident 없음 | `V25_TRANSITION_RECOVERY_REQUIRED` | M same-tx recovery capability만 |
...
| AGENTS pivot exact + pivot receipt, P application marker/incident 없음 | `V24_SEQ39_PIVOT_READY` | 기존 v2.4만 |
| exact seq40 + P17 + P receipt, M application marker/incident 없음 | `V24_SELECTOR_READY` | 기존 v2.4, M 준비만 |
...
동결한다. P quick logical contract `2026-07-30.r004.1`은 두 check이며 digest는
...
non-prefix, entry/inode mismatch, journal fork, intent 없는 final, incident+receipt,
wrong receipt occupant가 있으면 `PHYSICAL_DIVERGENCE_FAIL_CLOSED`이고 repository/
gate에 새 incident 또는 receipt를 쓰지 않는다. B의 external audit sink에만
signed observation을 남길 수 있다. public UI가 recovery로 묶어 보여도 internal
reason을 normal terminal과 혼동하지 않는다.

## 14. FP-008와 Approval2 제거

R004는 R003 §6 Approval2 전체를 폐기한다.
...
현재 R004 상태는
`R003_REVIEW_FAILED_R004_DESIGN_ONLY_SOURCE_DRIFT_BLOCKED`다. 다음 허용 행동은
R004 exact bytes의 작성자 내부검토와 독립검수뿐이다. findings 0과 source drift
