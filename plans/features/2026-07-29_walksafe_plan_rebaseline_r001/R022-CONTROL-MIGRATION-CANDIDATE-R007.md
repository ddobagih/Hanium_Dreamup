...
이 문서는 실패한 R006을 수정하지 않고 add-only successor 설계만 추가한다.
candidate, authorization request, canonical r022, Goal/event와 제품 코드를 만들거나
변경하지 않는다. R007 자체의 독립검수 findings가 모두 0이 되기 전에는 어느
transaction도 구현·발행·승인·적용하지 않는다.

## 1. frozen 입력과 현재 build blocker

R007은 다음 여덟 물리 문서를 선행 입력으로 결속한다.
...
| 실패한 R006 설계 | `R022-CONTROL-MIGRATION-CANDIDATE-R006.md` | `06b6d7f16f54b1ce034aee7d89addd54796fcc4a5053fc5d436c8df3eb50238a` | 72,003 |
| R006 FAIL 검수 | `R022-CONTROL-MIGRATION-CANDIDATE-R006-independent-review-r001.md` | `f74f1c893f9bb9bcb3dbca2497c84051ec6e601b17aff28db4c254505243645f` | 5,240 |
...
R004/R005의 FAIL 판정은 historical input으로 유지한다. R006 판정
`FAIL_REQUIRES_ADD_ONLY_SUCCESSOR_R007`을 현재 선행 판정으로 유지한다. R007은
R006의 정상 부분을 보존하고 그 FAIL 검수 §3 allowlist의 두 root-cause 교정만
적용한다.
...
모두 write 0으로 막는다. R007/R006/R005/R004 설계, R003, 기존 dirty bytes 또는 편의를 위한
...
`A0`와 `R0`는 R007, candidate, repository, gate root, environment, CLI path,
...
parent ACL hash/revision, repository/checkpoint, R007/candidate/review, transaction,
...
R007은 deterministic bytes와 opaque bytes를 같은 규칙으로 다루지 않는다.
...
B append는 R007 publication primitive의 선행 trust primitive다. preinstalled
...
old lease가 recovery lease라면 새 observation은 그 lease를 발행한 exact
`RECOVERY_OBSERVATION_CONSUMED`와 parent observation ID/hash를 추가로 결속한다.
이 predecessor edge는 original observation부터 latest observation까지 단일
lineage를 만들며 fork, cycle, skipped parent와 다른 transaction/lease edge는
physical divergence다.
...
recovery attempt/nonce, 현재 observation 전의 consumed-but-unclosed ancestor
observation ID/hash/consume-record hash를 oldest→newest로 정렬한 exact
`open_ancestor_lineage`와 그 count/digest를 결속한
...
복구 또는 terminal closure가 끝나면 validator가 current B head에서 아직 open인
consumed ancestor 전체와 current observation을 다시 도출한다. 단일
`RECOVERY_OBSERVATION_CLOSED`는 exact `transitively_closes` list를 oldest→newest
순서로 결속하고, 각 observation ID/hash/lease ID와 consume-record physical
hash/bytes, list count/digest, final result phase/receipt 및
`close_mode=TRANSITIVE_ALL_OPEN_ANCESTORS`를 고정한다. B expected-head CAS와
durable append receipt가 이 list 전체를 원자적으로 closed로 투영한다. 누락,
중복, 순서 변경, already-closed/nonancestor/다른 lineage member 또는 derived open
set과의 불일치는 write 0이다. exact close-attempt refetch만 byte-exact 허용한다.

recovery lease 자체가 close 전에 끝나면 새 lease ID의 후속 observation이 latest
lineage record가 되고 위 open ancestor list가 다음 consume/close로 전이된다.
따라서 마지막 successful close 하나가 O1→…→On의 모든 consumed open ancestor를
닫으며, consumed/closed/stale observation은 다시 매치되지 않는다.
...
closure 뒤 §6.1의 complete open-ancestor `transitively_closes` list를 결속한
`RECOVERY_OBSERVATION_CLOSED`가 durable하지 않으면 public success가 아니다. current
observation만 닫고 consumed ancestor를 남기거나 같은 lineage observation을 새
capability에 재사용하지 않는다. capability, predicate, current B head, complete
lineage close 또는 external durability가 불완전하면 write 0이다.
...
ACTIVE checker는 candidate root나 R001~R007 설계/review/fixture를 열지 않는다.
...
application marker와 §11 `POST_MARKER_DESCENDANT_SET`이 모두 absent일 때만
...
absent인 zero-quick과 partial/unclosed intent/spool/result를 모두
application marker와 §11 `POST_MARKER_DESCENDANT_SET`이 모두 absent일 때만
`M_BOOTSTRAP_RECOVERY_REQUIRED`로 분류한다.
...
`POST_MARKER_DESCENDANT_SET`은 application-consumption ingress의 intent/spool/raw/
descriptor, signed non-success ingress, verified consumed attestation, runtime/tool
O-CAPTURE intent/spool/raw/descriptor, application claim intent/temp/spool/claim,
transaction root/progress/resolved, promotion/final/checkpoint, post-check와
application terminal incident/receipt 전체다. 각 member는 같은 transaction의
다음 branch에서 자기 앞의 모든 durable predecessor physical binding을 직접
결속해야 한다.

```text
quick PASS
→ APPLICATION_CONSUMPTION_PROBE_STARTED
→ signed consumption ingress O-CAPTURE D_DONE
→ verified consumed attestation
→ runtime/tool O-CAPTURE D_DONE
→ APPLICATION_CLAIM_INTENT
→ application claim D_DONE
→ progress/resolved/promotions/post-check/terminal

또는

quick PASS
→ APPLICATION_CONSUMPTION_PROBE_STARTED
→ signed non-success ingress O-CAPTURE D_DONE
→ exact narrow terminal incident
```

marker absent인데 descendant가 하나라도 있거나, 어느 branch든 immediate/earlier
predecessor가 absent·nonexact인데 successor가 있으면
`PHYSICAL_DIVERGENCE_FAIL_CLOSED`다. marker exact 뒤 descendant zero 또는 위
순서의 exact prefix만 application recovery가 될 수 있다.
...
observation lineage의 latest record이고, 그 `transitively_closes`가 B projection이
도출한 O1→…→On open ancestor exact set/order 전체를 덮어 resulting open set이
empty여야 한다. latest On만 닫거나 ancestor 하나라도 남기면 stable/terminal이
아니다. 그 전에는
...
|---|---|---|
| unknown entry/fork/inode/bytes, consumed/closed/stale recovery observation의 current 재사용, same transaction에서 quick PASS receipt absent + application marker present, application marker absent + `POST_MARKER_DESCENDANT_SET` nonempty, post-marker predecessor absent/nonexact + successor present, 또는 same transaction·same terminal phase의 normal incident + competing post-commit success receipt | `PHYSICAL_DIVERGENCE_FAIL_CLOSED` | repository write 0 |
...
| P application marker absent + `POST_MARKER_DESCENDANT_SET` empty + (external bootstrap receipt 없음 + bootstrap claim/partial GP, 또는 external bootstrap receipt exact + pivot receipt 없음 + pivot intent absent/AGENTS before, 또는 pivot receipt 없음 + pivot intent/temp/AGENTS exact-after, 또는 pivot receipt exact + quick PASS receipt 없음 + zero/partial/unclosed quick, 또는 pivot receipt와 quick PASS receipt exact) | `SELECTOR_PREFLIGHT_BOOTSTRAP_RECOVERY_REQUIRED` | latest-observation-bound P same-tx recovery capability만 |
| P quick PASS receipt exact + application marker exact + P post-commit receipt 없음 + post-marker descendant zero 또는 exact ordered prefix + incident 없음 | `SELECTOR_PREFLIGHT_RECOVERY_REQUIRED` | latest-observation-bound P same-tx recovery capability만 |
| M application marker absent + `POST_MARKER_DESCENDANT_SET` empty + (external bootstrap receipt 없음 + bootstrap claim/partial GM, 또는 external bootstrap receipt exact + quick PASS receipt 없음 + zero/partial/unclosed quick, 또는 external bootstrap receipt와 quick PASS receipt exact), incident 없음 | `M_BOOTSTRAP_RECOVERY_REQUIRED` | latest-observation-bound M same-tx recovery capability만 |
| M quick PASS receipt exact + application marker exact + M post-commit receipt 없음 + target checkpoint absent + post-marker descendant zero 또는 exact ordered prefix through final index 1~14 + incident 없음 | `V25_TRANSITION_RECOVERY_REQUIRED` | latest-observation-bound M same-tx recovery capability만 |
...
동결한다. R004에서 동결되어 R005/R006/R007이 byte-exact 상속한 P quick logical contract
...
`RECOVERY_OBSERVATION_CLOSED`가 B latest head에 durable하고, 그
`transitively_closes`가 B projection이 도출한 O1→…→On consumed open ancestor
exact set/order 전체를 덮어 resulting open set이 empty여야 해당 lineage
lifecycle이 닫힌다. latest observation만 닫거나 ancestor 하나라도 남기는 close는
normal terminal/receipt를 public terminal로 만들지 않는다.

## 14. FP-008와 Approval2 제거

R007은 R006이 보존한 R003 §6 Approval2 폐기를 되돌리지 않는다.
...
- O1 consume 뒤 recovery lease crash, O2 observation/consume 뒤 successful close
  fixture에서 `transitively_closes=[O1,O2]`의 exact oldest→newest full list와
  derived count/digest가 일치할 때만 PASS; O2-only, O1 누락, member 중복,
  O2→O1 순서 변경, cross-transaction/cross-lineage member, consume-record
  hash/bytes 불일치와 open set 잔존은 모두 close write 0
- 같은 O1→O2 fixture에서 close append 전 crash는 repository terminal이 아니며,
  current B head에서 exact close-attempt refetch 또는 새 observation O3로 lineage를
  연장한 뒤 `[O1,O2,O3]` full transitive close만 PASS
...
- P와 M 각각 quick PASS→marker→ingress→attestation→runtime→claim→progress
  application branch를 독립 생성해, marker absent+ingress present,
  ingress absent/nonexact+attestation present, attestation absent/nonexact+runtime
  present, runtime absent/nonexact+claim present, claim absent/nonexact+progress
  present는 모두 predecessor physical divergence; immediate predecessor뿐 아니라
  각 successor 앞의 earlier predecessor 하나라도 제거·변조한 모든 prefix-gap
  fixture도 repository write 0
- P/M bootstrap recovery는 marker absent와 `POST_MARKER_DESCENDANT_SET` empty의
  conjunction일 때만 PASS하고, marker absent+각 descendant 단독 주입은 전부
  divergence; application recovery는 quick PASS와 marker가 exact하고 descendant가
  zero 또는 branch의 exact ordered prefix일 때만 PASS
...
현재 R007 상태는
`R006_REVIEW_FAILED_R007_DESIGN_ONLY_SOURCE_DRIFT_BLOCKED`다. 다음 허용 행동은
R007 exact bytes의 작성자 내부검토와 독립검수뿐이다. findings 0과 source drift
