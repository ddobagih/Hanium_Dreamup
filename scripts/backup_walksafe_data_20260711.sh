...
verify_upload_binding() {
  ...
  [[ "$(stat -Lc '%u' "${UPLOAD_ANCHOR}")" == "$(id -u)" ]] || return 1
  mode="$(stat -Lc '%a' "${UPLOAD_ANCHOR}")"
  (( (8#${mode} & 077) == 0 && (8#${mode} & 0500) == 0500 )) || return 1
  [[ "$(validate_private_path "${UPLOAD_DIR}/walksafe-upload-anchor")" == "${UPLOAD_DIR}/walksafe-upload-anchor" ]]
}
verify_upload_binding || { echo "upload directory changed or is not a private current-user directory" >&2; exit 2; }
...
[[ "$(stat -Lc '%d:%i' "${LOCK_PARENT}")" == "${LOCK_PARENT_DEVICE_INODE}" \
  && "$(stat -Lc '%u:%a' "${LOCK_PARENT_ANCHOR}")" == "$(id -u):700" ]] || {
  echo "maintenance lock parent changed or is not a private current-user 0700 directory" >&2
  exit 2
}
"${BACKUP_RUNTIME_PYTHON}" -I -S -B - "${LOCK_PARENT_FD}" "${LOCK_NAME}" "${LOCK_PARENT}" <<'PY'
import os
...
try:
    ...
    if (
        ...
        raise SystemExit("maintenance lock must be a single-link current-user-owned 0600 file")
    os.fsync(descriptor)
finally:
    os.close(descriptor)
os.fsync(parent_fd)
...
LOCK_AUTHORITY_DEVICE_INODE="${LOCK_PARENT_DEVICE_INODE}"
LOCK_PARENT_STABLE_STATE="$(stat -Lc '%d:%i:%y:%z' "${LOCK_PARENT_ANCHOR}")"
verify_lock_binding() {
  [[ -d "${LOCK_PARENT}" && ! -L "${LOCK_PARENT}" ]] || return 1
  [[ "$(stat -Lc '%d:%i' "${LOCK_PARENT}")" == "${LOCK_PARENT_DEVICE_INODE}" ]] || return 1
  [[ "$(stat -Lc '%d:%i:%y:%z' "${LOCK_PARENT}")" == "${LOCK_PARENT_STABLE_STATE}" ]] || return 1
  [[ "$(stat -Lc '%d:%i:%y:%z' "${LOCK_PARENT_ANCHOR}")" == "${LOCK_PARENT_STABLE_STATE}" ]] || return 1
...
if ! flock --exclusive --timeout "${LOCK_TIMEOUT_SECONDS}" "${LOCK_PARENT_FD}"; then
...
env -i PATH=/usr/bin:/bin DATABASE_URL="${PG_DUMP_DATABASE_URL}" \
  "${BACKEND_PYTHON}" -I -B "${SCRIPT_DIR}/check_walksafe_backup_source_20260713.py" \
    --upload-dir-fd "${UPLOAD_FD}" --archive-output --result-fd "${SOURCE_RESULT_FD}" | \
  gpg --no-options --batch --yes --trust-model always --encrypt \
