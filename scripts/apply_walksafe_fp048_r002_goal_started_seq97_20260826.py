#!/usr/bin/env python3
"""Prepare or append FP-048 R002 GOAL_STARTED seq97 after its R008 PASS gate."""
...
seq90 = correction.seq90
contract = correction.continuation
...
TARGET_GOAL_SHA256 = correction.GOAL_SHA256
...
STARTED_SCOPE = (
    "Graph v2.4 through FP048-R002/GAP-057 GOAL_STARTED seq97 after exact "
    "seq96 zero-credit gate-contract correction and fresh R008 five-check internal "
...
class StartApplyError(RuntimeError):
    """The exact seq96 correction and fresh R008 PASS gate do not authorize seq97."""
...
def require_exact_source(
    ...
    *,
    ...
    try:
        ...
        authority = gate.bind_published_contract_corrected_source(root, source)
    except Exception as exc:
        raise StartApplyError(f"source seq96 correction authority differs: {exc}") from exc
    ...
    _require(
        ...
        and correction.CORRECTION_SEQUENCE == SOURCE_SEQUENCE
        and correction.CORRECTION_EVENT_ID == SOURCE_EVENT_ID
        and correction.STARTED_SEQUENCE == EVENT_SEQUENCE
        and correction.STARTED_EVENT_ID == STARTED_EVENT_ID
        and gate.SOURCE_SEQUENCE == SOURCE_SEQUENCE
        and gate.EVENT_SEQUENCE == EVENT_SEQUENCE
        and gate.STARTED_EVENT_ID == STARTED_EVENT_ID,
        "source is not exact seq96",
    )
    previous_sha256 = ""
    ...
    source_binding = control.get("source_checkpoint_binding")
    r007_preflight = (
        source_binding.get("r007_preflight_attempt_004")
        if isinstance(source_binding, dict)
        else None
    )
    expected_contract_binding = correction.r008_contract_binding(root)
    expected_r007_preflight = correction.R007_PREFLIGHT_ATTEMPT_004
    _require(
    ...
    _require(
        control.get("sequence") == correction.CORRECTION_SEQUENCE
        and control.get("event_id") == correction.CORRECTION_EVENT_ID
        and control.get("event_id") == SOURCE_EVENT_ID
        and control.get("event_type") == "GOAL_START_GATE_CONTRACT_CORRECTED"
        ...
        and control.get("event_sha256") == authority.correction_event_sha256
        and transport_reanchor.strict_json_equal(
            replacement, authority.contract_binding
        ),
        "source seq96 correction differs",
    )
    _require(
        ...
        "source seq96 R008 contract binding differs",
    )
    _require(
        ...
        and isinstance(r007_preflight, dict)
        and set(r007_preflight)
        == {
            "event_id",
            "contract_id",
            "contract_version",
        ...
        and transport_reanchor.strict_json_equal(
            r007_preflight,
            expected_r007_preflight,
        )
        and r007_preflight.get("event_id") == STARTED_EVENT_ID
        and r007_preflight.get("contract_id")
        == correction.R007_CONTRACT_BINDING["contract_id"]
        and r007_preflight.get("contract_version")
        == correction.R007_CONTRACT_BINDING["contract_version"]
        and r007_preflight.get("status") == "PREVIEW_FAILED_BEFORE_NAMESPACE"
        and r007_preflight.get("authority_status") == "NONAUTHORITY"
        and r007_preflight.get("event_identity_status")
        == "REUSABLE_UNCONSUMED"
        and r007_preflight.get("namespace_present") is False
        and r007_preflight.get("receipt_present") is False
        and r007_preflight.get("failed_check_id")
        == "ROOT_FP048_R002_CONTROL_REGRESSION"
        and type(r007_preflight.get("exit_code")) is int
        and r007_preflight.get("exit_code") == 1
        and r007_preflight.get("error")
        == (
            "ROOT_FP048_R002_CONTROL_REGRESSION failed with exit code 1; "
            "see PREVIEW_ONLY"
        )
        and r007_preflight.get("reason_code")
        == "R007_ROOT_REGRESSION_INCLUDED_PREPUBLICATION_ONLY_SEQ95_TESTS",
        "source seq96 R007 preflight NONAUTHORITY binding differs",
    )
    statuses = state.get("status_by_goal")
    _require(
        state.get("transition_history_anchor_sha256")
        == authority.correction_event_sha256
...
def _source_activation(source: dict[str, Any], digest: str) -> dict[str, Any]:
    ...
    matches = [
        ...
        and event.get("sequence") == correction.CORRECTION_SEQUENCE
        and event.get("event_id") == correction.CORRECTION_EVENT_ID
        and event.get("event_id") == SOURCE_EVENT_ID
        and event.get("event_type") == "GOAL_START_GATE_CONTRACT_CORRECTED"
        and event.get("event_sha256") == digest
    ]
    _require(len(matches) == 1, "source seq96 trust anchor differs")
    return matches[0]
...
def _assert_zero_credit_projection(
    ...
    for key in (
        ...
        _require(
            ...
            f"seq97 changed zero-credit state: {key}",
        )
...
def project_seq97(
    ...
    *,
    ...
    _require(
        event_id == EVENT_ID and gate.STARTED_EVENT_ID == EVENT_ID,
        "seq97 event ID differs",
    )
    ...
    _require(set(event) == EVENT_FIELDS, "seq97 GOAL_STARTED field set differs")
    ...
    if final_sha256_by_path is None:
        ...
        _require(
            ...
            "seq97 final managed path inventory differs",
        )
    ...
    _require(
        ...
        "seq1-96 history prefix changed",
    )
    for field in (
        "focus_goal_id",
        "focus_goal_path",
    ...
    _require_exact_seq97_projection(
        root,
        checkpoint,
        state,
        event,
...
def _require_exact_seq97_projection(
    root: Path,
    ...
    *,
    ...
    _require(
        ...
        "seq97 history position differs",
    )
    ...
    _require(
        ...
        "seq97 gate binding differs",
    )
    occurred_at = _parse_timestamp(
        event.get("occurred_at"), label="seq97 occurred_at", second_precision=True
    )
    _require(
    ...
    _require(
        ...
        and current.get("release_completion_claimed") is False,
        "seq97 current-work projection differs",
    )
    ...
    _require(
        ...
        "seq97 working-tree snapshot projection differs",
    )
    ...
    if require_live_snapshot:
        ...
        _require(
            ...
            "seq97 working-tree snapshot hashes differ",
        )
    ...
    _require(
        ...
        "seq97 session handoff projection differs",
    )
...
def _gate_receipt_authority(
    ...
    _require(
        ...
        "checkpoint is not exact seq97",
    )
    event = history[EVENT_SEQUENCE - 1]
    _require(isinstance(event, dict), "seq97 event differs")
    binding = event.get("implementation_start_gate_binding")
    ...
    _require(
        ...
        and isinstance(binding.get("file_sha256"), str)
        and gate._impl.SHA256_RE.fullmatch(binding["file_sha256"]) is not None,
        "seq97 gate receipt binding differs",
    )
    ...
    _require(
        ...
        "seq97 gate receipt mode or size differs",
    )
    receipt = _strict_json(receipt_read.raw, "seq97 PASS receipt")
    _require(
        receipt_read.raw == json_bytes(receipt),
        "seq97 PASS receipt bytes are not canonical",
    )
    _require(
        sha256_bytes(receipt_read.raw) == binding["file_sha256"],
        "seq97 gate receipt file binding differs",
    )
...
def goal_start_gate_receipt_binding(
    ...
    """Return a detached binding only after exact R008 receipt verification."""
...
def _require_exact_r008_receipt(
    root: Path,
    source: dict[str, Any],
    source_raw: bytes,
    event: Mapping[str, Any],
    ...
    authority = gate.bind_published_contract_corrected_source(root, source)
    contract_binding = _correction().r008_contract_binding(root)
    _require(
        set(receipt) == gate._impl.RECEIPT_FIELDS,
        "seq97 R008 receipt field set differs",
    )
    expected_scalars = {
        "schema_version": "1.1",
    ...
    for field, expected in expected_scalars.items():
        _require(
            ...
            f"seq97 R008 receipt {field} differs",
        )
    _require(
        transport_reanchor.strict_json_equal(
    ...
    _require(
        ...
        "seq97 R008 receipt check count differs",
    )
    ...
    for index, (check_id, run) in enumerate(
        ...
        _require(isinstance(run, dict), f"seq97 R008 check {index} differs")
        ...
        _require(
            ...
            f"seq97 R008 check {index} evidence mode or size differs",
        )
        executed_at = _parse_timestamp(
            run.get("executed_at"),
            label=f"R008 check {index} executed_at",
        ...
        _require(
            ...
            f"seq97 R008 check {index} authority differs",
        )
        ...
        if check_id == "REPOSITORY_STATE":
            repository_payload = _strict_json(
                output_read.raw,
                "seq97 sealed REPOSITORY_STATE log",
            )
            _require(
                output_read.raw
                == contract.canonical_json_bytes(repository_payload) + b"\n",
                "seq97 sealed REPOSITORY_STATE log is not canonical",
            )
    ...
    _require(
        ...
        "seq97 R008 evidence namespace differs",
    )
    _require(
        ...
        "seq97 R008 repository snapshot differs",
    )
    expected_receipt = {
        **expected_scalars,
    ...
    _require(
        ...
        "seq97 R008 receipt bytes differ from exact producer order",
    )
...
def _restored_seq96_checkpoint(checkpoint: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(isinstance(source_event, dict), "seq96 inverse source event differs")
...
def reconstructed_seq96_checkpoint_bytes(
    ...
    """Reconstruct and validate the exact seq96 source bound by the R008 receipt."""
    ...
    canonical = correction.canonical_seq96_checkpoint_bytes(root, restored)
    reconstructed = correction.reconstructed_seq96_checkpoint_bytes(root, restored)
    _require(
        canonical == reconstructed,
        "restored seq96 canonical authority differs",
    )
    _require(
        receipt.get("source_checkpoint_sha256") == sha256_bytes(canonical),
        "seq97 gate receipt source checkpoint SHA-256 differs",
    )
    source = _strict_json(canonical, "restored seq96 checkpoint")
    _require_exact_r008_receipt(
        root,
        source,
        canonical,
        event,
...
def require_exact_seq97_projection(
    root: Path,
    checkpoint: Mapping[str, Any],
    *,
    require_live_snapshot: bool = True,
) -> None:
    """Require the exact single READY-to-IN_PROGRESS seq97 projection."""
    ...
    source_raw = reconstructed_seq96_checkpoint_bytes(root, checkpoint)
    source = _strict_json(source_raw, "reconstructed seq96 checkpoint")
    _correction().require_contract_corrected_checkpoint(
        root,
        source,
        run_external_validators=False,
        # seq96 is historical once the exact R008 namespace exists.  The
        # seq97 projection itself, above, owns current repository authority.
        require_live_snapshot=False,
    )
    authority = gate.bind_published_contract_corrected_source(root, source)
    _require(
        ...
        "seq97 R008 receipt authority differs",
    )
...
def require_started_checkpoint(
    ...
    *,
    ...
    """Public exact seq97 authority used by continuation and GoalGraph consumers."""
    ...
    if run_external_validators:
        errors = validate_projected_checkpoint(root, checkpoint)
        _require(
            not errors,
            "seq97 external consumer validation failed: " + "; ".join(errors),
        )
...
def validate_history_suffix(root: Path, checkpoint: Mapping[str, Any]) -> list[str]:
    try:
        state = checkpoint.get("goal_execution")
        history = state.get("transition_history") if isinstance(state, dict) else None
        _require(isinstance(history, list) and len(history) >= EVENT_SEQUENCE, "seq97 is missing")
        control = history[EVENT_SEQUENCE - 2]
        event = history[EVENT_SEQUENCE - 1]
        _require(isinstance(control, dict) and isinstance(event, dict), "seq97 history differs")
        binding = event.get("implementation_start_gate_binding")
        _require(
            isinstance(binding, dict)
            and set(binding) == {"document_id", "path", "file_sha256"}
        ...
        receipt = _strict_json(receipt_bytes, "seq97 PASS receipt")
        _require_exact_seq97_projection(
            root,
            checkpoint,
            state,
        ...
        _require(
            ...
            "seq97 repository snapshot binding differs",
        )
        reconstructed_source = reconstructed_seq96_checkpoint_bytes(
            root, checkpoint
        )
        _require(
...
def validate_projected_checkpoint(
    ...
    *,
    ...
    errors = [f"seq97: {error}" for error in validate_history_suffix(root, checkpoint)]
    descriptor, temporary_name = tempfile.mkstemp(
        dir=root / CHECKPOINT_RELATIVE.parent,
        prefix=".walksafe-fp048-r002-seq97-preflight.",
        suffix=".json",
    )
...
def validate_gate_evidence(
    ...
    *,
    ...
    activation_at = _parse_timestamp(
        ...
        label="seq96 correction occurred_at",
        second_precision=True,
    )
    _require(activation_at <= started_at, "gate precedes seq96 correction")
    runs = receipt.get("check_runs")
    _require(
...
def _require_context(
    ...
    _require(
        ...
        == RUNTIME_TRUST_CLOSURE_PATHS,
        "FP048-R002 R008 gate context differs",
    )


def require_published_r008_gate_for_seq96(
    root: Path,
    source: dict[str, Any],
) -> dict[str, str] | None:
    """Return the exact R008 receipt binding, or ``None`` before publication."""

    root = seq90._safe_root(root)
    _require(type(source) is dict, "seq96 source checkpoint differs")
    source_raw = json_bytes(source)
    require_exact_source(root, source, require_live_snapshot=False)
    source_authority = _correction()
    _require(
        source_raw
        == source_authority.canonical_seq96_checkpoint_bytes(root, source),
        "source checkpoint is not the exact canonical seq96 correction",
    )
    checkpoint_read = seq90._stable_read(root, CHECKPOINT_RELATIVE)
    _require(
        checkpoint_read.raw == source_raw
        and stat.S_ISREG(checkpoint_read.identity.mode)
        and not stat.S_ISLNK(checkpoint_read.identity.mode)
        and stat.S_IMODE(checkpoint_read.identity.mode) == 0o600,
        "live seq96 checkpoint bytes or mode differ",
    )

    event_relative = gate.GATE_ROOT_RELATIVE / EVENT_ID
    receipt_relative = event_relative / gate.RECEIPT_NAME
    event_present = os.path.lexists(root / event_relative)
    receipt_present = os.path.lexists(root / receipt_relative)
    if not event_present and not receipt_present:
        return None

    context = _load_context(root, gate.load_gate_context)
    _require_context(root, source, source_raw, context)
    ...
    evidence = validate_gate_evidence(
        ...
        context,
        event_id=EVENT_ID,
        capture_repository_state=_sealed_gate_repository_payload,
        retained_inputs=retained,
    )
    terminal_checkpoint = seq90._stable_read(root, CHECKPOINT_RELATIVE)
    terminal_inputs = _capture_gate_inputs(root, context, EVENT_ID)
    _require(
        terminal_checkpoint.identity == checkpoint_read.identity
        and terminal_checkpoint.raw == checkpoint_read.raw
        and set(terminal_inputs) == set(retained)
        and all(
            observed.identity == retained[path].identity
            and observed.raw == retained[path].raw
            for path, observed in terminal_inputs.items()
        ),
        "published R008 source or gate evidence changed during validation",
    )
    return copy.deepcopy(evidence.receipt_binding)
...
def _require_prepared_exact(
    ...
    *,
    ...
    _require(
        ...
        "R008 gate evidence changed or namespace membership differs",
    )
    ...
    seq90._require_preflight_cohort_unchanged(
        ...
        phase="during seq97 publication preflight",
    )
    refreshed = gate.load_gate_context(
        prepared.root,
        require_live_snapshot=False,
    )
    _require(refreshed == prepared.context, "R008 gate context changed")
    ...
    _require(not errors, "projected seq97 validation failed: " + "; ".join(errors))
    ...
    seq90._require_preflight_cohort_unchanged(
        ...
        git_head=transport.git_head,
        git_branch=transport.git_branch,
        phase="after seq97 projected consumer validation",
    )
    observed_gate_inputs = _capture_gate_inputs(
        prepared.root,
        prepared.context,
    ...
    _require(
        ...
        "R008 gate evidence changed after projected consumer validation",
    )
...
def _fresh_exact_projected_checkpoint(
    ...
    _require(observed_raw == authority_raw, "seq97 projected checkpoint raw bytes differ")
    projected = _strict_json(observed_raw, "seq97 projected checkpoint")
    _require(
        json_bytes(projected) == observed_raw
        and json_bytes(prepared.projected_checkpoint) == authority_raw,
        "seq97 projected checkpoint authority differs",
    )
    ...
    _require(
        ...
        "seq97 projected event authority differs",
    )
    return projected
...
def _require_private_checkpoint_authority(
    ...
    *,
    ...
    _require(
        ...
        f"seq97 checkpoint authority differs {phase}",
    )
...
def _write_projection_transport(
    ...
    *,
    ...
    _require(
        ...
        "seq97 checkpoint source differs before injected writer",
    )
    guard_called = False
    ...
    try:
        ...
        if not guard_called:
            raise seq90.PostcommitUncertain(
                "seq97 checkpoint writer returned without its commit guard"
            )
        ...
        if unchanged:
            ...
            raise StartApplyError(
                "seq97 checkpoint transport failed before replacement"
            ) from exc
        raise seq90.PostcommitUncertain(
            "seq97 checkpoint transport ended with an unclassified failure"
        ) from exc
...
def _verify_published_projection(prepared: PreparedProjection) -> None:
    try:
        ...
        _require(
            ...
            "published seq97 gate evidence changed or membership differs",
        )
        errors = validate_projected_checkpoint(
            prepared.root,
            projected_for_validation,
        )
        _require(not errors, "published seq97 validation failed: " + "; ".join(errors))
        ...
        _require(
            ...
            "published seq97 checkpoint changed during terminal consumers",
        )
        ...
        _require(
            ...
            "published seq97 gate evidence changed during terminal consumers",
        )
        ...
        raise seq90.PostcommitUncertain(
            "seq97 checkpoint was replaced but terminal validation failed"
        ) from exc
...
def prepare_projection(
    ...
    *,
    ...
    _require_private_checkpoint_authority(source_read, phase="before seq97 preflight")
    ...
    _require(git_head == expected_head, "live Git HEAD differs from seq96 source")
    ...
    _require(
        source_read.raw
        == correction.canonical_seq96_checkpoint_bytes(root, source),
        "source checkpoint is not the exact canonical seq96 correction",
    )
    live_errors = live_validator(root)
    _require(not live_errors, "live seq96 validation failed: " + "; ".join(live_errors))
    ...
    _require(refreshed == context, "R008 gate context changed during checker")
    return prepared
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.write:
            ...
            published = True
        message = (
            "FP048-R002 GOAL_STARTED seq97: PASS "
        ...
        try:
            ...
            if published:
                raise seq90.PostcommitUncertain(
                    "published seq97 PASS output delivery is uncertain"
                ) from exc
            raise
    except seq90.PostcommitUncertain as exc:
        print(f"FP048-R002 GOAL_STARTED seq97: POSTCOMMIT-UNCERTAIN: {exc}", file=sys.stderr)
        return 2
    except (
        StartApplyError,
        gate.GateError,
        ...
        print(f"FP048-R002 GOAL_STARTED seq97: FAIL: {exc}", file=sys.stderr)
        return 1
    return 0
