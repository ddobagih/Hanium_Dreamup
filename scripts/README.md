...
| `walksafe_admin_high_risk_gate.py` | 배포환경의 출시승인·권한변경·자료삭제가 singleton DB 관리자 상태 `NORMAL`, off-phone custody `ATTESTED`, 기기 결속 세션, 최근 TOTP 재확인을 모두 통과할 때만 시작되게 하는 공통 fail-closed 경계. `/admin/operations/release-approvals`, `/admin/operations/privilege-changes`, `/admin/operations/data-deletions`는 정책 결속 식별자이며 현재 공개 HTTP route가 아니다. 단일사용 재확인 nonce와 감사를 caller mutation 전에 commit하고, 이후 mutation transaction 전체에서 복구와 공유하는 advisory fence를 유지하며 상태·세션을 재검증한다. caller가 실패해도 nonce는 attempt-based로 다시 쓸 수 없고 연결 상실은 DB transaction rollback·fence 해제로 fail-closed 처리된다. 세션값은 실행 프로세스에만 주입하고 파일·로그에 저장하지 않음 |
| `provision_walksafe_admin_device_key.py` | 신뢰할 수 있는 로컬 DB 절차에서 관리자 Android 기기의 P-256 공개 SPKI DER 또는 canonical unpadded Base64url만 등록·회전한다. 필수 descriptor SHA-256 marker를 DB 연결 전에 교차검증하고, 개인키·네트워크 등록은 받지 않으며 동일 버전 재실행은 같은 공개키일 때만 멱등 처리함 |
...
| `build_walksafe_historical_git_witness_20260812.py`, `apply_walksafe_npc_goal_start_control_reanchor_seq58_20260812.py`, `run_walksafe_npc_single_admin_recovery_goal_start_gate_20260812.py`, `apply_walksafe_npc_goal_start_control_correction_seq59_20260812.py`, `apply_walksafe_npc_single_admin_recovery_goal_started_seq60_20260812.py` | NPC start-control 5개 도구. 두 private bundle의 commit/path/blob witness, zero-credit reanchor, exact R002 8-check gate, add-only correction과 현행 seq60 `GOAL_STARTED`를 분리한다. start receipt는 구현 완료·정식 시험·출시 증거가 아님 |
| `apply_walksafe_npc_single_admin_recovery_goal_started_seq59_20260812.py` | correction 전 만들어졌으나 start gate를 통과하지 못해 publication 전에 대체된 실패/superseded 역사 도구. 현재 start 또는 completion 전이에 사용하지 않음 |
| `run_walksafe_npc_single_admin_recovery_verification_20260813.py`, `build_walksafe_npc_single_admin_recovery_trace_20260812.py`, `build_walksafe_npc_single_admin_recovery_gap_backlog_r026_20260812.py`, `build_walksafe_npc_single_admin_recovery_gap_backlog_r027_20260813.py`, `build_walksafe_npc_single_admin_recovery_artifact_trace_successor_20260812.py`, `build_walksafe_phase1_exact257_successor_r015_20260812.py`, `build_walksafe_npc_single_admin_recovery_strict_review_gate_20260812.py`, `apply_walksafe_npc_single_admin_recovery_goal_completed_seq61_62_20260812.py` | NPC completion·보정 도구. R027은 R026 네 파일을 exact-byte add-only 선행자로 보존하고 거부된 R001 v1 검토와 v2 구현·검증을 묶어 현재 내부 증거만 정정한다. v2 결과는 관측 manifest·lane 로그/receipt·source manifest·start authority에서 trace builder로 깊게 재생한 bytes와 같아야 한다. GAP-008 `PARTIAL`, 실제 기기·복구훈련·정식·외부·배포·출시 `NOT_RUN`, release `NOT_ELIGIBLE`, credit 0은 유지한다. 나머지 도구는 4-lane 제품 검증, trace, exact-six/exact257, 별도 내부검수·completion receipt, seq61/62 전이를 담당함 |
...
| `bind_walksafe_admin_credential_issuer_key.py` | migration 후 기존 singleton 관리자 control의 미결속 issuer 지문을 owner용 migration DB 역할로 최초 1회 결속한다. 같은 키는 멱등이고 다른 키·runtime 역할은 거부하며 비밀값·파일 경로·DB URL을 출력하지 않음 |
