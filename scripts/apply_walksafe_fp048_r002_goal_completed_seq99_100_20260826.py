#!/usr/bin/env python3
"""Project FP-048 R002 internal evidence seq99 and completion seq100."""
...
from scripts import apply_walksafe_fp048_r002_goal_completed_seq98_99_20260826 as frozen_completion
from scripts import apply_walksafe_fp048_r002_goal_started_seq98_20260826 as started
...
COMPLETION_EVENT_ID = (
    "WS-GOAL-GRAPH-V2-4-GOAL-COMPLETED-FP048-R002-20260826-002"
)
...
SCRIPT_REL = Path("scripts/apply_walksafe_fp048_r002_goal_completed_seq99_100_20260826.py")
TEST_REL = Path("tests/test_apply_walksafe_fp048_r002_goal_completed_seq99_100_20260826.py")
STARTER_AUTHORITY_PINS: Mapping[Path, tuple[str, int]] = {
    started.SCRIPT_REL: (
        "c0b4511d2a5ec508a98603ad3db735179d07c46220e147037104538c14362920",
        45_598,
    ),
    started.TEST_REL: (
        "5df57e8e64277573e12abffb735035d96f8644a8dec017ae39ff7c40bcd01709",
        22_986,
    ),
}
...
COMPLETION_DOCUMENT_ID = (
    ...
    "COMPLETION-20260826-002"
)
COMPLETION_RECEIPT_REL = frozen_completion.COMPLETION_RECEIPT_REL
REVIEW_ROOT = Path(
    "docs/control/execution/workstream-transitions/seq99-100/review-rounds/R001"
)
...
def _private_read(root: Path, relative: Path) -> Any:
    result = started.frozen_start.seq90._stable_read(root, relative)
    require(
...
def _require_verification_summary(
    ...
    *,
    ...
    gateway = root / frozen_completion.GATEWAY_REL
    test_files = [
    ...
    expected = {
        "TYPECHECK": {
            "check_id": "TYPECHECK",
            "argv": [str(frozen_completion.LOCKED_NPM), "run", "typecheck", "--silent"],
            "status": "PASS",
        },
        "BUILD": {
            "check_id": "BUILD",
            "argv": [str(frozen_completion.LOCKED_NPM), "run", "build", "--silent"],
            "status": "PASS",
        },
        "FOCUSED_STATE_ENCRYPTION_MAINTENANCE": {
            "check_id": "FOCUSED_STATE_ENCRYPTION_MAINTENANCE",
            "argv": [
                str(frozen_completion.LOCKED_NODE),
                "--test",
                frozen_completion.FOCUSED_TEST_REL.as_posix(),
            ],
            "status": "PASS",
            "tap": focused_counts,
        },
        "ANDROID_GATEWAY_FULL": {
            ...
            "argv": [str(frozen_completion.LOCKED_NODE), "--test", *test_files],
            "status": "PASS",
            "tap": full_counts,
        },
    }
    ...
    require(
        ...
        and toolchain
        == {
            "node_path": str(frozen_completion.LOCKED_NODE),
            "node_version": frozen_completion.LOCKED_NODE_VERSION,
            "npm_path": str(frozen_completion.LOCKED_NPM),
            "npm_version": frozen_completion.LOCKED_NPM_VERSION,
        },
...
def build_completion_receipt(
    ...
    try:
        product_files = frozen_completion._verify_product_pins(root)
    ...
    require(source_at < started_at, "verification does not follow seq98 start")
    return {
        "schema_version": "1.0",
        "document_id": COMPLETION_DOCUMENT_ID,
        "goal_id": GOAL_ID,
...
def expected_review_assignment(
    ...
    def rows(paths: Sequence[Path]) -> list[dict[str, Any]]:
        return [
            _binding(relative, started.frozen_start.seq90._stable_read(root, relative).raw)
...
def reconstructed_seq98_checkpoint_bytes(
    ...
    require(
        ...
        "seq99 producer transaction lacks adjacent seq100 completion",
    )
    require(len(history) == COMPLETION_SEQUENCE, "inverse requires exact seq98 or seq100")
    ...
    require(
        ...
        "seq99/100 inverse suffix differs",
    )
    expected_history = {
        "length": SOURCE_SEQUENCE,
...
def project_seq99_100(
    ...
    *,
    ...
    ] = frozen_completion.runtime_authority.derive_runtime,
    ...
    source_at = _parse_time(source_tail["occurred_at"], label="seq98 occurred_at")
    ...
    checkpoint["canonical_bindings"] = frozen_completion._add_completion_binding(
    ...
    update: dict[str, Any] = {
        ...
        "status_changes": {},
        "runtime_after": frozen_completion._runtime_after(state),
    ...
    snapshot.update(
        {
            ...
            "scope": (
                "Graph v2.4 through FP048 R002 seq99 evidence and seq100 internal "
...
def validate_projection(
    ...
    *,
    ...
    expected, _update, _completion = project_seq99_100(
        root,
        source,
        evidence,
    ...
    require(strict_equal(projected, expected), "seq99/100 projection differs")
...
def prepare_review_inputs(
    ...
    *,
    verification_runner: Callable[[Path], Mapping[str, Any]] = (
        frozen_completion.run_fresh_verification
    ),
) -> dict[Path, bytes]:
    root = root.resolve(strict=True)
    validate_starter_authority_handshake(root)
    source_read = _private_read(root, CHECKPOINT_REL)
    source = strict_json(source_read.raw, "seq98 source checkpoint")
    require(source_read.raw == checkpoint_bytes(source), "seq98 source bytes differ")
    ...
    product_before = frozen_completion._verify_product_pins(root)
    ...
    require(
        ...
        "seq98 source changed during review preparation",
    )
    validate_starter_authority_handshake(root)
    require(
        frozen_completion._verify_product_pins(root) == product_before,
    ...
    require(
        current.identity == source_read.identity and current.raw == source_read.raw,
        "seq98 source changed while writing review inputs",
    )
    return outputs
...
def _capture_publication_inputs(
    ...
    seq90 = started.frozen_start.seq90
    ...
    seq97_raw = started.reconstructed_seq97_checkpoint_bytes(root, source)
    seq97 = strict_json(seq97_raw, "reconstructed seq97 checkpoint")
    start_paths = started._publication_retained_paths(root, seq97)
...
def _require_prepared_exact(prepared: PreparedProjection, *, phase: str) -> None:
    ...
    frozen_completion._verify_product_pins(prepared.root)
...
def prepare_projection(
    ...
    *,
    ...
    require(
        current.identity == source_read.identity and current.raw == source_read.raw,
        "seq98 source changed during fresh verification",
    )
    require(
        frozen_completion._verify_product_pins(root) == product_before,
    ...
    projected, update, completed = project_seq99_100(
        root,
        source,
        evidence,
...
def _terminal_validate(prepared: PreparedProjection) -> None:
    ...
    require(
        reconstructed_seq98_checkpoint_bytes(prepared.root, published)
        == prepared.source_bytes,
...
def write_projection(
    prepared: PreparedProjection,
    *,
    writer: Callable[..., None] = started.frozen_start.seq90.write_checkpoint,
) -> None:
    ...
    require(
        ...
        "seq98 source changed before completion publication",
    )
    _require_prepared_exact(prepared, phase="before completion publication")
    transport = started.frozen_start.seq90.Prepared(
        root=prepared.root,
    ...
    except started.frozen_start.seq90.PostcommitUncertain:
        raise
    except BaseException as exc:
        observed = _private_read(prepared.root, CHECKPOINT_REL)
        if observed.raw == prepared.projected_bytes:
            raise started.frozen_start.seq90.PostcommitUncertain(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.prepare_review:
            ...
            print(
                "FP048 R002 seq99-100 review preparation: PASS "
                + " ".join(
        ...
        if args.write:
            ...
            mode = "WRITE"
    except started.frozen_start.seq90.PostcommitUncertain as exc:
        print(f"FP048 R002 seq99-100 completion: UNCERTAIN: {exc}", file=sys.stderr)
        return 2
    except (CompletionApplyError, started.StartApplyError, OSError, TypeError, ValueError) as exc:
        print(f"FP048 R002 seq99-100 completion: FAIL: {exc}", file=sys.stderr)
        return 1
    print(
        "FP048 R002 seq99-100 completion: PASS "
