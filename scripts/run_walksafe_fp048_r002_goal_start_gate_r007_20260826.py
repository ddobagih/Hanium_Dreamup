#!/usr/bin/env python3
"""Run the post-seq94 FP-048 R002 R007 five-check private start gate."""
...
_BASE_PATH = ROOT / "scripts/run_walksafe_fp048_r002_goal_start_gate_r006_20260826.py"
_BASE_SPEC = importlib.util.spec_from_file_location(
    "_walksafe_fp048_r002_private_gate_runtime_r007_20260826",
    _BASE_PATH,
)
if _BASE_SPEC is None or _BASE_SPEC.loader is None:
    raise RuntimeError("FP048 R002 R006 gate runtime cannot be loaded privately")
base = importlib.util.module_from_spec(_BASE_SPEC)
sys.modules[_BASE_SPEC.name] = base
_BASE_SPEC.loader.exec_module(base)

_PRIVATE_R006_SYNC_RUNTIME_PATHS = base._sync_runtime_paths
_PRIVATE_R005_SYNC_RUNTIME_PATHS = base.base._sync_runtime_paths
_PRIVATE_R004_SYNC_RUNTIME_PATHS = base.base.base._sync_runtime_paths
_PRIVATE_R003_SYNC_RUNTIME_PATHS = base.base.base.base._sync_runtime_paths
_PRIVATE_R002_SYNC_RUNTIME_PATHS = base.base.base.base.base._sync_runtime_paths
...
POST_SEQ94_REGRESSION_TEST_RELATIVE = Path(
...
R006_GATE_RUNTIME_RELATIVE = Path(
    "scripts/run_walksafe_fp048_r002_goal_start_gate_r006_20260826.py"
)
R006_GATE_RUNTIME_TEST_RELATIVE = Path(
...
CONTRACT_DOCUMENT_ID = "WS-FP048-R002-INITIAL-START-GATE-CONTRACT-20260826-007"
CONTRACT_ID = "WS-FP048-R002-INTERNAL-START-GATE-R007"
CONTRACT_VERSION = "2026-08-26.6"
...
def _correction() -> Any:
    try:
        ...
        raise GateError(
            "FP048 R002 seq95 contract-correction authority is unavailable"
        ) from exc


def expected_r007_binding() -> dict[str, Any]:
    return {
        "schema_version": "1.2",
...
def _sync_runtime_paths() -> tuple[None, Any]:
    _unused, prior_authority = _PRIVATE_R006_SYNC_RUNTIME_PATHS()
    ...
    if not all(callable(getattr(correction, name, None)) for name in required):
        raise GateError("FP048 R002 seq95 contract-correction API differs")
    ...
    if (
        ...
        raise GateError("FP048 R002 seq95 contract-correction identity differs")
    source_paths = tuple(
        dict.fromkeys(
            (
                *prior_paths,
                ...
                goal_path,
                CONTRACT_RELATIVE,
                POST_SEQ94_REGRESSION_TEST_RELATIVE,
                CORRECTION_RELATIVE,
                ...
                R006_GATE_RUNTIME_RELATIVE,
                R006_GATE_RUNTIME_TEST_RELATIVE,
            )
        )
    )
    for name, value in {
...
def _document_id(event_id: str) -> str:
    ...
    if match is None or event_id != STARTED_EVENT_ID:
        raise GateError(
            f"event ID must equal the unused post-seq94 start ID {STARTED_EVENT_ID}"
        )
    return (
...
def bind_contract_corrected_source(
    ...
    *,
    ...
    try:
        ...
        raise GateError(f"seq95 contract correction rejected source: {exc}") from exc
    ...
    if not isinstance(history, list) or len(history) != SOURCE_SEQUENCE:
        raise GateError("gate source is not exact seq95")
    ...
    if (
        ...
        raise GateError("seq95 contract correction differs")
    binding = correction.r007_contract_binding(root)
    ...
    if (
        ...
        or historical_preflight_attempt
        != expected_historical_preflight_attempt_004_binding()
        or repeated_historical_preflight
        != expected_historical_preflight_attempt_004_binding()
        or preflight_attempt != expected_preflight_attempt_004_binding()
    ):
        raise GateError("seq95 R007 contract or historical preflight binding differs")
    ...
    if require_gate_namespace_absent and (
        ...
        raise GateError("R006 preflight-only namespace unexpectedly exists")
    ...
    if (
        ...
        raise GateError("seq95 READY runtime differs")
    goal_path = _repo_path(correction.GOAL_PATH, "correction GOAL_PATH")
    ...
    authority = ContractCorrectedAuthority(
        ...
        _sha(event.get("event_sha256"), "seq95 correction event SHA-256"),
        _impl._parse_timestamp(event.get("occurred_at"), label="seq95 occurred_at"),
...
def bind_published_contract_corrected_source(
    ...
    """Bind seq95 after the exact R007 gate namespace has been published."""
...
def expected_contract_binding() -> dict[str, Any]:
    if _active_authority is None:
        raise GateError("seq95 R007 contract authority has not been loaded")
...
def _load_gate_contract(
    ...
    *,
    ...
    _, correction = _sync_runtime_paths()
    authority_contract, authority_binding = correction.load_r007_contract(root)
    expected_binding = expected_r007_binding()
    if authority_binding != expected_binding:
        raise GateError("FP048 R002 R007 frozen binding differs")
    effective = dict(binding or expected_contract_binding())
    if effective != authority_binding:
        raise GateError("FP048 R002 R007 reviewed binding differs")
    ...
    if (
        ...
        raise GateError("FP048 R002 R007 contract bytes differ")
    try:
        contract = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise GateError("FP048 R002 R007 contract is not valid JSON") from exc
    if (
        ...
        raise GateError("FP048 R002 R007 canonical contract differs")
    raw_checks = contract.get("ordered_checks")
    if not isinstance(raw_checks, list):
        raise GateError("FP048 R002 R007 ordered checks are missing")
    ...
    if checks != expected_checks:
        raise GateError("FP048 R002 R007 five-check commands differ")
...
def _install_base_adapter() -> None:
    ...
    _sync_runtime_paths()
    if (
        base._sync_runtime_paths is not _PRIVATE_R006_SYNC_RUNTIME_PATHS
        or base.base._sync_runtime_paths is not _PRIVATE_R005_SYNC_RUNTIME_PATHS
        or base.base.base._sync_runtime_paths is not _PRIVATE_R004_SYNC_RUNTIME_PATHS
        ...
        raise GateError("private R006/R005/R004/R003/R002 sync authority differs")
    adapters = (
        base,
        base.base,
        base.base.base,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.preflight:
            ...
            print(
                "FP048-R002 post-seq94 initial-start gate preflight: PASS: "
                f"{len(results)} checks; no gate namespace created"
            )
            return 0
        receipt = run_gate(args.root, args.event_id)
        print(f"FP048-R002 post-seq94 initial-start gate: PASS: {receipt}")
        return 0
    except GateCheckFailed as exc:
        print(
            f"FP048-R002 post-seq94 initial-start gate: FAIL: {exc}",
            file=sys.stderr,
        )
        return exc.exit_code if exc.exit_code > 0 else 1
    except GatePostCommitUncertain as exc:
        print(
            "FP048-R002 post-seq94 initial-start gate: "
        ...
        print(
            f"FP048-R002 post-seq94 initial-start gate: ERROR: {exc}",
            file=sys.stderr,
        )
        return 2
