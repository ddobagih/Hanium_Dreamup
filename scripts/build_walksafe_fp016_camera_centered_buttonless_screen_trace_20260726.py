#!/usr/bin/env python3
"""Build the focused FP-016 camera-centered/buttonless-screen implementation trace."""
...
BUILDER_TEST = (
    ROOT / "tests/test_walksafe_fp016_camera_centered_buttonless_screen_trace_20260726.py"
)
...
GOAL_PATH = (
    ...
    "epic-02-fp016-camera-centered-buttonless-screen-r001.md"
)
...
FOCUSED_COMMAND = (
    ...
    "find app/build/test-results/testDebugUnitTest -mindepth 1 -delete && "
    "./gradlew :app:testDebugUnitTest "
    "--tests '*MainActivityFp016StaticTest' "
    "--tests '*CameraXFallbackCompositionStaticTest' "
    "--tests '*MainActivityAccessibilityStaticTest' "
    "--tests '*MainActivityFirstRunRegistrationStaticTest' "
    "--tests '*RuntimeMetricMainActivityStaticTest' "
    "--tests '*MainActivityNavigationCompositionTest' "
    "--tests '*MainActivityWalkSessionLifecycleStaticTest' "
    "--tests '*PermissionSessionLifecycleStaticTest' "
    "--tests '*MainActivityStartupCapabilityStaticTest' "
    "--tests '*WalkSafeStartupCapabilityTest' "
    "--tests '*WalkSessionLifecycleTest' "
    "--tests '*AndroidVoiceCommandTest' "
...
CONTROL_PLANE_COMMAND = (
    ...
    "build_walksafe_fp016_camera_centered_buttonless_screen_trace_20260726.py && "
    "/home/ddobagi/.local/share/hanium-dreamup/"
    "walksafe-general-cpu-verify-20260715/bin/python -m pytest -q tests/"
    "test_walksafe_fp016_camera_centered_buttonless_screen_trace_20260726.py"
)
...
BACKLOG_R018_JSON = (
    ROOT / "docs/control/audits/walksafe-implementation-remediation-backlog-20260726-r018.json"
)
FP014_OVERLAY_JSON = (
...
EXPECTED_PREDECESSOR_SHA256 = {
    GAP_R018_JSON: "833ba9a691ec5b7de66c8e16e2bd988f63781fc9f38869e63270b24da9d8bbcd",
    BACKLOG_R018_JSON: "667ae83090e73d39e8ea371f2543e560a5d8465505dad323c84e901f369e31b0",
    FP014_OVERLAY_JSON: "19f7f1e6caa02960ce744605c5ec23383d6a1ef6a93f07916beb988208993937",
}
...
EXPECTED_START_GATE_REPOSITORY_STATE_LOG_SHA256 = (
    "5eca1a12a43c034c457207b677b7d445f38349276ee9fde81a2ef0615b663007"
)
EXPECTED_PINNED_HEAD = "a3ad7eead6b5d834d3e0675422475a9aad351e3d"
EXPECTED_EXECUTION_EVENT_SEQUENCE = 24
...
EXPECTED_EXECUTION_STARTED_AT = "2026-07-26T05:43:31+09:00"
EXECUTOR_ID = "CODEX-FP016-CAMERA-CENTERED-BUTTONLESS-SCREEN-IMPLEMENTER-20260726-001"
EXECUTOR_TASK = "FP016_CAMERA_CENTERED_BUTTONLESS_SCREEN_IMPLEMENTATION"
EXPECTED_REVIEWER_ID = "WS-FP016-INDEPENDENT-REVIEWER-001"
EXPECTED_REVIEWER_TASK = "/root/fp016_final_evidence_review"
...
CANONICAL_LOG_IMPLEMENTATION_CONTENT_SET_SHA256_MARKER = (
    "WALKSAFE_IMPLEMENTATION_EXACT_13_CONTENT_SET_SHA256"
)
...
def validate_execution_history(goal_execution: dict[str, Any]) -> dict[str, Any]:
    ...
    require(
        ...
        "FP-016 execution event is not exact-one",
    )
    ...
    require(
        ...
        "FP-016 execution event differs",
    )
    return execution
...
def validate_inputs(
    ...
    gap = base.load_json(GAP_R018_JSON)
    backlog = base.load_json(BACKLOG_R018_JSON)
    overlay = base.load_json(FP014_OVERLAY_JSON)
    require(
        gap.get("metadata", {}).get("report_id")
        == "WS-IMPLEMENTATION-GAP-ANALYSIS-20260726-018",
        "r018 gap identity differs",
    )
    require(
        backlog.get("metadata", {}).get("backlog_id")
        == "WS-IMPLEMENTATION-REMEDIATION-BACKLOG-20260726-018",
        "r018 backlog identity differs",
    )
    require(
        overlay.get("metadata", {}).get("overlay_id")
        == "WS-EPIC-02-FP014-PERMISSION-DENIAL-REVOCATION-ACTIVE-LEDGER-OVERLAY-20260726-001",
...
def completion_boundary() -> dict[str, Any]:
    return {
        "formal_test_ids": [f"TC-FP-016-{index:02d}" for index in range(1, 5)],
...
def build_implementation(
    *,
    ...
    return {
        "schema_version": "1.0",
        "document_id": "WS-FP016-CAMERA-CENTERED-BUTTONLESS-SCREEN-IMPLEMENTATION-20260726-001",
        "goal_id": GOAL_ID,
        ...
        "implemented_controls": [
            "release ACTIVE and PAUSED walk surfaces are camera-centered and contain no general control buttons",
            "Gateway URL, token, coordinates, capture, upload and field-log controls are debug-only",
            "CameraX fallback binds Preview and ImageAnalysis to the same lifecycle",
            "system back pauses first, invalidates stale readiness and output authority, then opens an accessible exit confirmation",
            "canceling exit confirmation remains paused and requires the existing explicit resume contract",
            "persistent camera or required-voice failure transitions the walk session to SAFE_STOP",
            "high-frequency state is excluded from accessibility live regions",
            "a low-frequency polite TalkBack safety summary remains available",
            "dependency locks and verification metadata bind CameraX view and AndroidX activity dependencies",
            "focused Android regressions cover release surface, CameraX, accessibility, lifecycle and startup capability",
            "the trace binds exact product-ten and tooling-three scopes as one canonical exact-thirteen content set",
        ],
        "remaining_implementation_boundaries":
 [
            "TC-FP-016-01 through TC-FP-016-04 and the 279 formal tests were not run",
            "actual users, TalkBack users and Android devices were not tested",
            "Android platform review was not performed",
            "the operational screen contract was not approved",
...
def checked_log(
    ...
    *,
    ...
    execution_started_at = parse_aware_timestamp(
        ...
        "FP-016 execution start timestamp",
    )
    require(
        started_at >= execution_started_at,
        f"{name} log starts before the FP-016 execution event",
    )
    require(
        started_at <= ended_at,
    ...
    *,
    ...
    checks = [
        checked_log(
            ...
            name="FP-016 focused Android consent tests",
        ...
        checked_log(
            ...
            name="Android Gateway camera-centered-buttonless-screen typecheck and tests",
        ...
        checked_log(
            ...
            name="FP-016 deterministic trace regression",
    ...
    return {
        "schema_version": "1.0",
        "document_id": "WS-FP016-CAMERA-CENTERED-BUTTONLESS-SCREEN-VERIFICATION-20260726-001",
        "goal_id": GOAL_ID,
...
def build_review(
    ...
    return {
        "schema_version": "1.0",
        "document_id": "WS-FP016-CAMERA-CENTERED-BUTTONLESS-SCREEN-INTERNAL-REVIEW-20260726-001",
...
def build_receipt(
    ...
    return {
        "schema_version": "1.0",
        "document_id": (
            "WS-FP016-CAMERA-CENTERED-BUTTONLESS-SCREEN-WORK-ITEM-COMPLETION-20260726-001"
        ),
...
def build_gap(
    ...
    report["purpose"] = (
        "FP-016 camera-centered buttonless walk-screen "
        "fail-closed controls are implemented and verified internally. GAP-025 "
    ...
    report["source_bindings"] = predecessor_gap["source_bindings"] + [
        {**base.file_record(BUILDER), "name": "fp016_camera_screen_trace_builder"},
        {**base.file_record(BUILDER_TEST), "name": "fp016_camera_screen_builder_test"},
        {**base.file_record(START_GATE_RECEIPT), "name": "fp016_start_gate"},
        {
            **base.file_record(START_GATE_REPOSITORY_STATE_LOG),
            "name": "fp016_gate_repository_state",
        },
        generated_record(
            IMPLEMENTATION_JSON,
            implementation_text,
            name="fp016_camera_screen_implementation",
        ),
        generated_record(
            VERIFICATION_JSON,
            verification_text,
            name="fp016_camera_screen_verification",
        ),
    ]
    report["source_binding_sha256"] = base.object_sha256(report["source_bindings"])
    files = implementation_files()
    snapshot = {
        "scope_kind": "EPIC_02_FP016_EXACT_13_PATH_SET",
    ...
    report["summary"] = {
        **predecessor_gap["summary"],
        ...
        "headline": (
            "GAP-025 internal camera-centered buttonless screen controls are verified, "
    ...
    evidence_id = "EVD-FP016-CAMERA-CENTERED-BUTTONLESS-SCREEN-20260726"
    report["evidence_catalog"] = predecessor_gap["evidence_catalog"] + [
        {
            "evidence_id": evidence_id,
            "kind": "INTERNAL_PRODUCER_RESULT",
            "claim": (
                "FP-016 repository-internal camera-centered buttonless walk-screen "
    ...
    matches = [
        item for item in report["assessments"] if item["gap_id"] == "GAP-025"
    ]
    require(len(matches) == 1, "GAP-025 assessment count differs")
    assessment = matches[0]
    require(
        assessment.get("source_policy_id") == "FP-016"
        and assessment.get("status") == "CONFLICTING",
        "GAP-025 predecessor assessment differs",
    )
    ...
    carried_ids = [
        ...
        if item["gap_id"] != "GAP-025"
    ]
    require(len(carried_ids) == 67, "r018 carry-forward gap count differs")
    report["reassessment_scope"] = {
        "mode": "FOCUSED_EPIC02_FP016_GAP023_REASSESSMENT_WITH_R017_CARRY_FORWARD",
        "directly_reassessed_gap_ids": ["GAP-025"],
        "impact_reviewed_gap_ids": [],
        "reviewed_gap_ids": ["GAP-025"],
        "carried_forward_gap_count": 67,
        "carried_forward_gap_ids": carried_ids,
        "carry_forward_warning": (
            "The other 67 assessments are preserved from r018."
        ),
        "next_adjacent_gap": {
            "gap_id": "GAP-021",
            "source_policy_id": "FP-012",
            "reason": "EPIC-02 deterministic execution order advances to item 17.",
        },
        "predecessor": {
            "report_id": predecessor_gap["metadata"]["report_id"],
            "path": relative(GAP_R018_JSON),
            "file_sha256": EXPECTED_PREDECESSOR_SHA256[GAP_R018_JSON],
            "evidence_count": len(predecessor_gap["evidence_catalog"]),
            "revalidated_wholesale_in_r019": False,
        },
    }
    ...
    report["limitations"] = [
        "Only GAP-025 is directly reassessed; 67 assessments are carried from r018.",
        "TC-FP-016-01 through TC-FP-016-04 and all 279 formal tests are NOT_RUN.",
        "Actual-user, TalkBack-user and Android-device verification is NOT_RUN.",
        "Android platform review and operational screen-contract approval are NOT_RUN.",
...
def build_backlog(
    ...
    backlog["metadata"] = {
        "backlog_id": "WS-IMPLEMENTATION-REMEDIATION-BACKLOG-20260726-019",
        "version": "0.19.0",
        "status": "DRAFT_DIAGNOSTIC_COMPLETE",
    ...
    backlog["source_predecessor"] = {
        "path": relative(BACKLOG_R018_JSON),
        "file_sha256": EXPECTED_PREDECESSOR_SHA256[BACKLOG_R018_JSON],
    ...
    require(len(matching) == 1, "FP-016 backlog action count differs")
    require(matching[0].get("status") == "CONFLICTING", "FP-016 backlog status differs")
    matching[0]["status"] = "PARTIAL"
    matching[0]["action"] = (
        "Connect the verified internal camera-centered buttonless screen controls "
        "to actual-user, TalkBack-user, Android-device, platform, operational "
        "screen-contract, formal and release verification."
    )
    ...
    epic["current_status_reason"] = (
        "FP-016 internal camera-centered buttonless screen controls are verified; "
        "FP-012 and later policies plus external and release verification remain."
    )
    next_row = next(
        item
        for item in backlog["next_action_sequence"]
...
def build_overlay(
    ...
    return {
        ...
        "metadata": {
            "overlay_id": (
                "WS-EPIC-02-FP016-CAMERA-CENTERED-BUTTONLESS-SCREEN-"
                "ACTIVE-LEDGER-OVERLAY-20260726-001"
            ),
            "version": "0.1.0",
            "as_of": "2026-07-26",
            "status": "ACTIVE_EVENT_OVERLAY",
            "title": "EPIC-02 FP-016 camera-centered/buttonless-screen successor overlay",
        },
        "authority_boundary": {
        ...
        "source_bindings": {
            "predecessor_overlay": base.file_record(FP014_OVERLAY_JSON),
            "implementation_record": generated_record(
                IMPLEMENTATION_JSON,
                implementation_text,
                name="fp016_implementation",
            ),
            "verification_result": generated_record(
                VERIFICATION_JSON,
                verification_text,
                name="fp016_verification",
            ),
...
def build_successor(
    ...
    return {
        "schema_version": "1.0",
        "document_id": "WS-FP016-CAMERA-CENTERED-BUTTONLESS-SCREEN-SUCCESSOR-20260726-001",
        "goal_id": GOAL_ID,
...
def gap_markdown(value: dict[str, Any]) -> str:
    row = next(item for item in value["assessments"] if item["gap_id"] == "GAP-025")
    return (
        "# WalkSafe 구현 Gap 분석 r019\n\n"
        "- 직접 재평가: `FP-016 / GAP-025`\n"
        f"- 판정: `{row['status']}` (`CONFLICTING → PARTIAL`)\n"
        "- Android·Gateway focused/전체 회귀와 build·lint/typecheck·경계 검사: `PASS`\n"
        "- TC-FP-016-01~04·정식 279개·실제 사용자·TalkBack·기기·플랫폼·"
...
def overlay_markdown(value: dict[str, Any]) -> str:
    return (
        "# EPIC-02 FP-016 권한 거부·철회 Active ledger overlay\n\n"
        f"- Overlay: `{value['metadata']['overlay_id']}`\n"
        "- FP-016 내부 권한 거부·철회 구현·검증: `PARTIAL_IMPLEMENTATION_VERIFIED`\n"
...
def build_review_subject(
    ...
    return {
        ...
        "implementation_scope": {
            "scope": "EXACT_13_PATH_SET",
            "exact_path_count": 13,
            "product_path_count": 10,
...
def build_outputs(
    *,
    ...
    non_receipt_outputs = {
        ...
        GAP_R019_JSON: gap_text,
        GAP_R019_MD: gap_markdown(gap),
        BACKLOG_R019_JSON: backlog_text,
        BACKLOG_R019_MD: backlog_markdown(backlog),
        FP016_OVERLAY_JSON: json_text(overlay),
        FP016_OVERLAY_MD: overlay_markdown(overlay),
    }
    output_manifest = [
        {
            "path": relative(path),
    ...
    outputs = {
        ...
        GAP_R019_JSON: gap_text,
        GAP_R019_MD: non_receipt_outputs[GAP_R019_MD],
        BACKLOG_R019_JSON: backlog_text,
        BACKLOG_R019_MD: non_receipt_outputs[BACKLOG_R019_MD],
        FP016_OVERLAY_JSON: non_receipt_outputs[FP016_OVERLAY_JSON],
        FP016_OVERLAY_MD: non_receipt_outputs[FP016_OVERLAY_MD],
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.write_review_subject:
            ...
            print(
                "FP-016 camera-centered-buttonless-screen trace: PASS "
        ...
        print(f"FP-016 camera-centered-buttonless-screen trace: FAIL: {exc}", file=sys.stderr)
        return 1
    mode_name = "WRITE" if args.write else "CHECK"
    print(
        f"FP-016 camera-centered-buttonless-screen trace: PASS outputs={len(outputs)} "
        f"next=FP-012/GAP-021 mode={mode_name}"
    )
    return 0
