#!/usr/bin/env python3
"""Validate the WalkSafe v2.3 successor Goal package and runtime checkpoint."""
...
from scripts import check_walksafe_project_continuation_v2_3 as continuation  # noqa: E402


CHECKPOINT_RELATIVE = Path("docs/control/walksafe-project-continuation-checkpoint.json")
PACKAGE_RELATIVE = Path("docs/control/goals/walksafe-completion-graph-v2-3")
MANIFEST_RELATIVE = PACKAGE_RELATIVE / "static-plan-manifest-v2.3.0.json"
EXPECTED_PACKAGE_ID = "WS-GOAL-PACKAGE-WALKSAFE-COMPLETION-GRAPH-V2-3"
EXPECTED_PLAN_VERSION = "2.3.0"
EXPECTED_ROOT_GOAL_ID = "WS-GOAL-WALKSAFE-COMPLETION-GRAPH-V2"
EXPECTED_INITIAL_FOCUS_GOAL_ID = "WS-GOAL-EPIC-02-FP-005-R001"
EXPECTED_BOOTSTRAP_DYNAMIC_GOAL_ID = "WS-GOAL-EPIC-02-FP-018-R001"
EXPECTED_IMPORTED_DYNAMIC_GOAL_IDS = {
...
EXPECTED_COMMITTED_V23_HISTORY_PREFIX = (
    (
        "WS-GOAL-GRAPH-V2-3-PACKAGE-PREPARED-20260724-001",
        "PACKAGE_PREPARED",
        "3e2a585a8224527c5c732c3ac39cfc13fb35ed85613a1f97a0295339c71170ef",
    ),
    (
        "WS-GOAL-GRAPH-V2-3-PACKAGE-ACTIVATED-20260724-002",
        "PACKAGE_ACTIVATED",
    ...
    (
        "WS-GOAL-GRAPH-V2-3-CANONICAL-BINDINGS-UPDATED-FP005-20260724-001",
        "CANONICAL_BINDINGS_UPDATED",
        "3274593ee99167988bc84b0457add027ae45201456b826ada276fe8f5d4922b1",
    ),
    (
        "WS-GOAL-GRAPH-V2-3-GOAL-COMPLETED-FP005-20260724-001",
        "GOAL_COMPLETED",
        "389d864caedbab6fa0bdd157b094237ae55edaa139f7b583d20ea163fa583e13",
    ),
    (
        "WS-GOAL-GRAPH-V2-3-GOAL-MATERIALIZED-FP006-20260724-001",
        "GOAL_MATERIALIZED",
        "c956a273e25bf408aa5d75588131213f53e883dcc7a3cb2fdb7f36d88845e419",
    ),
    (
        "WS-GOAL-GRAPH-V2-3-GOAL-READY-FP006-20260724-001",
        "GOAL_READY",
        "7e73059f16a076029762d524c17ec3b05b8063f51e2fe11b2ca52c1170685980",
    ),
    (
...
EXPECTED_PACKAGE_ACTIVATION_AUTHORIZATION_SHA256 = (
    "1eedc137abb1cd72ead14fddd5e27c184ba5c9460a7bfdd6070e1f728bdceae9"
)
...
V22_ARCHIVED_CHECKPOINT_RELATIVE = (
    PACKAGE_RELATIVE / "superseded-v2.2.0-active-checkpoint.json"
)
V22_SUPERSESSION_RECORD_RELATIVE = (
    PACKAGE_RELATIVE / "active-supersession-record-v2.2.0.json"
)
EXPECTED_V22_PACKAGE_ID = (
    "WS-GOAL-PACKAGE-WALKSAFE-COMPLETION-GRAPH-V2-2"
)
EXPECTED_V22_PLAN_VERSION = "2.2.0"
V22_PACKAGE_GENERATION = 22
V23_PACKAGE_GENERATION = 23
...
EXPECTED_V22_ARCHIVED_CHECKPOINT_RAW_SHA256 = (
    "c15d677c9d8f9b68227cb0b2ffaac3aef4794e2bdb031b8cc4ca4a4790fc6024"
)
EXPECTED_FROZEN_V22_CHECKER_SHA256 = (
    "64c5e733499029f608dc56d76096bb25a97bfdf043fa483180565a55d7d68d9e"
)
EXPECTED_FROZEN_V22_TEST_SHA256 = (
    "ba195e5c26d8a91eca6b88e09587d2f58fcc213590977d7ef95b434e781db297"
)
ARCHIVED_CHANGED_ARTIFACT_ERROR_PATTERN = re.compile(
    r"^archived v2\.2: "
    r"(?:(?P<event_label>Goal graph event [1-9][0-9]*): )?"
    r"(?P<goal_id>[A-Za-z0-9._-]+): "
    r"implementation changed artifact (?P<index>[0-9]+) differs$"
)
REPLAYED_IMPORTED_CHANGED_ARTIFACT_ERROR_PATTERN = re.compile(
    r"^(?:(?P<event_label>Goal graph event [1-9][0-9]*): )?"
    r"(?P<goal_id>[A-Za-z0-9._-]+): "
    r"implementation changed artifact (?P<index>[0-9]+) differs$"
)
...
PRESTART_CONTROL_REPAIR_SEMANTIC_REGRESSION_COMMAND = (
    'WALKSAFE_LOCKED_TEST_PYTHON="${WALKSAFE_LOCKED_TEST_PYTHON:-/home/ddobagi/.local/share/hanium-dreamup/walksafe-general-cpu-verify-20260715/bin/python}" && test -x "${WALKSAFE_LOCKED_TEST_PYTHON}" && "${WALKSAFE_LOCKED_TEST_PYTHON}" -m pytest tests/test_walksafe_goal_graph_v2_3.py tests/test_walksafe_project_continuation_v2_3.py tests/test_walksafe_goal_graph_v2_2_history.py -q '
    "--deselect=tests/test_walksafe_goal_graph_v2_3.py::WalkSafeGoalGraphV23Test::test_current_goal_graph_is_valid "
    "--deselect=tests/test_walksafe_project_continuation_v2_3.py::WalkSafeProjectContinuationV23Test::test_current_checkpoint_is_valid"
)
...
PACKAGE_ACTIVATED_EVENT_FIELDS = frozenset(
    {
        "sequence",
        "event_id",
        "event_type",
...
def validate_committed_v23_history_prefix(history: Any) -> list[str]:
    """Keep the committed v2.3 lineage immutable while fixtures stay generic."""
    if not isinstance(history, list):
        return []
    committed_event_ids = {
        event_id
        for event_id, _, _ in EXPECTED_COMMITTED_V23_HISTORY_PREFIX[1:]
    }
    if not any(
        isinstance(event, dict)
        and event.get("event_id") in committed_event_ids
        for event in history
    ):
        return []
    if len(history) < len(EXPECTED_COMMITTED_V23_HISTORY_PREFIX):
        return ["Goal graph v2.3 committed history prefix is truncated"]
    errors: list[str] = []
    for sequence, expected in enumerate(
        EXPECTED_COMMITTED_V23_HISTORY_PREFIX,
        start=1,
    ):
        event_id, event_type, event_hash = expected
        ...
        if actual != (event_id, event_type, event_hash):
            errors.append(
                f"Goal graph v2.3 committed event {sequence} differs"
            )
    return errors
...
def validate_dynamic_work_items(
    ...
    for goal_id, node in nodes.items():
        ...
        expected_initial_status = (
            "READY"
            if goal_id == EXPECTED_BOOTSTRAP_DYNAMIC_GOAL_ID
            else "PLANNED"
        )
        ...
        elif goal_id not in EXPECTED_IMPORTED_DYNAMIC_GOAL_IDS:
...
def completion_event_with_package_generation(
    event: dict[str, Any],
    package_generation: int,
) -> dict[str, Any]:
    annotated = dict(event)
    annotated["_package_generation"] = package_generation
    return annotated


def completion_event_order(
    event: dict[str, Any],
) -> tuple[int, int] | None:
    generation = event.get("_package_generation", 0)
...
def canonical_event_without_internal_metadata(
    event: dict[str, Any],
) -> dict[str, Any]:
    return {
        key: value
...
def completed_successor_artifact_chain_reaches_live(
    ...
    *,
    ...
    source_order = completion_event_order(source_event)
    ...
    candidates = sorted(
        (
            (completion_event_order(event), candidate_goal_id)
            for candidate_goal_id, event in completion_event_by_goal.items()
            if candidate_goal_id != goal_id
            and isinstance(event, dict)
            and completion_event_order(event) is not None
            and completion_event_order(event) > source_order
    ...
    for _, candidate_goal_id in candidates:
        candidate_event = completion_event_by_goal[candidate_goal_id]
        ...
        if (
            ...
            return False
        if "_package_generation" in candidate_event:
            canonical_event = canonical_event_without_internal_metadata(
                candidate_event
            )
            if (
                candidate_event.get("event_type") != "GOAL_COMPLETED"
                or candidate_event.get("subject_goal_id")
                != candidate_goal_id
                or candidate_event.get("completion_receipt_binding")
                != binding
                or (
                    candidate_event.get("completion_evidence_bindings")
                    or {}
                ).get(receipt_role)
                != binding
                or candidate_event.get("event_sha256")
                != event_sha256(canonical_event)
            ):
                return False
        try:
            receipt = load_json(receipt_path)
            ...
            return False
        if (
            "_package_generation" in candidate_event
            and receipt.get("target_goal_id") != candidate_goal_id
        ):
            return False
...
def validate_transition_history(
    ...
    seed_errors, imported_completion_seed = (
        imported_predecessor_completion_seed(root, state)
    )
    errors.extend(seed_errors)
    for imported_goal_id, imported_record in (
        imported_completion_seed.items()
    ):
        imported_event = imported_record.get("event")
        imported_bindings = imported_record.get("bindings")
        if isinstance(imported_event, dict):
            completion_event_by_goal[imported_goal_id] = imported_event
            historical_completion_event_by_goal[
                imported_goal_id
            ] = imported_event
        if isinstance(imported_bindings, dict):
            typed_bindings = {
                role: binding
                for role, binding in imported_bindings.items()
                if isinstance(role, str) and isinstance(binding, dict)
            }
            completion_bindings_by_goal[imported_goal_id] = typed_bindings
            historical_completion_bindings_by_goal[
                imported_goal_id
            ] = typed_bindings
    for declared_event in history:
        ...
        if (
            ...
            historical_completion_event_by_goal[declared_goal_id] = (
                completion_event_with_package_generation(
                    declared_event,
                    V23_PACKAGE_GENERATION,
                )
            )
    ...
    for index, event in enumerate(history, start=1):
        ...
        if (
            ...
            and set(event) != PACKAGE_ACTIVATED_EVENT_FIELDS
        ):
            errors.append(
                f"{label}: PACKAGE_ACTIVATED event fields differ"
            )
        ...
        if event_type == "PACKAGE_PREPARED":
            ...
            for goal_id, status in changes.items():
                if status == "COMPLETE_AT_TARGET":
                    refs = list(event_references)
                    completion_event_by_goal[goal_id] = (
                        completion_event_with_package_generation(
                            event,
                            V23_PACKAGE_GENERATION,
                        )
    ...
    if (
        ...
        errors.append(
            "Goal graph preparation event lacks the active v2.2 tail binding"
        )
    if state.get("activation_status") == "READY_NOT_ACTIVATED":
        if len(history) != 1:
    ...
    if repair_event_count:
        errors.append(
            "Goal graph v2.3 must not replay the frozen v2.2 prestart "
            "control repair"
        )
...
def validate_root_goal_status_summary(
    state: dict[str, Any],
    statuses: dict[str, Any],
) -> list[str]:
    """The summary follows the Master Goal, never the current leaf."""
    if state.get("goal_status") != statuses.get(EXPECTED_ROOT_GOAL_ID):
        return ["Goal graph root summary status differs"]
    return []
...
def validate_runtime(
    ...
    errors.extend(validate_root_goal_status_summary(state, statuses))
...
EXPECTED_V23_NATIVE_PATHS = {
    MANIFEST_RELATIVE.as_posix(),
    (PACKAGE_RELATIVE / "README.md").as_posix(),
    V22_SUPERSESSION_RECORD_RELATIVE.as_posix(),
    V22_ARCHIVED_CHECKPOINT_RELATIVE.as_posix(),
    (PACKAGE_RELATIVE / "templates/dynamic-node-template.md").as_posix(),
    (PACKAGE_RELATIVE / "templates/policy-gap-work-item.md").as_posix(),
}
EXPECTED_V23_PROTECTED_PATHS = (
    EXPECTED_V23_NATIVE_PATHS - {MANIFEST_RELATIVE.as_posix()}
)
EXPECTED_IMPORTED_GOAL_COUNT = 17
EXPECTED_MANAGED_GOAL_PATH_COUNT = 23
IMPORTED_GOAL_BASE_FIELDS = {
    "goal_id",
    "path",
    "sha256",
    "goal_kind",
...
IMPORTED_GOAL_WORK_ITEM_FIELDS = {"materialized_event_sha256"}
IMPORTED_GOAL_COMPLETE_FIELDS = {
    "completion_event_sha256",
    "completion_evidence_refs",
}


def native_package_paths(root: Path) -> list[str]:
    package_root = root / PACKAGE_RELATIVE
    return sorted(
        path.relative_to(root).as_posix()
        for path in package_root.rglob("*")
...
def imported_goal_binding_map(
    state: dict[str, Any],
) -> dict[str, dict[str, Any]]:
    value = state.get("imported_predecessor_goal_bindings")
    if not isinstance(value, dict):
        return {}
    return {
        goal_id: record
        for goal_id, record in value.items()
        if isinstance(goal_id, str) and isinstance(record, dict)
    }


def discover_package_paths(root: Path) -> list[str]:
    """Return the mixed v2.3 managed set: six native plus 17 imported Goals."""
    managed = set(native_package_paths(root))
    ...
    if isinstance(state, dict):
        managed.update(
            record.get("path")
            for record in imported_goal_binding_map(state).values()
            if isinstance(record.get("path"), str)
        )
    return sorted(managed)


def _load_archived_v22_checkpoint(
    root: Path,
) -> tuple[list[str], dict[str, Any]]:
    path = resolve_safe_repo_file(
        root,
    ...
    if path is None:
        return ["archived active v2.2 checkpoint is missing or unsafe"], {}
    if sha256_file(path) != EXPECTED_V22_ARCHIVED_CHECKPOINT_RAW_SHA256:
        return ["archived active v2.2 checkpoint raw SHA-256 differs"], {}
    try:
        checkpoint = load_json(path)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        return [f"archived active v2.2 checkpoint cannot be loaded: {exc}"], {}
    return [], checkpoint


def _validate_archived_checkpoint_with_frozen_v22_checker(
    root: Path,
) -> list[str]:
    """Run the frozen v2.2 checker in an isolated module namespace."""
    errors: list[str] = []
    checker_path = root / "scripts/check_walksafe_goal_graph.py"
    test_path = root / "tests/test_walksafe_goal_graph.py"
    if not checker_path.is_file():
        return ["frozen v2.2 checker is missing"]
    if not test_path.is_file():
        return ["frozen v2.2 checker test is missing"]
    if sha256_file(checker_path) != EXPECTED_FROZEN_V22_CHECKER_SHA256:
        errors.append("frozen v2.2 checker content changed")
    if sha256_file(test_path) != EXPECTED_FROZEN_V22_TEST_SHA256:
        errors.append("frozen v2.2 checker test content changed")
    if errors:
        return errors
    archive_errors, archived = _load_archived_v22_checkpoint(root)
    if archive_errors or not archived:
        return archive_errors
    archived_state = archived.get("goal_execution")
    if not isinstance(archived_state, dict):
        return ["archived v2.2 goal_execution is missing"]
    module_name = (
        "_walksafe_goal_graph_v22_frozen_"
        f"{hashlib.sha256(str(root).encode()).hexdigest()[:16]}"
    )
    spec = importlib.util.spec_from_file_location(module_name, checker_path)
    if spec is None or spec.loader is None:
        return ["frozen v2.2 checker cannot be imported"]
    module = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(module)
        module.CHECKPOINT_RELATIVE = V22_ARCHIVED_CHECKPOINT_RELATIVE
        archived_errors = module.validate(
            root,
            check_continuation=False,
        )
    except Exception as exc:  # pragma: no cover - fail closed at trust boundary
        return [f"frozen v2.2 archive validation failed to execute: {exc}"]
    return [f"archived v2.2: {error}" for error in archived_errors]


def partition_archived_changed_artifact_errors(
    errors: list[str],
) -> tuple[list[str], list[str]]:
    hard_errors: list[str] = []
    deferred_errors: list[str] = []
    for error in errors:
        if ARCHIVED_CHANGED_ARTIFACT_ERROR_PATTERN.fullmatch(error):
            deferred_errors.append(error)
        else:
            hard_errors.append(error)
    return hard_errors, deferred_errors


def archived_changed_artifact_core_error(error: str) -> str | None:
    match = ARCHIVED_CHANGED_ARTIFACT_ERROR_PATTERN.fullmatch(error)
    if match is None:
        return None
    return (
        f"{match.group('goal_id')}: implementation changed artifact "
        f"{match.group('index')} differs"
    )


def _archived_v22_goal_projection(
    root: Path,
    archived: dict[str, Any],
) -> tuple[list[str], dict[str, dict[str, Any]]]:
    errors: list[str] = []
    state = archived.get("goal_execution")
    if not isinstance(state, dict):
        return ["archived v2.2 goal_execution is missing"], {}
    statuses = state.get("status_by_goal")
    evidence = state.get("completion_evidence_by_goal")
    ...
    if not isinstance(statuses, dict):
        return ["archived v2.2 status map is malformed"], {}
    if not isinstance(evidence, dict):
        evidence = {}
    if not isinstance(inventory, dict):
        inventory = {}
    if not isinstance(history, list):
        return ["archived v2.2 transition history is malformed"], {}
    ...
    if manifest_path is None:
        return ["frozen v2.2 manifest is missing or unsafe"], {}
    try:
        manifest = load_json(manifest_path)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        return [f"frozen v2.2 manifest cannot be loaded: {exc}"], {}
    paths_by_id = {
        goal_id: node.get("path")
        for goal_id, node in manifest_node_map(manifest).items()
        if isinstance(node.get("path"), str)
    }
    paths_by_id.update(
        {
            goal_id: record.get("path")
    ...
    completion_event_by_goal: dict[str, str] = {}
    for event in history:
        if not isinstance(event, dict):
            continue
        if event.get("event_type") in {"GOAL_COMPLETED", "PACKAGE_COMPLETED"}:
            goal_id = event.get("subject_goal_id")
            digest = event.get("event_sha256")
            if isinstance(goal_id, str) and isinstance(digest, str):
                completion_event_by_goal[goal_id] = digest
    first_event_hash = (
    ...
    for goal_id, status in sorted(statuses.items()):
        path_value = paths_by_id.get(goal_id)
        path = resolve_safe_repo_file(root, path_value)
        if path is None:
            errors.append(f"archived v2.2 Goal path is missing: {goal_id}")
            continue
        try:
            node, _ = parse_goal(path)
        except (OSError, ValueError, tomllib.TOMLDecodeError) as exc:
            errors.append(f"archived v2.2 Goal cannot be parsed: {goal_id}: {exc}")
            continue
        record: dict[str, Any] = {
            "goal_id": goal_id,
            "path": str(path_value),
            "sha256": sha256_file(path),
            "goal_kind": node.get("goal_kind"),
            "status": status,
            "source_package_id": EXPECTED_V22_PACKAGE_ID,
        }
        if node.get("goal_kind") == "WORK_ITEM":
            materialized = inventory.get(goal_id)
            materialized_hash = (
                materialized.get("materialized_event_sha256")
                if isinstance(materialized, dict)
                else None
            )
            record["materialized_event_sha256"] = materialized_hash
        if status == "COMPLETE_AT_TARGET":
            completion_event_hash = completion_event_by_goal.get(goal_id)
            if completion_event_hash is None and goal_id == "WS-GOAL-EPIC-01":
                completion_event_hash = first_event_hash
            record["completion_event_sha256"] = completion_event_hash
            record["completion_evidence_refs"] = evidence.get(goal_id)
        projection[goal_id] = record
    return errors, projection


def validate_imported_predecessor_goal_bindings(
    root: Path,
    state: dict[str, Any],
    archived: dict[str, Any],
) -> list[str]:
    errors, expected = _archived_v22_goal_projection(root, archived)
    actual = imported_goal_binding_map(state)
    raw_actual = state.get("imported_predecessor_goal_bindings")
    if not isinstance(raw_actual, dict):
        return errors + ["imported predecessor Goal bindings must be an object"]
    if len(expected) != EXPECTED_IMPORTED_GOAL_COUNT:
        errors.append("archived v2.2 Goal projection count differs")
    if set(actual) != set(expected):
        errors.append("imported predecessor Goal ID set differs from v2.2")
        return errors
    for goal_id in sorted(expected):
        record = actual[goal_id]
        expected_record = expected[goal_id]
        expected_fields = set(IMPORTED_GOAL_BASE_FIELDS)
        if expected_record.get("goal_kind") == "WORK_ITEM":
            expected_fields.update(IMPORTED_GOAL_WORK_ITEM_FIELDS)
        if expected_record.get("status") == "COMPLETE_AT_TARGET":
            expected_fields.update(IMPORTED_GOAL_COMPLETE_FIELDS)
        if set(record) != expected_fields:
            errors.append(f"{goal_id}: imported Goal binding field set differs")
        if record != expected_record:
            errors.append(f"{goal_id}: imported Goal binding differs from v2.2 archive")
    return errors


def validate_superseded_v2_2(
    root: Path,
    manifest: dict[str, Any],
    *,
    deferred_changed_artifact_errors: list[str] | None = None,
) -> tuple[list[str], dict[str, Any]]:
    errors: list[str] = []
    manifest_path = resolve_safe_repo_file(
        root,
    ...
    if manifest_path is None:
        errors.append("frozen v2.2 manifest is missing or unsafe")
    elif sha256_file(manifest_path) != EXPECTED_V22_MANIFEST_SHA256:
        errors.append("frozen v2.2 manifest content changed")
    ...
    if archived:
        ...
        if (
            archived_state.get("package_id") != EXPECTED_V22_PACKAGE_ID
            or archived_state.get("static_plan_version")
            != EXPECTED_V22_PLAN_VERSION
            or archived_state.get("static_plan_manifest_sha256")
            != EXPECTED_V22_MANIFEST_SHA256
            or archived_state.get("activation_status") != "ACTIVE"
            or archived_state.get("package_status") != "ACTIVE"
            or not isinstance(history, list)
            or len(history) != EXPECTED_V22_TRANSITION_EVENT_COUNT
            or archived_state.get("transition_history_anchor_sha256")
            != EXPECTED_V22_TRANSITION_HISTORY_ANCHOR_SHA256
        ):
            errors.append("archived active v2.2 runtime boundary differs")

    expected_supersedes = {
        "package_id": EXPECTED_V22_PACKAGE_ID,
        "plan_version": EXPECTED_V22_PLAN_VERSION,
        ...
        "transition_event_count": EXPECTED_V22_TRANSITION_EVENT_COUNT,
    ...
    if manifest.get("supersedes") != expected_supersedes:
        errors.append("v2.3 manifest active v2.2 supersedes binding differs")
    ...
    if record_path is None:
        errors.append("active v2.2 supersession record is missing or unsafe")
    else:
        try:
            record = load_json(record_path)
        except (OSError, ValueError, json.JSONDecodeError) as exc:
            errors.append(f"active v2.2 supersession record cannot be loaded: {exc}")
        else:
            if record.get("schema_version") != "1.0":
                errors.append("active v2.2 supersession record schema differs")
            if record.get("superseded_package") != expected_supersedes:
                errors.append("active v2.2 supersession record package binding differs")
            if (
                record.get("successor_package_id") != EXPECTED_PACKAGE_ID
                or record.get("successor_plan_version")
                != EXPECTED_PLAN_VERSION
            ):
                errors.append("active v2.2 supersession record successor differs")
            if not isinstance(record.get("reason"), str) or not record["reason"].strip():
                errors.append("active v2.2 supersession reason is missing")
            if not isinstance(record.get("execution_effect"), str) or not record[
                "execution_effect"
            ].strip():
                errors.append("active v2.2 supersession execution effect is missing")
    frozen_errors = _validate_archived_checkpoint_with_frozen_v22_checker(
        root
    )
    hard_errors, deferred_errors = (
        partition_archived_changed_artifact_errors(frozen_errors)
    )
    errors.extend(hard_errors)
    if deferred_changed_artifact_errors is not None:
        deferred_changed_artifact_errors.extend(deferred_errors)
    return errors, archived


def _normalized_dynamic_contracts(
    manifest: dict[str, Any],
    *,
    package_from: Path,
    package_to: Path,
    ...
    for value in manifest.get("dynamic_node_contracts", []):
        if not isinstance(value, dict):
            continue
        item = json.loads(json.dumps(value))
        template = item.get("template_path")
        if isinstance(template, str):
            item["template_path"] = template.replace(
                package_from.as_posix(),
                package_to.as_posix(),
                1,
            )
...
def validate_manifest(
    ...
    manifest_path = resolve_safe_repo_file(root, MANIFEST_RELATIVE.as_posix())
    if manifest_path is None:
        return ["Goal graph v2.3 manifest is missing or unsafe"]
    manifest_hash = sha256_file(manifest_path)
    if EXPECTED_MANIFEST_SHA256.startswith("__FINALIZE_"):
        errors.append("Goal graph v2.3 manifest checker trust anchor is not finalized")
    elif manifest_hash != EXPECTED_MANIFEST_SHA256:
        errors.append("Goal graph v2.3 manifest differs from checker trust anchor")
    if (
        checkpoint_state.get("static_plan_manifest_path")
        != MANIFEST_RELATIVE.as_posix()
        or checkpoint_state.get("static_plan_manifest_sha256") != manifest_hash
        or checkpoint_state.get("static_plan_version") != EXPECTED_PLAN_VERSION
    ):
        errors.append("Goal graph v2.3 manifest checkpoint binding differs")
    if (
        manifest.get("schema_version") != "2.1"
        or manifest.get("package_id") != EXPECTED_PACKAGE_ID
        or manifest.get("plan_version") != EXPECTED_PLAN_VERSION
    ):
        errors.append("Goal graph v2.3 manifest identity differs")
    supersedes = manifest.get("supersedes")
    if (
        not isinstance(supersedes, dict)
        or supersedes.get("package_id") != EXPECTED_V22_PACKAGE_ID
        or supersedes.get("plan_version") != EXPECTED_V22_PLAN_VERSION
        or supersedes.get("activation_status") != "ACTIVE"
        or supersedes.get("manifest_path")
        != V22_MANIFEST_RELATIVE.as_posix()
        or supersedes.get("manifest_sha256")
        != EXPECTED_V22_MANIFEST_SHA256
        or supersedes.get("archived_checkpoint_path")
        != V22_ARCHIVED_CHECKPOINT_RELATIVE.as_posix()
        or supersedes.get("archived_checkpoint_raw_sha256")
        != EXPECTED_V22_ARCHIVED_CHECKPOINT_RAW_SHA256
        or supersedes.get("transition_event_count")
        != EXPECTED_V22_TRANSITION_EVENT_COUNT
        or supersedes.get("transition_history_anchor_sha256")
        != EXPECTED_V22_TRANSITION_HISTORY_ANCHOR_SHA256
        or supersedes.get("supersession_record_path")
        != V22_SUPERSESSION_RECORD_RELATIVE.as_posix()
    ):
        errors.append("active v2.2 supersedes binding differs")

    native_paths = native_package_paths(root)
    imported_paths = {
        record.get("path")
        for record in imported_goal_binding_map(checkpoint_state).values()
        if isinstance(record.get("path"), str)
    }
    dynamic_inventory = checkpoint_state.get("dynamic_goal_inventory")
    if not isinstance(dynamic_inventory, dict):
        dynamic_inventory = {}
    successor_dynamic_paths = {
        record.get("path")
        for goal_id, record in dynamic_inventory.items()
        if (
            isinstance(goal_id, str)
            and isinstance(record, dict)
            and isinstance(record.get("path"), str)
            and record.get("path") not in imported_paths
        )
    }
    invalid_successor_paths = {
        path
        for path in successor_dynamic_paths
        if (
            not path.startswith(
                f"{PACKAGE_RELATIVE.as_posix()}/work-items/"
            )
            or not path.endswith(".md")
        )
    }
    if invalid_successor_paths:
        errors.append(
            "Goal graph v2.3 successor dynamic path is invalid: "
            f"{sorted(invalid_successor_paths)}"
        )
    expected_native_paths = EXPECTED_V23_NATIVE_PATHS | successor_dynamic_paths
    if set(native_paths) != expected_native_paths:
        errors.append("Goal graph v2.3 native/support dynamic path set differs")
    protected = manifest.get("protected_files")
    protected_map: dict[str, str] = {}
    if not isinstance(protected, list):
        errors.append("Goal graph v2.3 protected_files must be a list")
    else:
        for index, record in enumerate(protected):
            if not isinstance(record, dict) or set(record) != {"path", "sha256"}:
                errors.append(f"Goal graph v2.3 protected file {index} is malformed")
                continue
            path_value = record.get("path")
            digest = record.get("sha256")
            if (
                not isinstance(path_value, str)
                or path_value in protected_map
                or not isinstance(digest, str)
            ):
                errors.append(
                    f"Goal graph v2.3 protected file {index} is invalid or duplicated"
                )
                continue
            protected_map[path_value] = digest
            path = resolve_safe_repo_file(root, path_value)
            if path is None or sha256_file(path) != digest:
                errors.append(f"Goal graph v2.3 protected file changed: {path_value}")
    if set(protected_map) != EXPECTED_V23_PROTECTED_PATHS:
        errors.append("Goal graph v2.3 protected native path set differs")
    if len(discovered_paths) != (
        EXPECTED_MANAGED_GOAL_PATH_COUNT + len(successor_dynamic_paths)
    ):
        errors.append("Goal graph v2.3 mixed managed path count differs")

    predecessor_path = resolve_safe_repo_file(
        root,
        V22_MANIFEST_RELATIVE.as_posix(),
    )
    predecessor: dict[str, Any] = {}
    ...
    for field in (
        ...
        "artifact_work_scheduling_contract",
        "dynamic_inventory_contract",
    ):
        if manifest.get(field) != predecessor.get(field):
            errors.append(f"Goal graph v2.3 inherited contract differs: {field}")
    policy_contract = manifest.get("policy_gap_contract")
    if policy_contract != predecessor.get("policy_gap_contract"):
        errors.append("Goal graph v2.3 inherited contract differs: policy_gap_contract")
    if (
        not isinstance(policy_contract, dict)
        or policy_contract.get("bootstrap_consumed_policy_gap_pairs")
        != EXPECTED_BOOTSTRAP_CONSUMED_POLICY_GAP_PAIRS
    ):
        errors.append("policy/Gap bootstrap consumed pair contract differs")
    previous_graph = predecessor.get("goal_graph")
    current_graph = manifest.get("goal_graph")
    if not isinstance(current_graph, dict) or not isinstance(previous_graph, dict):
        errors.append("Goal graph v2.3 graph contract is missing")
    else:
        if current_graph.get("static_nodes") != previous_graph.get("static_nodes"):
            errors.append("Goal graph v2.3 imported static topology differs")
        if current_graph.get("initial_focus_goal_id") != EXPECTED_INITIAL_FOCUS_GOAL_ID:
            errors.append("Goal graph v2.3 initial focus differs")
        if current_graph.get("initial_ready_frontier_goal_ids") != [
            "WS-GOAL-EPIC-02-FP-005-R001",
            "WS-GOAL-EPIC-03",
            "WS-GOAL-EPIC-12",
        ]:
            errors.append("Goal graph v2.3 initial ready frontier differs")
    if _normalized_dynamic_contracts(
        manifest,
        package_from=PACKAGE_RELATIVE,
        package_to=V22_PACKAGE_RELATIVE,
    ) != _normalized_dynamic_contracts(
        predecessor,
        package_from=V22_PACKAGE_RELATIVE,
        package_to=V22_PACKAGE_RELATIVE,
    ):
        errors.append("Goal graph v2.3 dynamic node contracts differ")

    import_contract = manifest.get("imported_predecessor_goal_contract")
    expected_import_contract = {
        "source_package_id": EXPECTED_V22_PACKAGE_ID,
        "source_plan_version": EXPECTED_V22_PLAN_VERSION,
        "source_checkpoint_path": V22_ARCHIVED_CHECKPOINT_RELATIVE.as_posix(),
        "runtime_binding_field": "imported_predecessor_goal_bindings",
        "expected_goal_count": EXPECTED_IMPORTED_GOAL_COUNT,
    ...
    if import_contract != expected_import_contract:
        errors.append("Goal graph v2.3 imported predecessor contract differs")

    transition = manifest.get("transition_contract")
    if not isinstance(transition, dict):
        errors.append("Goal graph v2.3 transition contract is missing")
    else:
        if transition.get("check_command_contract_version") != (
            CHECK_COMMAND_CONTRACT_VERSION
        ):
            errors.append("Goal graph v2.3 check command version differs")
        if transition.get("quick_activation_check_ids") != list(
            QUICK_ACTIVATION_CHECK_IDS
        ):
            errors.append("Goal graph v2.3 quick check IDs differ")
        if transition.get(
            "quick_activation_check_contract_sha256"
        ) != QUICK_ACTIVATION_CHECK_CONTRACT_SHA256:
            errors.append("Goal graph v2.3 quick check contract differs")
        if transition.get("implementation_start_gate_check_ids") != list(
            IMPLEMENTATION_START_GATE_CHECK_IDS
        ):
            errors.append("Goal graph v2.3 full gate IDs differ")
        if transition.get(
            "implementation_start_gate_check_contract_sha256"
        ) != IMPLEMENTATION_START_GATE_CHECK_CONTRACT_SHA256:
            errors.append("Goal graph v2.3 full gate contract differs")
        if transition.get("predecessor_runtime_import_event") != "PACKAGE_PREPARED":
            errors.append("Goal graph v2.3 predecessor import event differs")
        if transition.get("predecessor_runtime_import_is_projection_only") is not True:
            errors.append("Goal graph v2.3 predecessor import boundary differs")
    return errors
...
def validate_dynamic_goal_inventory(
    ...
    """Validate v2.2 materialization seals through the archived v2.2 chain."""
    errors: list[str] = []
    archive_errors, archived = _load_archived_v22_checkpoint(root)
    errors.extend(archive_errors)
    if not archived:
    ...
    archived_state = archived.get("goal_execution")
    if not isinstance(archived_state, dict):
        return errors + ["archived v2.2 dynamic inventory is unavailable"]
    archived_inventory = archived_state.get("dynamic_goal_inventory")
    inventory = state.get("dynamic_goal_inventory")
    if not isinstance(archived_inventory, dict):
        return errors + ["archived v2.2 dynamic Goal inventory is malformed"]
    if not isinstance(inventory, dict):
        return errors + ["v2.3 dynamic Goal inventory is malformed"]
    if not set(archived_inventory).issubset(inventory):
        errors.append("v2.3 lost an imported v2.2 dynamic Goal")
        return errors
    for goal_id, archived_record in archived_inventory.items():
        if inventory.get(goal_id) != archived_record:
            errors.append(
                f"{goal_id}: imported v2.2 dynamic Goal lineage changed"
            )
    history = archived_state.get("transition_history")
    archived_events_by_hash = {
        event.get("event_sha256"): event
        for event in history
        if isinstance(event, dict) and isinstance(event.get("event_sha256"), str)
    } if isinstance(history, list) else {}
    current_history = state.get("transition_history")
    current_events_by_hash = {
        event.get("event_sha256"): event
        for event in current_history
        if isinstance(event, dict) and isinstance(event.get("event_sha256"), str)
    } if isinstance(current_history, list) else {}
    for goal_id, record in sorted(inventory.items()):
        node = nodes.get(goal_id)
        if not isinstance(record, dict) or not isinstance(node, dict):
            errors.append(f"{goal_id}: imported dynamic Goal is malformed")
            continue
        path = paths_by_id.get(goal_id)
        resolved = resolve_safe_repo_file(root, path)
        if resolved is None or record.get("sha256") != sha256_file(resolved):
            errors.append(f"{goal_id}: imported dynamic Goal content differs")
        event = (
            archived_events_by_hash.get(record.get("materialized_event_sha256"))
            if goal_id in archived_inventory
            else current_events_by_hash.get(record.get("materialized_event_sha256"))
        )
        if not isinstance(event, dict) or event.get("event_type") not in {
            ...
            errors.append(f"{goal_id}: archived materialization lineage is missing")
            continue
        if goal_id in archived_inventory:
            continue
        expected = {
        ...
        for field, value in expected.items():
            if record.get(field) != value:
                errors.append(
                    f"{goal_id}: successor dynamic inventory differs: {field}"
                )
        event_bindings = {
            "artifact_trigger_evidence_refs": node.get(
                "artifact_trigger_evidence_refs"
            ...
            "materialized_goal_id": goal_id,
            "materialized_goal_path": path,
            "materialized_goal_content_sha256": sha256_file(resolved),
            "materialized_from_role": node.get("materialized_from_role"),
            "materialized_from_path": node.get("materialized_from_path"),
            "materialized_from_document_id": node.get(
                "materialized_from_document_id"
        ...
        for field, value in event_bindings.items():
            if event.get(field) != value:
                errors.append(
                    f"{goal_id}: successor materialization event differs: {field}"
                )
    return errors


def imported_predecessor_completion_seed(
    root: Path,
    state: dict[str, Any],
) -> tuple[list[str], dict[str, dict[str, Any]]]:
    """Load imported completion events/bindings from the frozen v2.2 chain."""
    errors, archived = _load_archived_v22_checkpoint(root)
    if not archived:
        return errors, {}
    archived_state = archived.get("goal_execution")
    if not isinstance(archived_state, dict):
        return errors + ["archived v2.2 completion seed is unavailable"], {}
    history = archived_state.get("transition_history")
    if not isinstance(history, list):
        return errors + ["archived v2.2 completion history is malformed"], {}
    events_by_hash = {
        event.get("event_sha256"): event
        for event in history
        if isinstance(event, dict) and isinstance(event.get("event_sha256"), str)
    ...
    imported_activation_bindings = activation_event.get(
        "imported_completion_evidence_bindings_by_goal"
    )
    if not isinstance(imported_activation_bindings, dict):
        imported_activation_bindings = {}
    seed: dict[str, dict[str, Any]] = {}
    for goal_id, record in imported_goal_binding_map(state).items():
        completion_hash = record.get("completion_event_sha256")
        if not isinstance(completion_hash, str):
            continue
        event = events_by_hash.get(completion_hash)
        if not isinstance(event, dict):
            errors.append(
                f"{goal_id}: imported completion event is absent from v2.2"
            )
            continue
        event_bindings = event.get("completion_evidence_bindings")
        if not isinstance(event_bindings, dict):
            event_bindings = imported_activation_bindings.get(goal_id)
        if not isinstance(event_bindings, dict):
            errors.append(
                f"{goal_id}: imported completion evidence bindings are absent"
            )
            event_bindings = {}
        seed[goal_id] = {
            "event": completion_event_with_package_generation(
                event,
                V22_PACKAGE_GENERATION,
            ),
...
def combined_completion_lineage(
    root: Path,
    state: dict[str, Any],
) -> tuple[
    list[str],
    ...
    errors, imported_seed = imported_predecessor_completion_seed(
        root,
        state,
    )
    events: dict[str, dict[str, Any]] = {}
    bindings: dict[str, dict[str, dict[str, Any]]] = {}
    for goal_id, record in imported_seed.items():
        event = record.get("event")
        event_bindings = record.get("bindings")
        if isinstance(event, dict):
            events[goal_id] = event
        if isinstance(event_bindings, dict):
            bindings[goal_id] = {
                role: binding
                for role, binding in event_bindings.items()
                if isinstance(role, str) and isinstance(binding, dict)
            }
    history = state.get("transition_history")
    if not isinstance(history, list):
        return errors + ["current completion history is malformed"], events, bindings
    for event in history:
        if (
        ...
        if not isinstance(goal_id, str):
            continue
        events[goal_id] = completion_event_with_package_generation(
            event,
            V23_PACKAGE_GENERATION,
        )
        bindings[goal_id] = (
    ...
    return errors, events, bindings


def current_goal_nodes(
    root: Path,
    state: dict[str, Any],
) -> tuple[list[str], dict[str, dict[str, Any]]]:
    errors: list[str] = []
    ...
    for relative in sorted(
        value for value in relative_paths if isinstance(value, str)
    ):
        path = resolve_safe_repo_file(root, relative)
        if path is None:
            errors.append(f"current Goal path is missing or unsafe: {relative}")
            continue
        try:
            node, _ = parse_goal(path)
        except (OSError, ValueError, tomllib.TOMLDecodeError) as exc:
            errors.append(f"current Goal cannot be parsed: {relative}: {exc}")
            continue
...
def completed_cross_package_successor_reaches_live(
    ...
    *,
    ...
    if not isinstance(state, dict):
        return False
    node_errors, nodes = current_goal_nodes(root, state)
    lineage_errors, events, bindings = combined_completion_lineage(
        root,
        state,
    )
    if node_errors or lineage_errors:
        return False
    return completed_successor_artifact_chain_reaches_live(
        root,
        goal_id=goal_id,
        relative_path=relative_path,
        historical_sha256=historical_sha256,
        nodes=nodes,
        completion_bindings_by_goal=bindings,
        completion_event_by_goal=events,
    )


def archived_completion_binding_snapshots(
    archived: dict[str, Any],
) -> dict[str, dict[str, dict[str, Any]]]:
    state = archived.get("goal_execution")
    ...
    for event in history:
        ...
        if isinstance(goal_id, str):
            result[goal_id] = {
                role: dict(binding)
                for role, binding in current_snapshot.items()
            }
    return result


def validate_deferred_archived_implementation_changes(
    root: Path,
    *,
    checkpoint: dict[str, Any],
    nodes: dict[str, dict[str, Any]],
    ...
    if not isinstance(state, dict) or not isinstance(archived_state, dict):
        return list(deferred_errors)
    statuses = state.get("status_by_goal")
    archived_statuses = archived_state.get("status_by_goal")
    if not isinstance(statuses, dict) or not isinstance(
        archived_statuses,
        dict,
    ):
        return list(deferred_errors)
    ...
    for error in deferred_errors:
        ...
        if match is None:
            return list(deferred_errors)
        grouped.setdefault(match.group("goal_id"), []).append(error)

    lineage_errors, completion_events, completion_bindings = (
        combined_completion_lineage(root, state)
    )
    snapshots = archived_completion_binding_snapshots(archived)
    result: list[str] = []
    for goal_id, goal_errors in grouped.items():
        relevant_completed_successors = [
            candidate_goal_id
            for candidate_goal_id, node in nodes.items()
            if (
                candidate_goal_id != goal_id
                and node.get("work_item_type")
                in CANONICAL_PRODUCER_WORK_TYPES
                and goal_transitively_depends_on(
                    nodes,
                ...
                and (
                    completion_event_order(
                        completion_events.get(candidate_goal_id, {})
                    )
                    is not None
                    and completion_event_order(
                        completion_events.get(candidate_goal_id, {})
                    )[0]
                    == V23_PACKAGE_GENERATION
                    or (
                        statuses.get(candidate_goal_id)
                        == "COMPLETE_AT_TARGET"
                        and archived_statuses.get(candidate_goal_id)
                        != "COMPLETE_AT_TARGET"
                    )
                )
            )
        ]
        if not relevant_completed_successors:
            continue
        snapshot = snapshots.get(goal_id)
        node = nodes.get(goal_id)
        if lineage_errors or not isinstance(snapshot, dict) or not isinstance(
            node,
            dict,
        ):
            result.extend(goal_errors)
            result.extend(
                f"archived v2.2 deferred replay: {error}"
                for error in lineage_errors
            )
            continue
        revalidation_errors = validate_internal_producer_result_evidence(
            root,
            goal_id=goal_id,
            node=node,
        ...
        expected_inner_errors = {
            core_error
            for error in goal_errors
            if (
                core_error := archived_changed_artifact_core_error(
                    error
                )
            )
            is not None
        }
        remaining_changed_errors = {
            error
            for error in revalidation_errors
            if error in expected_inner_errors
        }
        other_errors = [
            error
            for error in revalidation_errors
            if error not in expected_inner_errors
        ]
        if other_errors:
            result.extend(goal_errors)
            result.extend(
            ...
            result.extend(
                error
                for error in goal_errors
                if archived_changed_artifact_core_error(error)
                in remaining_changed_errors
            )
    return list(dict.fromkeys(result))


_validate_transition_history_v22_semantics = validate_transition_history
...
def validate_transition_history(
    ...
    """Validate the new chain plus its exact active-v2.2 import seed."""
    errors: list[str] = []
    archive_errors, archived = _load_archived_v22_checkpoint(root)
    errors.extend(archive_errors)
    history = state.get("transition_history")
    if not isinstance(history, list) or not history or not isinstance(
        ...
        return errors + ["Goal graph v2.3 transition history is missing"]
    errors.extend(validate_committed_v23_history_prefix(history))
    first = history[0]
    if first.get("event_type") != "PACKAGE_PREPARED":
        errors.append("Goal graph v2.3 first event must be PACKAGE_PREPARED")
    if first.get("previous_event_sha256") != "":
        errors.append("Goal graph v2.3 starts with a non-empty previous hash")
    if (
        first.get("supersedes_event_sha256")
        != EXPECTED_V22_TRANSITION_HISTORY_ANCHOR_SHA256
    ):
        errors.append("Goal graph v2.3 first event does not bind the v2.2 tail")

    if archived:
        archived_state = archived.get("goal_execution")
        if not isinstance(archived_state, dict):
            archived_state = {}
        expected_imported = imported_goal_binding_map(state)
        expected_import = {
            "source_package_id": EXPECTED_V22_PACKAGE_ID,
            "source_plan_version": EXPECTED_V22_PLAN_VERSION,
            ...
            "imported_predecessor_goal_bindings_sha256": (
                canonical_json_sha256(expected_imported)
            ),
            "canonical_binding_snapshot_sha256": canonical_json_sha256(
                canonical_binding_snapshot(
                    canonical_binding_map(archived),
                )
            ),
        }
        if first.get("active_predecessor_import") != expected_import:
            errors.append("Goal graph v2.3 active predecessor import differs")
        if first.get("status_changes") != archived_state.get("status_by_goal"):
            errors.append("Goal graph v2.3 imported status snapshot differs")
        if first.get("canonical_binding_snapshot_after") != (
            canonical_binding_snapshot(canonical_binding_map(archived))
        ):
            errors.append("Goal graph v2.3 imported canonical snapshot differs")
        if len(history) == 1:
            for field in (
                "blockers_by_goal",
                ...
                if state.get(field) != archived_state.get(field):
                    errors.append(
                        f"Goal graph v2.3 imported runtime seed differs: {field}"
                    )
    replay_errors = _validate_transition_history_v22_semantics(
        root,
        checkpoint,
        state,
        nodes,
    ...
    imported_completed_ids = {
        goal_id
        for goal_id, record in imported_goal_binding_map(state).items()
        if record.get("status") == "COMPLETE_AT_TARGET"
    }
    for error in replay_errors:
        match = REPLAYED_IMPORTED_CHANGED_ARTIFACT_ERROR_PATTERN.fullmatch(
            error
        )
        if (
            match is not None
            and match.group("goal_id") in imported_completed_ids
        ):
            continue
        errors.append(error)
    return errors
...
def validate(
    ...
    *,
    ...
    deferred_archive_changed_artifact_errors: list[str] = []
    predecessor_errors, archived_v22 = validate_superseded_v2_2(
        root,
        manifest,
        deferred_changed_artifact_errors=(
            deferred_archive_changed_artifact_errors
        ),
    )
    errors.extend(predecessor_errors)
    if archived_v22:
        errors.extend(
            validate_imported_predecessor_goal_bindings(
                root,
                state,
                archived_v22,
            )
    ...
    imported_bindings = imported_goal_binding_map(state)
    runtime_inventory = state.get("dynamic_goal_inventory")
    if not isinstance(runtime_inventory, dict):
        runtime_inventory = {}
    goal_paths = sorted(
        {
            record["path"]
            for record in imported_bindings.values()
            if isinstance(record.get("path"), str)
        }
        | {
            record["path"]
    ...
    if archived_v22:
        errors.extend(
            validate_deferred_archived_implementation_changes(
                root,
                checkpoint=checkpoint,
                nodes=nodes,
                deferred_errors=(
