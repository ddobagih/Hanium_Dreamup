#!/usr/bin/env python3
"""Run the post-seq96 FP-048 R002 R009 five-check private start gate."""
...
_BASE_PATH = ROOT / "scripts/run_walksafe_fp048_r002_goal_start_gate_r008_20260826.py"
_BASE_SPEC = importlib.util.spec_from_file_location(
    "_walksafe_fp048_r002_private_gate_runtime_r009_20260826",
    _BASE_PATH,
)
if _BASE_SPEC is None or _BASE_SPEC.loader is None:
    raise RuntimeError("FP048 R002 R008 gate runtime cannot be loaded privately")
base = importlib.util.module_from_spec(_BASE_SPEC)
sys.modules[_BASE_SPEC.name] = base
_BASE_SPEC.loader.exec_module(base)

_PRIVATE_R008_SYNC_RUNTIME_PATHS = base._sync_runtime_paths
_PRIVATE_R007_SYNC_RUNTIME_PATHS = base.base._sync_runtime_paths
_PRIVATE_R006_SYNC_RUNTIME_PATHS = base.base.base._sync_runtime_paths
_PRIVATE_R005_SYNC_RUNTIME_PATHS = base.base.base.base._sync_runtime_paths
_PRIVATE_R004_SYNC_RUNTIME_PATHS = base.base.base.base.base._sync_runtime_paths
_PRIVATE_R003_SYNC_RUNTIME_PATHS = base.base.base.base.base.base._sync_runtime_paths
_PRIVATE_R002_SYNC_RUNTIME_PATHS = base.base.base.base.base.base.base._sync_runtime_paths
...
POST_SEQ96_REGRESSION_TEST_RELATIVE = Path(
...
R008_GATE_RUNTIME_RELATIVE = Path(
    "scripts/run_walksafe_fp048_r002_goal_start_gate_r008_20260826.py"
)
R008_GATE_RUNTIME_TEST_RELATIVE = Path(
...
RESERVED_POST_NAMESPACE_FAILURE_EVENT_ID = base.RESERVED_POST_NAMESPACE_FAILURE_EVENT_ID
...
CONTRACT_DOCUMENT_ID = "WS-FP048-R002-INITIAL-START-GATE-CONTRACT-20260826-009"
...
def _correction() -> Any:
    try:
        ...
        raise GateError(
            "FP048 R002 seq97 snapshot-hygiene correction authority is unavailable"
        ) from exc


def expected_r009_binding() -> dict[str, Any]:
    return {
        "schema_version": "1.2",
...
expected_r008_preflight_attempt_004_binding = expected_preflight_attempt_004_binding


def _checkpoint_bound_gate_event_directories(
    checkpoint_bytes: bytes,
) -> tuple[tuple[Path, ...], tuple[Path, ...]]:
    """Collect evidence paths while ignoring only four exact nonauthority receipts."""
    ...
    def require_nonauthority_receipt_metadata(value: Mapping[str, object]) -> None:
        ...
        if (
            not any(_strict_json_equal(value, expected) for expected in exact_nonauthority)
            ...
            raise GateError("checkpoint nonauthority preflight receipt metadata differs")
        ...
        if (
            ...
            raise GateError("checkpoint nonauthority preflight receipt path differs")
...
def _sync_runtime_paths() -> tuple[None, Any]:
    _unused, prior_authority = _PRIVATE_R008_SYNC_RUNTIME_PATHS()
    ...
    if not all(callable(getattr(correction, name, None)) for name in required):
        raise GateError("FP048 R002 seq97 snapshot-hygiene correction API differs")
    ...
    if (
        ...
        raise GateError("FP048 R002 seq97 snapshot-hygiene correction identity differs")
    source_paths = tuple(
        dict.fromkeys(
            (
                *prior_paths,
                ...
                goal_path,
                CONTRACT_RELATIVE,
                POST_SEQ96_REGRESSION_TEST_RELATIVE,
                CORRECTION_RELATIVE,
                ...
                R008_GATE_RUNTIME_RELATIVE,
                R008_GATE_RUNTIME_TEST_RELATIVE,
            )
        )
    )
    for name, value in {
...
def _document_id(event_id: str) -> str:
    ...
    if match is None or event_id != STARTED_EVENT_ID:
        raise GateError(
            f"event ID must equal the unused post-seq96 start ID {STARTED_EVENT_ID}"
        )
    return (
...
def bind_contract_corrected_source(
    ...
    *,
    ...
    try:
        ...
        raise GateError(f"seq97 snapshot-hygiene correction rejected source: {exc}") from exc
    ...
    if not isinstance(history, list) or len(history) != SOURCE_SEQUENCE:
        raise GateError("gate source is not exact seq97")
    ...
    if (
        ...
        raise GateError("seq97 snapshot-hygiene correction differs")
    binding = correction.r009_contract_binding(root)
    ...
    preflight_attempt = (
        source_binding.get("r008_preflight_attempt_004")
    ...
    if (
        ...
        raise GateError("seq97 R009 contract or historical preflight binding differs")
    ...
    if require_gate_namespace_absent and (
        ...
        raise GateError("R008 preflight-only namespace unexpectedly exists")
    ...
    if (
        ...
        raise GateError("seq97 READY runtime differs")
    goal_path = _repo_path(correction.GOAL_PATH, "correction GOAL_PATH")
    ...
    authority = ContractCorrectedAuthority(
        ...
        _sha(event.get("event_sha256"), "seq97 correction event SHA-256"),
        _impl._parse_timestamp(event.get("occurred_at"), label="seq97 occurred_at"),
...
def bind_published_contract_corrected_source(
    ...
    """Bind seq97 after the exact R009 gate namespace has been published."""
...
def expected_contract_binding() -> dict[str, Any]:
    if _active_authority is None:
        raise GateError("seq97 R009 contract authority has not been loaded")
...
def _load_gate_contract(
    ...
    *,
    ...
    _, correction = _sync_runtime_paths()
    authority_binding = correction.r009_contract_binding(root)
    expected_binding = expected_r009_binding()
    if authority_binding != expected_binding:
        raise GateError("FP048 R002 R009 frozen binding differs")
    effective = dict(binding or expected_contract_binding())
    if effective != authority_binding:
        raise GateError("FP048 R002 R009 reviewed binding differs")
    ...
    if len(raw) != CONTRACT_BYTE_LENGTH or _impl.sha256_bytes(raw) != CONTRACT_FILE_SHA256:
        raise GateError("FP048 R002 R009 contract bytes differ")
    try:
        contract = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise GateError("FP048 R002 R009 contract is not valid JSON") from exc
    if (
        ...
        raise GateError("FP048 R002 R009 canonical contract differs")
    raw_checks = contract.get("ordered_checks")
    if not isinstance(raw_checks, list):
        raise GateError("FP048 R002 R009 ordered checks are missing")
    ...
    if checks != expected_checks:
        raise GateError("FP048 R002 R009 five-check commands differ")
...
def _install_base_adapter() -> None:
    ...
    _sync_runtime_paths()
    if (
        base._sync_runtime_paths is not _PRIVATE_R008_SYNC_RUNTIME_PATHS
        or base.base._sync_runtime_paths is not _PRIVATE_R007_SYNC_RUNTIME_PATHS
        or base.base.base._sync_runtime_paths is not _PRIVATE_R006_SYNC_RUNTIME_PATHS
        or base.base.base.base._sync_runtime_paths is not _PRIVATE_R005_SYNC_RUNTIME_PATHS
        or base.base.base.base.base._sync_runtime_paths is not _PRIVATE_R004_SYNC_RUNTIME_PATHS
        or base.base.base.base.base.base._sync_runtime_paths is not _PRIVATE_R003_SYNC_RUNTIME_PATHS
        or base.base.base.base.base.base.base._sync_runtime_paths
        is not _PRIVATE_R002_SYNC_RUNTIME_PATHS
    ):
        raise GateError("private R008/R007/R006/R005/R004/R003/R002 sync authority differs")
    adapters = (
        base,
        base.base,
        base.base.base,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.preflight:
            ...
            print(
                "FP048-R002 post-seq96 initial-start gate preflight: PASS: "
                f"{len(results)} checks; no gate namespace created"
            )
            return 0
        receipt = run_gate(args.root, args.event_id)
        print(f"FP048-R002 post-seq96 initial-start gate: PASS: {receipt}")
        return 0
    except GateCheckFailed as exc:
        print(
            f"FP048-R002 post-seq96 initial-start gate: FAIL: {exc}",
            file=sys.stderr,
        )
        return exc.exit_code if exc.exit_code > 0 else 1
    except GatePostCommitUncertain as exc:
        print(
            "FP048-R002 post-seq96 initial-start gate: "
        ...
        print(
            f"FP048-R002 post-seq96 initial-start gate: ERROR: {exc}",
            file=sys.stderr,
        )
        return 2
