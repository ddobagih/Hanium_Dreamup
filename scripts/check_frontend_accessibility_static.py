...
def main() -> int:
    ...
    navigation_hook = ROOT / "apps/web/app/_walksafe/hooks/useNavigationGuidance.ts"
    navigation_api = ROOT / "apps/web/lib/navigation-api.ts"
    navigation_types = ROOT / "apps/web/types/navigation.ts"
    ...
    navigation_hook_text = navigation_hook.read_text(encoding="utf-8")
    navigation_api_text = navigation_api.read_text(encoding="utf-8")
    navigation_types_text = navigation_types.read_text(encoding="utf-8")
    camera_text = camera.read_text(encoding="utf-8")
    css_text = css.read_text(encoding="utf-8")
    pwa_hook_text = pwa_hook.read_text(encoding="utf-8")
    manifest_text = manifest.read_text(encoding="utf-8")
    sw_text = sw.read_text(encoding="utf-8")
    navigation_runtime_text = "\n".join(
        [page_text, assist_text, navigation_hook_text, navigation_api_text, navigation_types_text]
    )
    ...
    checks = [
        ...
        (
            'export type WalkingRouteProvider = "tmap_pedestrian";' in navigation_types_text
            and 'payload.provider === "tmap_pedestrian"' in navigation_api_text
            and 'WALKING_ROUTE_PROVIDER_LABEL = "TMAP"' in navigation_api_text
            and "kakao" not in navigation_runtime_text.lower()
            and "navigationPriorityNotice" not in navigation_runtime_text,
            "walking navigation runtime must enforce the TMAP-only provider contract",
        ),
