#!/usr/bin/env python3
"""Run the post-seq93 FP-048 R002 R006 five-check private start gate."""
...
_BASE_PATH = ROOT / "scripts/run_walksafe_fp048_r002_goal_start_gate_r005_20260826.py"
_BASE_SPEC = importlib.util.spec_from_file_location(
    "_walksafe_fp048_r002_private_gate_runtime_r006_20260826",
    _BASE_PATH,
)
if _BASE_SPEC is None or _BASE_SPEC.loader is None:
    raise RuntimeError("FP048 R002 R005 gate runtime cannot be loaded privately")
base = importlib.util.module_from_spec(_BASE_SPEC)
sys.modules[_BASE_SPEC.name] = base
_BASE_SPEC.loader.exec_module(base)

_PRIVATE_R005_SYNC_RUNTIME_PATHS = base._sync_runtime_paths
_PRIVATE_R004_SYNC_RUNTIME_PATHS = base.base._sync_runtime_paths
_PRIVATE_R003_SYNC_RUNTIME_PATHS = base.base.base._sync_runtime_paths
_PRIVATE_R002_SYNC_RUNTIME_PATHS = base.base.base.base._sync_runtime_paths
...
POST_SEQ93_REGRESSION_TEST_RELATIVE = Path(
...
R005_GATE_RUNTIME_RELATIVE = Path(
    "scripts/run_walksafe_fp048_r002_goal_start_gate_r005_20260826.py"
)
R005_GATE_RUNTIME_TEST_RELATIVE = Path(
...
# The R005 preview failed before creating a namespace.  The closest seq82-83
# authority treats that condition as non-consuming, so the unused ID remains
# the only authorized identity for the R006 gate and seq95 transition.
STARTED_EVENT_ID = (
    "WS-GOAL-GRAPH-V2-4-GOAL-STARTED-FP048-R002-20260826-004"
)
PREFLIGHT_ONLY_EVENT_ID = STARTED_EVENT_ID
PREFLIGHT_FAILURE_REASON_CODE = "R005_PREVIEW_FAILED_NO_NAMESPACE_NONAUTHORITY"
...
CONTRACT_DOCUMENT_ID = "WS-FP048-R002-INITIAL-START-GATE-CONTRACT-20260826-006"
CONTRACT_ID = "WS-FP048-R002-INTERNAL-START-GATE-R006"
CONTRACT_VERSION = "2026-08-26.5"
...
def _correction() -> Any:
    try:
        ...
        raise GateError(
            "FP048 R002 seq94 contract-correction authority is unavailable"
        ) from exc
...
def expected_r006_binding() -> dict[str, Any]:
    return {
        "schema_version": "1.2",
...
def _sync_runtime_paths() -> tuple[None, Any]:
    _unused, prior_authority = _PRIVATE_R005_SYNC_RUNTIME_PATHS()
    ...
    if not all(callable(getattr(correction, name, None)) for name in required):
        raise GateError("FP048 R002 seq94 contract-correction API differs")
    parent_goal_id = getattr(
        correction, "PARENT_GOAL_ID", prior_authority.PARENT_GOAL_ID
    )
    predecessor_goal_id = getattr(
        correction, "PREDECESSOR_GOAL_ID", prior_authority.PREDECESSOR_GOAL_ID
    )
    ready_event_id = getattr(
        correction, "READY_EVENT_ID", prior_authority.READY_EVENT_ID
    )
    ready_frontier = tuple(
        getattr(correction, "READY_FRONTIER", prior_authority.READY_FRONTIER)
    )
    ...
    if (
        ...
        or ready_event_id != prior_authority.READY_EVENT_ID
        or ready_frontier != tuple(prior_authority.READY_FRONTIER)
        or correction.MANIFEST_SHA256 != MANIFEST_SHA256
        or correction.SOURCE_SEQUENCE != 93
        ...
        raise GateError("FP048 R002 seq94 contract-correction identity differs")
    source_paths = tuple(
        dict.fromkeys(
            (
                *prior_paths,
                ...
                goal_path,
                CONTRACT_RELATIVE,
                POST_SEQ93_REGRESSION_TEST_RELATIVE,
                CORRECTION_RELATIVE,
                ...
                R005_GATE_RUNTIME_RELATIVE,
                R005_GATE_RUNTIME_TEST_RELATIVE,
            )
        )
    )
    for name, value in {
...
def _document_id(event_id: str) -> str:
    ...
    if match is None or event_id != STARTED_EVENT_ID:
        raise GateError(
            f"event ID must equal the unused post-seq93 start ID {STARTED_EVENT_ID}"
        )
    return (
...
def bind_contract_corrected_source(
    ...
    *,
    ...
    try:
        ...
        raise GateError(f"seq94 contract correction rejected source: {exc}") from exc
    ...
    if not isinstance(history, list) or len(history) != SOURCE_SEQUENCE:
        raise GateError("gate source is not exact seq94")
    ready = history[SOURCE_READY_SEQUENCE - 1]
    event = history[-1]
    ready_event_id = getattr(
        correction, "READY_EVENT_ID", base._reanchor().READY_EVENT_ID
    )
    if (
        ...
        raise GateError("seq94 contract correction differs")
    binding = correction.r006_contract_binding(root)
    ...
    if (
        ...
        or preflight_attempt != expected_preflight_attempt_004_binding()
    ):
        raise GateError("seq94 R006 contract or preflight binding differs")
    ...
    if require_gate_namespace_absent and (
        ...
        raise GateError("R005 preflight-only namespace unexpectedly exists")
    ...
    if (
        ...
        raise GateError("seq94 READY runtime differs")
    goal_path = _repo_path(correction.GOAL_PATH, "correction GOAL_PATH")
    ...
    authority = ContractCorrectedAuthority(
        ...
        _sha(event.get("event_sha256"), "seq94 correction event SHA-256"),
        _impl._parse_timestamp(event.get("occurred_at"), label="seq94 occurred_at"),
...
def bind_published_contract_corrected_source(
    ...
    """Bind seq94 after the exact R006 gate namespace has been published."""
...
def expected_contract_binding() -> dict[str, Any]:
    if _active_authority is None:
        raise GateError("seq94 R006 contract authority has not been loaded")
...
def _load_gate_contract(
    ...
    *,
    ...
    _, correction = _sync_runtime_paths()
    authority_contract, authority_binding = correction.load_r006_contract(root)
    expected_binding = expected_r006_binding()
    if authority_binding != expected_binding:
        raise GateError("FP048 R002 R006 frozen binding differs")
    effective = dict(binding or expected_contract_binding())
    if effective != authority_binding:
        raise GateError("FP048 R002 R006 reviewed binding differs")
    ...
    if len(raw) != CONTRACT_BYTE_LENGTH or _impl.sha256_bytes(raw) != CONTRACT_FILE_SHA256:
        raise GateError("FP048 R002 R006 contract bytes differ")
    try:
        contract = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise GateError("FP048 R002 R006 contract is not valid JSON") from exc
    if (
        ...
        raise GateError("FP048 R002 R006 canonical contract differs")
    raw_checks = contract.get("ordered_checks")
    if not isinstance(raw_checks, list):
        raise GateError("FP048 R002 R006 ordered checks are missing")
    ...
    if checks != expected_checks:
        raise GateError("FP048 R002 R006 five-check commands differ")
...
def _install_base_adapter() -> None:
    ...
    _sync_runtime_paths()
    if (
        base._sync_runtime_paths is not _PRIVATE_R005_SYNC_RUNTIME_PATHS
        or base.base._sync_runtime_paths is not _PRIVATE_R004_SYNC_RUNTIME_PATHS
        or base.base.base._sync_runtime_paths is not _PRIVATE_R003_SYNC_RUNTIME_PATHS
    ):
        raise GateError("private R005/R004/R003 sync authority differs")
    base._install_base_adapter = _install_base_adapter
    base.base._install_base_adapter = _install_base_adapter
    base.base.base._install_base_adapter = _install_base_adapter
    runtime = base.base.base.base
    ...
    for adapter in (base, base.base, base.base.base, runtime):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.preflight:
            ...
            print(
                "FP048-R002 post-seq93 initial-start gate preflight: PASS: "
                f"{len(results)} checks; no gate namespace created"
            )
            return 0
        receipt = run_gate(args.root, args.event_id)
        print(f"FP048-R002 post-seq93 initial-start gate: PASS: {receipt}")
        return 0
    except GateCheckFailed as exc:
        print(f"FP048-R002 post-seq93 initial-start gate: FAIL: {exc}", file=sys.stderr)
        return exc.exit_code if exc.exit_code > 0 else 1
    except GatePostCommitUncertain as exc:
        print(
            f"FP048-R002 post-seq93 initial-start gate: POSTCOMMIT-UNCERTAIN: {exc}",
            file=sys.stderr,
        )
        return 2
    except (GateError, OSError, TypeError, ValueError) as exc:
        print(f"FP048-R002 post-seq93 initial-start gate: ERROR: {exc}", file=sys.stderr)
        return 2
