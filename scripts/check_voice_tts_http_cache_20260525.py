...
def post_tts(url_parts, text: str, timeout: float) -> tuple[int, dict[str, str], int]:
...
def main() -> int:
    ...
    for text in texts:
        try:
            first_status, first_headers, first_bytes = post_tts(parsed, text, args.timeout)
            second_status, second_headers, second_bytes = post_tts(parsed, text, args.timeout)
