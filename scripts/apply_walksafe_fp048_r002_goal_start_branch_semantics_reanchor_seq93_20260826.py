#!/usr/bin/env python3
"""Prepare the reviewed zero-credit FP048 R002 seq93 branch-semantics reanchor."""

from __future__ import annotations

import argparse
import copy
from dataclasses import dataclass
from datetime import datetime, timedelta
import hashlib
import json
import os
from pathlib import Path
import stat
import sys
from typing import Any, Callable, Mapping, Sequence


ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from scripts import (
    apply_walksafe_fp048_r002_goal_start_control_reanchor_seq90_20260826 as seq90,
)
from scripts import (
    apply_walksafe_fp048_r002_start_gate_contract_correction_seq92_20260826
    as seq92,
)
from scripts import check_walksafe_project_continuation_v2_4 as continuation


class BranchSemanticsReanchorError(seq90.ControlReanchorError):
    """The seq93 branch-semantics reanchor cannot be proven or published."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise BranchSemanticsReanchorError(message)


CHECKPOINT_REL = seq92.CHECKPOINT_REL
checkpoint_json_bytes = seq92.checkpoint_json_bytes
GOAL_ID = seq92.GOAL_ID
GOAL_PATH = seq92.GOAL_PATH
GOAL_SHA256 = seq92.GOAL_SHA256
WORK_ITEM_ID = seq92.WORK_ITEM_ID
MANIFEST_SHA256 = seq92.MANIFEST_SHA256
EVENT_FIELDS = seq92.EVENT_FIELDS
CLAIM_BOUNDARY = copy.deepcopy(seq92.CLAIM_BOUNDARY)
PARENT_GOAL_ID = seq90.PARENT_GOAL_ID
PREDECESSOR_GOAL_ID = seq90.PREDECESSOR_GOAL_ID
READY_EVENT_ID = seq90.READY_EVENT_ID
READY_EVENT_SHA256 = seq90.SOURCE_TAIL_SHA256
SOURCE_READY_EVENT_ID = READY_EVENT_ID
SOURCE_READY_EVENT_SHA256 = READY_EVENT_SHA256
READY_FRONTIER = seq90.READY_FRONTIER
READY_FRONTIER_GOAL_IDS = READY_FRONTIER

SOURCE_SEQUENCE = 92
SOURCE_EVENT_ID = seq92.CORRECTION_EVENT_ID
SOURCE_EVENT_SHA256 = (
    "16e7f959d4c36d4d41f77ede9cedc625b42031dbcbf5616e5d715e3b15937c82"
)
SOURCE_CHECKPOINT_SHA256 = (
    "a39f3bafd8ccce0b047bed9e4e5a13db521ad786e709dd3bf6be1e5e31c80b43"
)
SOURCE_CHECKPOINT_BYTE_LENGTH = 3_123_587

FROZEN_SEQ92_R004_REVIEW_BINDINGS = {
    "assignment": {
        "path": seq92.REVIEW_ASSIGNMENT_REL.as_posix(),
        "sha256": "5d03d30d14798fd3770e9adfd5482cb3fb592a06f82a0fa69e9fc2439129437d",
        "byte_length": 10_879,
    },
    "review_result": {
        "path": seq92.REVIEW_RESULT_REL.as_posix(),
        "sha256": "40620c7466221b152cb01028e7852808bf3e792e768e534dda099a0789631c89",
        "byte_length": 3_398,
    },
    "independent_review": {
        "path": seq92.INDEPENDENT_REVIEW_REL.as_posix(),
        "sha256": "c4e599472ef974fe01fa80e56e35f77ecf7dd5d53ec5b084af51dae6f74e41c6",
        "byte_length": 4_196,
    },
}
FROZEN_SEQ92_R004_REVIEWED_INPUTS: tuple[tuple[str, str, int], ...] = (
    (
        "scripts/apply_walksafe_fp048_r002_start_gate_contract_correction_seq92_20260826.py",
        "57a047a2f6cfe8f2d7ab26b561929e1814563e8ad69e57162d0903fd29252c0f",
        59_286,
    ),
    (
        "tests/test_apply_walksafe_fp048_r002_start_gate_contract_correction_seq92_20260826.py",
        "aec5080a3b6751a7f697dc98a13fc5d375f717627d113f9907472620a185efc4",
        27_050,
    ),
    (
        "scripts/apply_walksafe_fp048_r002_goal_start_control_correction_seq91_20260826.py",
        "e1459a64ac87a7c895269d138bc995bd2da1798253199cd21b883fa3fbe12123",
        83_844,
    ),
    (
        "tests/test_apply_walksafe_fp048_r002_goal_start_control_correction_seq91_20260826.py",
        "2a48432a7c7477d10c119e77bf44746953d30363966f8ab0ead1c4e63d8f7ab2",
        49_342,
    ),
    (
        "scripts/apply_walksafe_fp048_r002_goal_start_control_reanchor_seq90_20260826.py",
        "f5d8ac24d05b3a0e98ece7d5d977219c37859d44d5540ba991749cbffdd63ac2",
        105_892,
    ),
    (
        "tests/test_apply_walksafe_fp048_r002_goal_start_control_reanchor_seq90_20260826.py",
        "a6d12e95e169b563f4d1642460da36c27cc43884836931ea65f81276c4fa71ad",
        26_198,
    ),
    (
        "docs/control/execution/goal-contracts/WS-GOAL-EPIC-03-FP-048-R002/initial-start-gate-contract-r003.json",
        "bce459dc4454b11638bc7d09320ba90671aea02c711a7b502db13e52d7b45e75",
        3_882,
    ),
    (
        "docs/control/execution/goal-contracts/WS-GOAL-EPIC-03-FP-048-R002/initial-start-gate-contract-r004.json",
        "f0b4dfaa18bed37a4b33b6b3520bb19261268cdc57244de23bf0815a10062485",
        3_494,
    ),
    (
        "docs/control/execution/workstream-transitions/seq91-92/review-rounds/R009/review-assignment.json",
        "e7aa0379db6d013bc754c350617f08d14a2c4eeea7700cd477c78636d58eedca",
        12_974,
    ),
    (
        "docs/control/execution/workstream-transitions/seq91-92/review-rounds/R009/review-result.json",
        "c7c59129b3ba8a9944cbe7cdb06264f9f3cc229a3502c0f34d87334f54c077de",
        2_840,
    ),
    (
        "docs/control/execution/workstream-transitions/seq91-92/review-rounds/R009/independent-review.json",
        "56a0907f30bb9156d1d732627a0f3def3d5421520c12226d897ad420836dc781",
        4_681,
    ),
    (
        "docs/control/execution/workstream-transitions/seq92-93/authorization.json",
        "79164f500327150d7fc90bdf44bf635080cf2c6762806ee8452eb09ddb30f37f",
        4_659,
    ),
    (
        "scripts/check_walksafe_project_continuation_v2_4.py",
        "9f4fc4e073f22b84f70404a48112e37a56b2d819e2a0ddde95e56a88eae2e729",
        518_765,
    ),
    (
        "tests/test_walksafe_project_continuation_v2_4.py",
        "ece87b4322ed7e473c7d91d8b39f4dec778383e3f499e32b3d2547bda427e056",
        258_268,
    ),
    (
        "scripts/check_walksafe_goal_graph_v2_4.py",
        "72c6ed6c73f1d6dc47d7f85736f9f621f9274c87850d17ce175ab02c037768a2",
        800_978,
    ),
    (
        "tests/test_walksafe_goal_graph_v2_4.py",
        "53e3175fd07723bfa8115af2c9770d784452d1269878465b77c59f0e88883cc0",
        514_962,
    ),
    (
        "scripts/run_walksafe_test_layers_current.sh",
        "4ed94795f1a3a27c6d7a46d6015bb34e87028e044d16d7e3dfeb0f17e1f6b1ba",
        26_855,
    ),
    (
        "scripts/run_walksafe_fp048_r002_goal_start_gate_r004_20260826.py",
        "81ec186dc04cc8bc34079e64b29a4b06de0e423676a206524e47a3ba99771c2a",
        22_576,
    ),
    (
        "tests/test_walksafe_fp048_r002_goal_start_gate_r004_20260826.py",
        "48e628146718b7aa2650b5924c0b5f46f02bbc356e52af791f0a61066366c66a",
        24_700,
    ),
    (
        "scripts/apply_walksafe_fp048_r002_goal_started_seq93_20260826.py",
        "65bdb09c80e869dc25d86649c1afbbeadaae42bfa1fdfefab8da37bd9bf2d121",
        76_574,
    ),
    (
        "tests/test_apply_walksafe_fp048_r002_goal_started_seq93_20260826.py",
        "5f78b94d3af89363b1cdc6a590792a9fb92102b0a40049da67b101bf42b28646",
        53_800,
    ),
)

REANCHOR_SEQUENCE = 93
REANCHOR_EVENT_ID = (
    "WS-GOAL-GRAPH-V2-4-GOAL-START-CONTROL-REANCHORED-"
    "FP048-R002-BRANCH-SEMANTICS-20260826-001"
)
REANCHOR_EVENT_TYPE = "GOAL_START_CONTROL_REANCHORED"
CONTROL_REANCHOR_SEQUENCE = REANCHOR_SEQUENCE
CONTROL_REANCHOR_EVENT_ID = REANCHOR_EVENT_ID
STARTED_SEQUENCE = 94
STARTED_EVENT_ID = (
    "WS-GOAL-GRAPH-V2-4-GOAL-STARTED-FP048-R002-20260826-004"
)

LOGICAL_BRANCH = "current"
LOGICAL_BRANCH_SEMANTICS = "WORKSTREAM_LABEL"
PHYSICAL_GIT_BRANCH = "recovery/fp046-r008-wip-20260822"
BRANCH_MISMATCH_REASON_CODE = (
    "LOGICAL_CURRENT_LABEL_IS_NOT_PHYSICAL_GIT_BRANCH"
)

UNCONSUMED_GATE_EVENT_ID = seq92.STARTED_EVENT_ID
UNCONSUMED_GATE_DIR_REL = Path(
    "docs/control/execution/goal-gates"
) / UNCONSUMED_GATE_EVENT_ID
UNCONSUMED_GATE_RECEIPT_REL = (
    UNCONSUMED_GATE_DIR_REL / "implementation-start-gate-receipt.json"
)
UNCONSUMED_GATE_FILE_PINS: tuple[tuple[str, str, int], ...] = (
    (
        "01-CONTINUATION.log",
        "25b5c8cf65854e11a04a9d0810dd2503585037683809c153f578c08a076ac5a7",
        39,
    ),
    (
        "02-GOAL_GRAPH.log",
        "f880c51e169f2a6f03875ab90156c15b9c5ce260ed0761b0048556b08d089516",
        108,
    ),
    (
        "03-TEST_LAYER_REGISTRY_VALIDATE.log",
        "1eef448a998bf080cbe648b330b4c9369d49ea95bbccf662ce8d077b56c96187",
        35,
    ),
    (
        "04-ROOT_FP048_R002_CONTROL_REGRESSION.log",
        "12237e5928da4c62d811f987e36aa3435fe9616a8e7fd391babdb10b16ad099c",
        191,
    ),
    (
        "05-REPOSITORY_STATE.log",
        "1394e16a2868fc90ac2f8faaff9739a05f9cbccc16b095b68f5940b761efcc97",
        183_482,
    ),
    (
        "implementation-start-gate-receipt.json",
        "9462d509176f44df8b06a47434f14a77cfe980d5107b53203a91773ba2a3db98",
        6_489,
    ),
)

R004_CONTRACT_REL = seq92.R004_CONTRACT_REL
R005_CONTRACT_REL = Path(
    "docs/control/execution/goal-contracts/WS-GOAL-EPIC-03-FP-048-R002/"
    "initial-start-gate-contract-r005.json"
)
R005_RUNNER_REL = Path(
    "scripts/run_walksafe_fp048_r002_goal_start_gate_r005_20260826.py"
)
R005_RUNNER_TEST_REL = Path(
    "tests/test_walksafe_fp048_r002_goal_start_gate_r005_20260826.py"
)
SEQ94_STARTER_REL = Path(
    "scripts/apply_walksafe_fp048_r002_goal_started_seq94_20260826.py"
)
SEQ94_STARTER_TEST_REL = Path(
    "tests/test_apply_walksafe_fp048_r002_goal_started_seq94_20260826.py"
)
R005_CONTRACT_ID = "WS-FP048-R002-INTERNAL-START-GATE-R005"
R005_CONTRACT_VERSION = "2026-08-26.4"
R005_DOCUMENT_ID = "WS-FP048-R002-INITIAL-START-GATE-CONTRACT-20260826-005"
R005_FILE_SHA256 = (
    "ee38d33edecb44cfd4f0fd4cebc1190db79809c8ad81c60a8d9567792eb27506"
)
R005_CANONICAL_SHA256 = (
    "d611973d65c69229b081190f47cdcad8b61976be28281222cd7b27c4411a631a"
)
R005_BYTE_LENGTH = 3_514
R005_SUCCESSOR_REASON_CODE = (
    "SEQ93_BRANCH_SEMANTICS_REANCHOR_AND_FRESH_GATE_REQUIRED"
)

TRANSITION_ROOT = Path(
    "docs/control/execution/workstream-transitions/seq93-94"
)
AUTHORIZATION_REL = TRANSITION_ROOT / "authorization.json"
R001_REVIEW_ROOT = TRANSITION_ROOT / "review-rounds/R001"
R001_REVIEW_ASSIGNMENT_REL = R001_REVIEW_ROOT / "review-assignment.json"
R001_REVIEW_RESULT_REL = R001_REVIEW_ROOT / "review-result.json"
R001_INDEPENDENT_REVIEW_REL = R001_REVIEW_ROOT / "independent-review.json"
R001_REVIEW_ASSIGNMENT_SHA256 = (
    "4f4a020730137a6ab747bf6f015d76c60fab2c706bb870db79542312a4685f5e"
)
R001_REVIEW_ASSIGNMENT_BYTE_LENGTH = 11_704
R001_STALE_REVIEW_FINDING_COUNTS = {"P0": 0, "P1": 3, "P2": 1}
R001_STALE_REVIEW_REASON_CODES = (
    "P1_PHYSICAL_GIT_BRANCH_NOT_BOUND_AT_PREPARE",
    "P1_R005_NONCANONICAL_SEQ93_SOURCE_CAN_BURN_EVENT_004",
    "P1_CONTINUATION_LEGACY_R004_FALLBACK_ACCEPTS_UNCONSUMED_PASS",
    "P2_SEQ93_EVENT_CHRONOLOGY_NOT_REVALIDATED",
)
R002_REVIEW_ROOT = TRANSITION_ROOT / "review-rounds/R002"
R002_REVIEW_ASSIGNMENT_REL = R002_REVIEW_ROOT / "review-assignment.json"
R002_REVIEW_RESULT_REL = R002_REVIEW_ROOT / "review-result.json"
R002_INDEPENDENT_REVIEW_REL = R002_REVIEW_ROOT / "independent-review.json"
R002_REVIEW_ASSIGNMENT_SHA256 = (
    "de6449fda52464eb37194acf8ab768b64e2738f9bb655e39c1a6edc691e18d05"
)
R002_REVIEW_ASSIGNMENT_BYTE_LENGTH = 12_352
R002_STALE_REVIEW_FINDING_COUNTS = {"P0": 1, "P1": 0, "P2": 0}
R002_STALE_REVIEW_REASON_CODES = (
    "P0_R005_ADAPTER_RECURSION_PREVENTS_GATE_EXECUTION",
)
ROUND_ID = "R003"
REVIEW_ROOT = TRANSITION_ROOT / f"review-rounds/{ROUND_ID}"
REVIEW_ASSIGNMENT_REL = REVIEW_ROOT / "review-assignment.json"
REVIEW_RESULT_REL = REVIEW_ROOT / "review-result.json"
INDEPENDENT_REVIEW_REL = REVIEW_ROOT / "independent-review.json"
REVIEW_PATHS = (
    REVIEW_ASSIGNMENT_REL,
    REVIEW_RESULT_REL,
    INDEPENDENT_REVIEW_REL,
)

SCRIPT_REL = Path(
    "scripts/apply_walksafe_fp048_r002_goal_start_branch_semantics_reanchor_seq93_20260826.py"
)
TEST_REL = Path(
    "tests/test_apply_walksafe_fp048_r002_goal_start_branch_semantics_reanchor_seq93_20260826.py"
)
CONTINUATION_SCRIPT_REL = Path("scripts/check_walksafe_project_continuation_v2_4.py")
CONTINUATION_TEST_REL = Path("tests/test_walksafe_project_continuation_v2_4.py")
GOAL_GRAPH_SCRIPT_REL = Path("scripts/check_walksafe_goal_graph_v2_4.py")
GOAL_GRAPH_TEST_REL = Path("tests/test_walksafe_goal_graph_v2_4.py")
LAYER_RUNNER_REL = Path("scripts/run_walksafe_test_layers_current.sh")
REVIEWED_CONTROL_PATHS = (
    SCRIPT_REL,
    TEST_REL,
    seq92.SCRIPT_REL,
    seq92.TEST_REL,
    R005_CONTRACT_REL,
    AUTHORIZATION_REL,
    CONTINUATION_SCRIPT_REL,
    CONTINUATION_TEST_REL,
    GOAL_GRAPH_SCRIPT_REL,
    GOAL_GRAPH_TEST_REL,
    LAYER_RUNNER_REL,
    R005_RUNNER_REL,
    R005_RUNNER_TEST_REL,
    SEQ94_STARTER_REL,
    SEQ94_STARTER_TEST_REL,
)

CORRECTION_REASON = {
    "branch_mismatch_reason_code": BRANCH_MISMATCH_REASON_CODE,
    "logical_branch": LOGICAL_BRANCH,
    "logical_branch_semantics": LOGICAL_BRANCH_SEMANTICS,
    "observed_gate_event_id": UNCONSUMED_GATE_EVENT_ID,
    "observed_physical_git_branch": PHYSICAL_GIT_BRANCH,
    "reason_code": "R004_PASS_BRANCH_SEMANTICS_AMBIGUOUS",
    "remediation": (
        "SEAL_R004_PASS_AS_UNCONSUMED_AND_RUN_BRANCH_AWARE_R005_GATE"
    ),
}
PROJECTED_TRANSITION = {
    "reanchor_event_id": REANCHOR_EVENT_ID,
    "reanchor_sequence": REANCHOR_SEQUENCE,
    "reanchor_transition": "READY_TO_READY",
    "started_event_id": STARTED_EVENT_ID,
    "started_sequence": STARTED_SEQUENCE,
    "started_transition": "READY_TO_IN_PROGRESS_AFTER_FRESH_R005_PASS",
}
REVIEWER = {
    "id": "codex-fp048-r002-seq93-branch-semantics-independent-reviewer-20260826",
    "task_id": "/root/seq93_branch_semantics_independent_review_r003",
}
INDEPENDENT_CHECKS = [
    "EXACT_PUBLISHED_SEQ92_SOURCE_CAS_PASS",
    "R004_PASS_SIX_FILE_PRIVATE_EVIDENCE_EXACT_PASS",
    "R004_PASS_SEALED_AS_PASS_UNCONSUMED_PASS",
    "LOGICAL_AND_PHYSICAL_BRANCH_SEMANTICS_SEPARATED_PASS",
    "R004_TO_R005_CONTRACT_SUPERSESSION_PASS",
    "SEQ93_READY_TO_READY_ZERO_CREDIT_PASS",
    "SEQ93_TO_SEQ92_EXACT_BYTE_RECONSTRUCTION_PASS",
    "FINAL_MANAGED_AND_GIT_VISIBLE_COHORT_RESEAL_PASS",
    "PRIVATE_0600_CHECKPOINT_MODE_PRECOMMIT_AND_POSTCOMMIT_PASS",
    "PROJECTED_CONTINUATION_AND_GOAL_GRAPH_PASS",
]
REVIEW_RESULT_FIELDS = frozenset(
    {
        "assignment_binding",
        "claim_boundary",
        "correction_reason",
        "decision",
        "document_id",
        "external_independence_claimed",
        "findings",
        "goal_id",
        "passed_gate_attempt_003_binding",
        "previous_contract_binding",
        "projected_transition",
        "replacement_contract_binding",
        "reviewed_at",
        "reviewer",
        "round_id",
        "schema_version",
    }
)
INDEPENDENT_REVIEW_FIELDS = REVIEW_RESULT_FIELDS | {
    "independent_checks",
    "review_result_binding",
}

SCOPE = (
    "Graph v2.4 FP-048 R002 remains READY through zero-credit seq93 branch-"
    "semantics reanchor; R004 PASS is sealed PASS_UNCONSUMED, while fresh R005 "
    "gate and seq94 start remain NOT_RUN."
)
CURRENT_FOCUS = (
    "FP-048 R002/GAP-057 READY; R004 PASS sealed as PASS_UNCONSUMED; "
    "R005 gate armed; seq94 start NOT_RUN"
)
NEXT_ACTION = (
    "R005 event-scoped five-check gate를 -004 event ID로 실행하고 PASS receipt로 "
    "seq94 start를 검증한다."
)
HANDOFF_EPIC = (
    "EPIC-03 / FP-048 R002/GAP-057 READY_R004_PASS_UNCONSUMED_R005_GATE_NOT_RUN"
)
VERIFICATION_STATUS = (
    "FP048_R002_SEQ93_ZERO_CREDIT_BRANCH_SEMANTICS_REANCHORED_R005_GATE_NOT_RUN"
)


def sha256_bytes(raw: bytes) -> str:
    return hashlib.sha256(raw).hexdigest()


def _strict_json_equal(left: Any, right: Any) -> bool:
    return seq90.canonical_json_bytes({"value": left}) == seq90.canonical_json_bytes(
        {"value": right}
    )


def _binding(relative: Path, raw: bytes) -> dict[str, Any]:
    return {
        "path": relative.as_posix(),
        "sha256": sha256_bytes(raw),
        "byte_length": len(raw),
    }


def frozen_seq92_r004_review_binding(
    root: Path = ROOT,
    source_event: Mapping[str, Any] | None = None,
) -> dict[str, dict[str, Any]]:
    """Validate the immutable R004 review, without replaying mutable inputs."""

    root = seq90._safe_root(root)
    document_by_role: dict[str, dict[str, Any]] = {}
    observed: dict[str, dict[str, Any]] = {}
    for role, relative in (
        ("assignment", seq92.REVIEW_ASSIGNMENT_REL),
        ("review_result", seq92.REVIEW_RESULT_REL),
        ("independent_review", seq92.INDEPENDENT_REVIEW_REL),
    ):
        raw = seq90._stable_read(root, relative).raw
        expected = FROZEN_SEQ92_R004_REVIEW_BINDINGS[role]
        require(
            _binding(relative, raw) == expected,
            f"frozen seq92 R004 {role} bytes differ",
        )
        document = seq90.strict_json(raw, relative.as_posix())
        require(
            raw == seq90.canonical_json_bytes(document),
            f"frozen seq92 R004 {role} is noncanonical",
        )
        document_by_role[role] = document
        observed[role] = copy.deepcopy(expected)

    assignment = document_by_role["assignment"]
    result = document_by_role["review_result"]
    independent = document_by_role["independent_review"]
    historical_inputs = [
        {"path": path, "sha256": digest, "byte_length": length}
        for path, digest, length in FROZEN_SEQ92_R004_REVIEWED_INPUTS
    ]
    require(
        len(historical_inputs) == 21
        and assignment.get("reviewed_control_inputs") == historical_inputs
        and assignment.get("document_id")
        == "WS-FP048-R002-SEQ92-93-REVIEW-ASSIGNMENT-20260826-R004"
        and assignment.get("round_id") == "R004"
        and assignment.get("goal_id") == GOAL_ID
        and assignment.get("required_reviewer") == seq92.REVIEWER
        and result.get("document_id")
        == "WS-FP048-R002-SEQ92-93-REVIEW-RESULT-20260826-R004"
        and result.get("assignment_binding") == observed["assignment"]
        and result.get("decision")
        == "APPROVE_INTERNAL_ZERO_CREDIT_CONTROL_ONLY"
        and result.get("findings") == []
        and independent.get("document_id")
        == "WS-FP048-R002-SEQ92-93-INDEPENDENT-REVIEW-20260826-R004"
        and independent.get("assignment_binding") == observed["assignment"]
        and independent.get("review_result_binding") == observed["review_result"]
        and independent.get("decision")
        == "CONCUR_INTERNAL_ZERO_CREDIT_CONTROL_ONLY"
        and independent.get("findings") == []
        and independent.get("independent_checks") == seq92.INDEPENDENT_CHECKS,
        "frozen seq92 R004 review authority differs",
    )
    if source_event is not None:
        require(
            source_event.get("transition_control_review_binding") == observed,
            "seq92 source frozen R004 review binding differs",
        )
    return copy.deepcopy(observed)


def _stored_passed_gate_attempt_003_binding() -> dict[str, Any]:
    return {
        "event_id": UNCONSUMED_GATE_EVENT_ID,
        "directory": UNCONSUMED_GATE_DIR_REL.as_posix(),
        "directory_mode": "0700",
        "status": "PASS_UNCONSUMED",
        "files": [
            {
                "path": (UNCONSUMED_GATE_DIR_REL / name).as_posix(),
                "sha256": digest,
                "byte_length": length,
            }
            for name, digest, length in UNCONSUMED_GATE_FILE_PINS
        ],
    }


def _source_checkpoint_binding(
    gate_binding: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    return {
        "path": CHECKPOINT_REL.as_posix(),
        "sha256": SOURCE_CHECKPOINT_SHA256,
        "byte_length": SOURCE_CHECKPOINT_BYTE_LENGTH,
        "sequence": SOURCE_SEQUENCE,
        "tail_event_id": SOURCE_EVENT_ID,
        "tail_event_sha256": SOURCE_EVENT_SHA256,
        "passed_gate_attempt_003": copy.deepcopy(
            dict(gate_binding or _stored_passed_gate_attempt_003_binding())
        ),
    }


def _parse_time(value: str, label: str) -> datetime:
    try:
        parsed = datetime.fromisoformat(value)
    except (TypeError, ValueError) as exc:
        raise BranchSemanticsReanchorError(f"{label} differs") from exc
    require(parsed.tzinfo is not None and parsed.microsecond == 0, f"{label} differs")
    return parsed


def _gate_receipt_and_binding(
    root: Path,
) -> tuple[dict[str, Any], dict[str, Any]]:
    root = seq90._safe_root(root)
    directory = root / UNCONSUMED_GATE_DIR_REL
    info = directory.lstat()
    require(
        stat.S_ISDIR(info.st_mode)
        and not stat.S_ISLNK(info.st_mode)
        and stat.S_IMODE(info.st_mode) == 0o700,
        "R004 PASS gate evidence directory mode differs",
    )
    expected_names = {name for name, _digest, _length in UNCONSUMED_GATE_FILE_PINS}
    require(
        {child.name for child in directory.iterdir()} == expected_names,
        "R004 PASS gate evidence inventory differs",
    )
    for name, expected_digest, expected_length in UNCONSUMED_GATE_FILE_PINS:
        relative = UNCONSUMED_GATE_DIR_REL / name
        read = seq90._stable_read(root, relative)
        mode = stat.S_IMODE((root / relative).lstat().st_mode)
        require(
            mode == 0o600
            and len(read.raw) == expected_length
            and sha256_bytes(read.raw) == expected_digest,
            f"R004 PASS gate evidence differs: {name}",
        )
    receipt_raw = seq90._stable_read(root, UNCONSUMED_GATE_RECEIPT_REL).raw
    receipt = seq90.strict_json(receipt_raw, UNCONSUMED_GATE_RECEIPT_REL.as_posix())
    snapshot = receipt.get("repository_snapshot")
    runs = receipt.get("check_runs")
    require(
        receipt.get("status") == "PASS"
        and receipt.get("target_transition_event_id") == UNCONSUMED_GATE_EVENT_ID
        and receipt.get("target_goal_id") == GOAL_ID
        and receipt.get("target_goal_content_sha256") == GOAL_SHA256
        and receipt.get("source_activation_event_sha256") == SOURCE_EVENT_SHA256
        and receipt.get("source_checkpoint_sha256") == SOURCE_CHECKPOINT_SHA256
        and receipt.get("implementation_start_gate_contract_binding")
        == seq92.r004_contract_binding(root)
        and isinstance(runs, list)
        and len(runs) == 5
        and all(
            isinstance(row, dict)
            and row.get("exit_code") == 0
            and row.get("output_path")
            == (UNCONSUMED_GATE_DIR_REL / UNCONSUMED_GATE_FILE_PINS[index][0]).as_posix()
            and row.get("output_sha256") == UNCONSUMED_GATE_FILE_PINS[index][1]
            for index, row in enumerate(runs)
        )
        and isinstance(snapshot, dict)
        and snapshot.get("branch") == PHYSICAL_GIT_BRANCH,
        "R004 PASS receipt authority differs",
    )
    return receipt, _stored_passed_gate_attempt_003_binding()


def passed_gate_attempt_003_binding(root: Path = ROOT) -> dict[str, Any]:
    return copy.deepcopy(_gate_receipt_and_binding(root)[1])


def r004_contract_binding(root: Path = ROOT) -> dict[str, Any]:
    return seq92.r004_contract_binding(root)


def load_r005_contract(
    root: Path = ROOT,
) -> tuple[dict[str, Any], dict[str, Any]]:
    root = seq90._safe_root(root)
    raw = seq90._stable_read(root, R005_CONTRACT_REL).raw
    require(
        len(raw) == R005_BYTE_LENGTH and sha256_bytes(raw) == R005_FILE_SHA256,
        "R005 contract bytes differ",
    )
    document = seq90.strict_json(raw, R005_CONTRACT_REL.as_posix())
    require(
        raw == seq90.canonical_json_bytes(document)
        and continuation.canonical_json_sha256(document) == R005_CANONICAL_SHA256,
        "R005 contract canonical authority differs",
    )
    checks = document.get("ordered_checks")
    supersedes = document.get("supersedes")
    previous = r004_contract_binding(root)
    require(
        document.get("schema_version") == "1.2"
        and document.get("document_id") == R005_DOCUMENT_ID
        and document.get("contract_id") == R005_CONTRACT_ID
        and document.get("contract_version") == R005_CONTRACT_VERSION
        and document.get("gate_purpose") == "INITIAL_START"
        and document.get("target_goal_id") == GOAL_ID
        and document.get("target_goal_content_sha256") == GOAL_SHA256
        and document.get("successor_reason_code") == R005_SUCCESSOR_REASON_CODE
        and isinstance(checks, list)
        and [row.get("check_id") for row in checks]
        == list(seq92.correction.R003_CONTRACT_CHECK_IDS)
        and all(
            isinstance(row, dict)
            and set(row) == {"check_id", "command"}
            and isinstance(row["command"], str)
            and row["command"]
            for row in checks
        )
        and isinstance(supersedes, dict)
        and supersedes.get("path") == previous["path"]
        and supersedes.get("file_sha256") == previous["file_sha256"]
        and supersedes.get("canonical_sha256")
        == previous["canonical_contract_sha256"]
        and supersedes.get("contract_id") == previous["contract_id"]
        and supersedes.get("contract_version") == previous["contract_version"]
        and supersedes.get("source_reanchor_event_id") == REANCHOR_EVENT_ID
        and supersedes.get("source_reanchor_event_sequence") == REANCHOR_SEQUENCE,
        "R005 contract semantics differ",
    )
    claim = document.get("claim_boundary")
    require(
        isinstance(claim, dict)
        and claim.get("goal_started") is False
        and claim.get("start_gate_status") == "NOT_RUN"
        and claim.get("release_status") == "NOT_ELIGIBLE"
        and all(
            type(value) is int and value == 0
            for key, value in claim.items()
            if key.endswith("_credit_delta")
        ),
        "R005 contract claim boundary differs",
    )
    binding = {
        "schema_version": "1.2",
        "document_id": R005_DOCUMENT_ID,
        "path": R005_CONTRACT_REL.as_posix(),
        "file_sha256": R005_FILE_SHA256,
        "contract_id": R005_CONTRACT_ID,
        "contract_version": R005_CONTRACT_VERSION,
        "canonical_contract_sha256": R005_CANONICAL_SHA256,
    }
    return document, binding


def r005_contract_binding(root: Path = ROOT) -> dict[str, Any]:
    return load_r005_contract(root)[1]


def r005_runner_binding(root: Path = ROOT) -> dict[str, Any]:
    root = seq90._safe_root(root)
    return _binding(R005_RUNNER_REL, seq90._stable_read(root, R005_RUNNER_REL).raw)


def require_exact_seq92_source(
    raw: bytes,
    source: Mapping[str, Any],
    root: Path = ROOT,
) -> None:
    root = seq90._safe_root(root)
    require(len(raw) == SOURCE_CHECKPOINT_BYTE_LENGTH, "seq92 source length differs")
    require(sha256_bytes(raw) == SOURCE_CHECKPOINT_SHA256, "seq92 source CAS differs")
    require(raw == checkpoint_json_bytes(source), "seq92 source is noncanonical")
    state = source.get("goal_execution")
    history = state.get("transition_history") if isinstance(state, dict) else None
    require(
        isinstance(history, list)
        and len(history) == SOURCE_SEQUENCE
        and history[-1].get("sequence") == SOURCE_SEQUENCE
        and history[-1].get("event_id") == SOURCE_EVENT_ID
        and history[-1].get("event_sha256") == SOURCE_EVENT_SHA256,
        "source is not exact seq92",
    )
    restored_seq91_raw = checkpoint_json_bytes(seq92._restored_seq91_checkpoint(source))
    require(
        len(restored_seq91_raw) == seq92.SOURCE_CHECKPOINT_BYTE_LENGTH
        and sha256_bytes(restored_seq91_raw) == seq92.SOURCE_CHECKPOINT_SHA256,
        "seq92 inverse source authority differs",
    )
    frozen_seq92_r004_review_binding(root, history[-1])
    require(
        history[-1].get("contract_supersession", {}).get(
            "replacement_contract_binding"
        )
        == r004_contract_binding(root)
        and state.get("goal_status") == "READY"
        and state.get("status_by_goal", {}).get(GOAL_ID) == "READY"
        and source.get("current_work", {}).get("status") == "READY"
        and source.get("approved_state", {}).get("formal_test_not_run_count") == 279
        and source.get("verification_boundary", {}).get("release_eligible") is False,
        "seq92 source authority or credit boundary differs",
    )


def load_exact_seq92_source(
    root: Path = ROOT,
) -> tuple[bytes, dict[str, Any]]:
    root = seq90._safe_root(root)
    read = seq90._stable_read(root, CHECKPOINT_REL)
    source = seq90.strict_json(read.raw, CHECKPOINT_REL.as_posix())
    require_exact_seq92_source(read.raw, source, root)
    return read.raw, source


def build_authorization(root: Path, source: Mapping[str, Any]) -> bytes:
    source_raw = checkpoint_json_bytes(source)
    require_exact_seq92_source(source_raw, source, root)
    value = {
        "authorization_scope": {
            "authorized_actions": [
                "SEQ93_ZERO_CREDIT_BRANCH_SEMANTICS_REANCHOR",
                "SEAL_R004_PASS_AS_PASS_UNCONSUMED",
                "R005_PRIVATE_INITIAL_START_GATE_AFTER_SEQ93",
                "SEQ94_GOAL_STARTED_AFTER_FRESH_R005_FIVE_CHECK_PASS",
            ],
            "conditions": [
                "EXACT_PUBLISHED_SEQ92_SOURCE_CAS_REQUIRED",
                "R004_PASS_SIX_FILE_PRIVATE_EVIDENCE_MUST_REMAIN_EXACT",
                "SEQ93_MUST_PRESERVE_READY_STATUS_AND_ZERO_CREDIT",
                "LOGICAL_WORKSTREAM_BRANCH_MUST_NOT_IMPERSONATE_PHYSICAL_GIT_BRANCH",
                "SEQ94_REQUIRES_A_FRESH_R005_FIVE_CHECK_PASS_RECEIPT",
            ],
            "excluded_actions": [
                "CONSUME_R004_PASS_FOR_GOAL_START",
                "FORMAL_OR_ACTUAL_DEVICE_TEST_CREDIT",
                "EXTERNAL_REVIEW_OR_DEPLOYMENT_CREDIT",
                "RELEASE_OR_COMPLETION_CREDIT",
            ],
        },
        "authorization_status": (
            "AUTHORIZED_FOR_CONDITIONAL_SEQ93_94_BRANCH_SEMANTICS_CONTINUATION"
        ),
        "claim_boundary": copy.deepcopy(CLAIM_BOUNDARY),
        "correction_reason": copy.deepcopy(CORRECTION_REASON),
        "document_id": "WS-FP048-R002-SEQ93-94-AUTHORIZATION-20260826-001",
        "evidence_type": "USER_CONTINUATION_AUTHORIZATION",
        "goal_id": GOAL_ID,
        "passed_gate_attempt_003_binding": passed_gate_attempt_003_binding(root),
        "previous_contract_binding": r004_contract_binding(root),
        "projected_transition": copy.deepcopy(PROJECTED_TRANSITION),
        "replacement_contract_binding": r005_contract_binding(root),
        "schema_version": "1.0",
        "source_checkpoint_binding": _source_checkpoint_binding(
            passed_gate_attempt_003_binding(root)
        ),
        "source_transition_control_review_binding": copy.deepcopy(
            source["goal_execution"]["transition_history"][-1][
                "transition_control_review_binding"
            ]
        ),
        "target_started_event_id": STARTED_EVENT_ID,
    }
    return seq90.canonical_json_bytes(value)


def authorization_binding(root: Path, source: Mapping[str, Any]) -> dict[str, Any]:
    root = seq90._safe_root(root)
    raw = seq90._stable_read(root, AUTHORIZATION_REL).raw
    require(raw == build_authorization(root, source), "seq93 authorization differs")
    return _binding(AUTHORIZATION_REL, raw)


def _r001_stale_assignment_binding(root: Path) -> dict[str, Any]:
    root = seq90._safe_root(root)
    raw = seq90._stable_read(root, R001_REVIEW_ASSIGNMENT_REL).raw
    require(
        len(raw) == R001_REVIEW_ASSIGNMENT_BYTE_LENGTH
        and sha256_bytes(raw) == R001_REVIEW_ASSIGNMENT_SHA256,
        "stale seq93 R001 review assignment bytes differ",
    )
    document = seq90.strict_json(raw, R001_REVIEW_ASSIGNMENT_REL.as_posix())
    require(
        raw == seq90.canonical_json_bytes(document)
        and document.get("schema_version") == "1.0"
        and document.get("document_id")
        == "WS-FP048-R002-SEQ93-94-REVIEW-ASSIGNMENT-20260826-R001"
        and document.get("round_id") == "R001"
        and document.get("goal_id") == GOAL_ID
        and document.get("required_reviewer", {}).get("task_id")
        == "/root/seq93_branch_semantics_independent_review_r001",
        "stale seq93 R001 review assignment authority differs",
    )
    require(
        not os.path.lexists(root / R001_REVIEW_RESULT_REL)
        and not os.path.lexists(root / R001_INDEPENDENT_REVIEW_REL),
        "stale seq93 R001 reviewer results must remain absent",
    )
    return {
        "path": R001_REVIEW_ASSIGNMENT_REL.as_posix(),
        "sha256": R001_REVIEW_ASSIGNMENT_SHA256,
        "byte_length": R001_REVIEW_ASSIGNMENT_BYTE_LENGTH,
        "round_id": "R001",
        "disposition": "STALE_REVIEW_BLOCKED",
        "finding_counts": copy.deepcopy(R001_STALE_REVIEW_FINDING_COUNTS),
        "reason_codes": list(R001_STALE_REVIEW_REASON_CODES),
    }


def _r002_stale_assignment_binding(root: Path) -> dict[str, Any]:
    root = seq90._safe_root(root)
    raw = seq90._stable_read(root, R002_REVIEW_ASSIGNMENT_REL).raw
    require(
        len(raw) == R002_REVIEW_ASSIGNMENT_BYTE_LENGTH
        and sha256_bytes(raw) == R002_REVIEW_ASSIGNMENT_SHA256,
        "stale seq93 R002 review assignment bytes differ",
    )
    document = seq90.strict_json(raw, R002_REVIEW_ASSIGNMENT_REL.as_posix())
    require(
        raw == seq90.canonical_json_bytes(document)
        and document.get("schema_version") == "1.0"
        and document.get("document_id")
        == "WS-FP048-R002-SEQ93-94-REVIEW-ASSIGNMENT-20260826-R002"
        and document.get("round_id") == "R002"
        and document.get("goal_id") == GOAL_ID
        and document.get("required_reviewer", {}).get("task_id")
        == "/root/seq93_branch_semantics_independent_review_r002"
        and document.get("supersedes") == _r001_stale_assignment_binding(root),
        "stale seq93 R002 review assignment authority differs",
    )
    require(
        not os.path.lexists(root / R002_REVIEW_RESULT_REL)
        and not os.path.lexists(root / R002_INDEPENDENT_REVIEW_REL),
        "stale seq93 R002 reviewer results must remain absent",
    )
    return {
        "path": R002_REVIEW_ASSIGNMENT_REL.as_posix(),
        "sha256": R002_REVIEW_ASSIGNMENT_SHA256,
        "byte_length": R002_REVIEW_ASSIGNMENT_BYTE_LENGTH,
        "round_id": "R002",
        "disposition": "STALE_REVIEW_BLOCKED",
        "finding_counts": copy.deepcopy(R002_STALE_REVIEW_FINDING_COUNTS),
        "reason_codes": list(R002_STALE_REVIEW_REASON_CODES),
        "supersedes": _r001_stale_assignment_binding(root),
    }


def build_review_assignment(root: Path, source: Mapping[str, Any]) -> bytes:
    reviewed = [
        _binding(path, seq90._stable_read(root, path).raw)
        for path in REVIEWED_CONTROL_PATHS
    ]
    value = {
        "assigner": {"id": "codex-root", "task_id": "/root"},
        "authorization_binding": authorization_binding(root, source),
        "claim_boundary": copy.deepcopy(CLAIM_BOUNDARY),
        "correction_reason": copy.deepcopy(CORRECTION_REASON),
        "document_id": "WS-FP048-R002-SEQ93-94-REVIEW-ASSIGNMENT-20260826-R003",
        "executor": {
            "id": "codex-fp048-r002-seq93-branch-semantics-executor-20260826",
            "task_id": "/root/seq93_branch_reanchor_impl",
        },
        "goal_id": GOAL_ID,
        "passed_gate_attempt_003_binding": passed_gate_attempt_003_binding(root),
        "previous_contract_binding": r004_contract_binding(root),
        "projected_transition": copy.deepcopy(PROJECTED_TRANSITION),
        "replacement_contract_binding": r005_contract_binding(root),
        "required_reviewer": copy.deepcopy(REVIEWER),
        "review_scope": [
            "exact published seq92 source CAS and source transition review binding",
            "exact private 0700/0600 R004 PASS six-file evidence inventory",
            "R004 PASS is append-only sealed as PASS_UNCONSUMED",
            "logical workstream current label is separated from the physical Git branch",
            "R004-to-R005 append-only contract supersession",
            "seq93 READY-to-READY transition with every credit delta zero",
            "exact seq93-to-seq92 byte reconstruction",
            "preconsumer, postconsumer, and commit-point cohort reseal",
            "private 0600 checkpoint mode before and after publication",
            "conditional seq94 start only after fresh R005 five-check PASS receipt",
        ],
        "reviewed_control_inputs": reviewed,
        "round_id": ROUND_ID,
        "schema_version": "1.0",
        "source_checkpoint_binding": _source_checkpoint_binding(
            passed_gate_attempt_003_binding(root)
        ),
        "supersedes": _r002_stale_assignment_binding(root),
    }
    return seq90.canonical_json_bytes(value)


def _load_physical_review(
    root: Path,
    source: Mapping[str, Any],
) -> tuple[dict[str, dict[str, Any]], datetime, datetime]:
    assignment_raw = seq90._stable_read(root, REVIEW_ASSIGNMENT_REL).raw
    result_raw = seq90._stable_read(root, REVIEW_RESULT_REL).raw
    independent_raw = seq90._stable_read(root, INDEPENDENT_REVIEW_REL).raw
    require(
        assignment_raw == build_review_assignment(root, source),
        "seq93 review assignment differs",
    )
    assignment = seq90.strict_json(assignment_raw, REVIEW_ASSIGNMENT_REL.as_posix())
    result = seq90.strict_json(result_raw, REVIEW_RESULT_REL.as_posix())
    independent = seq90.strict_json(independent_raw, INDEPENDENT_REVIEW_REL.as_posix())
    require(
        set(result) == REVIEW_RESULT_FIELDS
        and set(independent) == INDEPENDENT_REVIEW_FIELDS
        and result_raw == seq90.canonical_json_bytes(result)
        and independent_raw == seq90.canonical_json_bytes(independent),
        "seq93 review documents are noncanonical",
    )
    assignment_binding_value = _binding(REVIEW_ASSIGNMENT_REL, assignment_raw)
    result_binding_value = _binding(REVIEW_RESULT_REL, result_raw)
    common = {
        "claim_boundary": CLAIM_BOUNDARY,
        "correction_reason": CORRECTION_REASON,
        "goal_id": GOAL_ID,
        "passed_gate_attempt_003_binding": passed_gate_attempt_003_binding(root),
        "previous_contract_binding": r004_contract_binding(root),
        "projected_transition": PROJECTED_TRANSITION,
        "replacement_contract_binding": r005_contract_binding(root),
        "reviewer": REVIEWER,
        "round_id": ROUND_ID,
        "schema_version": "1.0",
    }
    require(
        assignment.get("document_id")
        == "WS-FP048-R002-SEQ93-94-REVIEW-ASSIGNMENT-20260826-R003"
        and assignment.get("required_reviewer") == REVIEWER
        and assignment.get("supersedes") == _r002_stale_assignment_binding(root)
        and result.get("document_id")
        == "WS-FP048-R002-SEQ93-94-REVIEW-RESULT-20260826-R003"
        and independent.get("document_id")
        == "WS-FP048-R002-SEQ93-94-INDEPENDENT-REVIEW-20260826-R003"
        and result.get("assignment_binding") == assignment_binding_value
        and independent.get("assignment_binding") == assignment_binding_value
        and independent.get("review_result_binding") == result_binding_value
        and result.get("decision") == "APPROVE_INTERNAL_ZERO_CREDIT_CONTROL_ONLY"
        and independent.get("decision") == "CONCUR_INTERNAL_ZERO_CREDIT_CONTROL_ONLY"
        and result.get("findings") == []
        and independent.get("findings") == []
        and result.get("external_independence_claimed") is False
        and independent.get("external_independence_claimed") is False
        and independent.get("independent_checks") == INDEPENDENT_CHECKS
        and all(result.get(key) == value for key, value in common.items())
        and all(independent.get(key) == value for key, value in common.items()),
        "seq93 review approval differs",
    )
    result_at = _parse_time(result.get("reviewed_at"), "seq93 result reviewed_at")
    independent_at = _parse_time(
        independent.get("reviewed_at"), "seq93 independent reviewed_at"
    )
    source_at = _parse_time(
        source["goal_execution"]["transition_history"][-1]["occurred_at"],
        "seq92 occurred_at",
    )
    require(
        result_at > source_at and independent_at > result_at,
        "seq93 review time order differs",
    )
    return (
        {
            "assignment": assignment_binding_value,
            "review_result": result_binding_value,
            "independent_review": _binding(INDEPENDENT_REVIEW_REL, independent_raw),
        },
        result_at,
        independent_at,
    )


def _event_time(
    source: Mapping[str, Any],
    supplied: str | None,
    independent_reviewed_at: datetime,
    gate_generated_at: datetime,
) -> str:
    candidate = (
        _parse_time(supplied, "seq93 occurred_at")
        if supplied is not None
        else datetime.now().astimezone().replace(microsecond=0)
    )
    source_time = _parse_time(
        source["goal_execution"]["transition_history"][-1]["occurred_at"],
        "seq92 occurred_at",
    )
    return max(
        candidate,
        source_time + timedelta(seconds=1),
        independent_reviewed_at + timedelta(seconds=1),
        gate_generated_at + timedelta(seconds=1),
    ).isoformat()


def _require_seq93_event_chronology(
    source: Mapping[str, Any],
    event: Mapping[str, Any],
    gate_receipt: Mapping[str, Any],
    review_result_at: datetime,
    independent_review_at: datetime,
) -> None:
    source_at = _parse_time(
        source["goal_execution"]["transition_history"][-1]["occurred_at"],
        "seq92 occurred_at",
    )
    event_at = _parse_time(event.get("occurred_at"), "seq93 occurred_at")
    execution_window = gate_receipt.get("execution_window")
    require(isinstance(execution_window, dict), "R004 PASS execution window differs")
    gate_started_at = _parse_time(
        execution_window.get("started_at"), "R004 PASS started_at"
    )
    gate_ended_at = _parse_time(
        execution_window.get("ended_at"), "R004 PASS ended_at"
    )
    gate_generated_at = _parse_time(
        gate_receipt.get("generated_at"), "R004 PASS generated_at"
    )
    require(
        source_at < gate_started_at <= gate_ended_at < gate_generated_at,
        "R004 PASS receipt chronology differs",
    )
    require(
        gate_ended_at < event_at and gate_generated_at < event_at,
        "seq93 event predates sealed R004 PASS receipt",
    )
    require(
        source_at
        < gate_generated_at
        < review_result_at
        < independent_review_at
        < event_at,
        "seq93 event and physical review chronology differs",
    )


def _final_managed_paths(
    source: Mapping[str, Any], visible: Sequence[str]
) -> tuple[str, ...]:
    paths = set(source["working_tree_snapshot"]["managed_changed_paths"])
    paths.update(
        path
        for path in visible
        if not path.startswith("docs/control/execution/goal-gates/")
    )
    paths.discard(CHECKPOINT_REL.as_posix())
    paths.update(path.as_posix() for path in REVIEWED_CONTROL_PATHS)
    paths.update(path.as_posix() for path in REVIEW_PATHS)
    return tuple(sorted(paths))


def project_seq93(
    root: Path,
    source: Mapping[str, Any],
    *,
    managed_paths: Sequence[str],
    path_set_sha256: str,
    content_set_sha256: str,
    occurred_at: str,
    authorization_binding_value: Mapping[str, Any],
    review_binding: Mapping[str, Any],
    passed_gate_binding: Mapping[str, Any],
    replacement_contract_binding: Mapping[str, Any],
    replacement_runner_binding: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    del root
    source_event = source["goal_execution"]["transition_history"][-1]
    paths = sorted(set(managed_paths))
    require(
        paths == list(managed_paths)
        and all(
            isinstance(path, str)
            and path
            and not Path(path).is_absolute()
            and ".." not in Path(path).parts
            for path in paths
        )
        and path_set_sha256
        == hashlib.sha256(("\n".join(paths) + "\n").encode("utf-8")).hexdigest()
        and seq90.SHA256_RE.fullmatch(content_set_sha256) is not None,
        "seq93 managed paths differ",
    )
    require(
        dict(passed_gate_binding) == _stored_passed_gate_attempt_003_binding(),
        "seq93 PASS_UNCONSUMED binding differs",
    )
    projected = copy.deepcopy(source)
    current = projected["current_work"]
    current["status"] = "READY"
    current["current_focus"] = CURRENT_FOCUS
    current["next_action"] = NEXT_ACTION
    current["release_completion_claimed"] = False
    snapshot = projected["working_tree_snapshot"]
    snapshot.update(
        {
            "scope": SCOPE,
            "managed_changed_paths": paths,
            "managed_changed_path_count": len(paths),
            "path_set_sha256": path_set_sha256,
            "content_set_sha256": content_set_sha256,
        }
    )
    handoff = projected["session_handoff"]
    require(handoff.get("branch") == LOGICAL_BRANCH, "logical branch label differs")
    handoff["changed_files"] = copy.deepcopy(paths)
    mirror = handoff["source_commit_or_snapshot"]
    mirror["file_count"] = len(paths)
    mirror["path_set_sha256"] = path_set_sha256
    mirror["content_set_sha256"] = content_set_sha256
    handoff["current_epic"] = HANDOFF_EPIC
    handoff["last_updated_by_work_item"] = WORK_ITEM_ID
    handoff["last_verification_status"] = VERIFICATION_STATUS
    handoff["next_single_action"] = NEXT_ACTION
    source_binding = _source_checkpoint_binding(passed_gate_binding)
    event: dict[str, Any] = {
        "sequence": REANCHOR_SEQUENCE,
        "event_id": REANCHOR_EVENT_ID,
        "event_type": REANCHOR_EVENT_TYPE,
        "occurred_on": _parse_time(occurred_at, "seq93 occurred_at").date().isoformat(),
        "occurred_at": occurred_at,
        "previous_focus_goal_id": GOAL_ID,
        "previous_focus_content_sha256": GOAL_SHA256,
        "focus_goal_id": GOAL_ID,
        "focus_goal_content_sha256": GOAL_SHA256,
        "subject_goal_id": GOAL_ID,
        "from_status": "READY",
        "to_status": "READY",
        "static_plan_manifest_sha256": MANIFEST_SHA256,
        "status_changes": {},
        "runtime_after": copy.deepcopy(source_event["runtime_after"]),
        "blockers_after": copy.deepcopy(source_event["blockers_after"]),
        "blocker_resolution_ids_after": copy.deepcopy(
            source_event["blocker_resolution_ids_after"]
        ),
        "source_checkpoint_version": source["schema_version"],
        "evidence_refs": [
            "FP048-R002_SEQ92_R004_CONTRACT_CORRECTION_SOURCE",
            "FP048-R002_R004_PASS_SIX_FILE_EVIDENCE_SEALED_UNCONSUMED",
            "FP048-R002_LOGICAL_PHYSICAL_BRANCH_SEMANTICS_SEPARATED",
            "FP048-R002_R005_BRANCH_AWARE_GATE_CONTRACT",
            "FP048-R002_SEQ93_94_TRANSITION_CONTROL_REVIEW",
        ],
        "source_checkpoint_binding": source_binding,
        "source_ready_event_binding": copy.deepcopy(
            source_event["source_ready_event_binding"]
        ),
        "authorization_binding": copy.deepcopy(dict(authorization_binding_value)),
        "contract_supersession": {
            "previous_contract_binding": copy.deepcopy(
                source_event["contract_supersession"]["replacement_contract_binding"]
            ),
            "reason_code": R005_SUCCESSOR_REASON_CODE,
            "replacement_contract_binding": copy.deepcopy(
                dict(replacement_contract_binding)
            ),
        },
        "start_gate_runner_binding": copy.deepcopy(dict(replacement_runner_binding)),
        "transition_control_review_binding": copy.deepcopy(dict(review_binding)),
        "repository_context_reanchor": {
            "before": {
                **copy.deepcopy(source_binding),
                "current_work": copy.deepcopy(source["current_work"]),
                "working_tree_snapshot": copy.deepcopy(
                    source["working_tree_snapshot"]
                ),
                "session_handoff": copy.deepcopy(source["session_handoff"]),
            },
            "after": {
                "base_commit": source["working_tree_snapshot"]["base_head"],
                "branch": LOGICAL_BRANCH,
                "logical_branch": LOGICAL_BRANCH,
                "logical_branch_semantics": LOGICAL_BRANCH_SEMANTICS,
                "physical_git_branch": PHYSICAL_GIT_BRANCH,
                "branch_mismatch_reason_code": BRANCH_MISMATCH_REASON_CODE,
                "current_head": source["session_handoff"][
                    "source_commit_or_snapshot"
                ]["current_head"],
                "managed_changed_path_count": len(paths),
                "path_set_sha256": path_set_sha256,
                "content_set_sha256": content_set_sha256,
            },
        },
        "noncredit_successor_edges": copy.deepcopy(
            source_event["noncredit_successor_edges"]
        ),
        "claim_boundary": copy.deepcopy(CLAIM_BOUNDARY),
        "unchanged_control_projection": seq92.correction._unchanged_projection(
            projected
        ),
        "canonical_binding_snapshot_after": copy.deepcopy(
            source_event["canonical_binding_snapshot_after"]
        ),
        "correction_reason": copy.deepcopy(CORRECTION_REASON),
        "previous_event_sha256": SOURCE_EVENT_SHA256,
    }
    event["event_sha256"] = continuation.event_sha256(event)
    require(set(event) == EVENT_FIELDS, "seq93 reanchor event field set differs")
    state = projected["goal_execution"]
    state["transition_history"].append(event)
    state["transition_history_anchor_sha256"] = event["event_sha256"]
    state["validation_cutoff_at"] = occurred_at
    require(
        state["goal_status"] == "READY"
        and state["status_by_goal"][GOAL_ID] == "READY"
        and current["status"] == "READY"
        and "IN_PROGRESS" not in state["status_by_goal"].values(),
        "seq93 READY state differs",
    )
    for field in (
        "approved_state",
        "authority_boundary",
        "canonical_bindings",
        "verification_boundary",
    ):
        require(
            _strict_json_equal(projected[field], source[field]),
            f"seq93 changed {field}",
        )
    require(
        all(
            type(value) is int and value == 0
            for key, value in CLAIM_BOUNDARY.items()
            if key.endswith("_credit_delta") or key == "goal_status_change_count"
        )
        and CLAIM_BOUNDARY["implementation_start_authorized"] is False,
        "seq93 credit boundary differs",
    )
    return projected, event


def _repository_before(event: Mapping[str, Any]) -> dict[str, Any]:
    context = event.get("repository_context_reanchor")
    before = context.get("before") if isinstance(context, dict) else None
    after = context.get("after") if isinstance(context, dict) else None
    source_binding = _source_checkpoint_binding()
    require(
        isinstance(context, dict)
        and set(context) == {"before", "after"}
        and isinstance(before, dict)
        and set(before)
        == set(source_binding)
        | {"current_work", "working_tree_snapshot", "session_handoff"}
        and {key: before.get(key) for key in source_binding} == source_binding
        and isinstance(before.get("current_work"), dict)
        and isinstance(before.get("working_tree_snapshot"), dict)
        and isinstance(before.get("session_handoff"), dict)
        and isinstance(after, dict)
        and set(after)
        == {
            "base_commit",
            "branch",
            "logical_branch",
            "logical_branch_semantics",
            "physical_git_branch",
            "branch_mismatch_reason_code",
            "current_head",
            "managed_changed_path_count",
            "path_set_sha256",
            "content_set_sha256",
        }
        and after.get("branch") == after.get("logical_branch") == LOGICAL_BRANCH
        and after.get("logical_branch_semantics") == LOGICAL_BRANCH_SEMANTICS
        and after.get("physical_git_branch") == PHYSICAL_GIT_BRANCH
        and after.get("branch_mismatch_reason_code")
        == BRANCH_MISMATCH_REASON_CODE,
        "seq93 repository context differs",
    )
    return before


def _restored_seq92_checkpoint(checkpoint: Mapping[str, Any]) -> dict[str, Any]:
    state = checkpoint.get("goal_execution")
    history = state.get("transition_history") if isinstance(state, dict) else None
    require(
        isinstance(history, list) and len(history) == REANCHOR_SEQUENCE,
        "seq92 inverse source is not exact seq93",
    )
    before = _repository_before(history[-1])
    restored = copy.deepcopy(checkpoint)
    restored_state = restored["goal_execution"]
    source_event = restored_state["transition_history"][SOURCE_SEQUENCE - 1]
    restored_state["transition_history"] = restored_state["transition_history"][
        :SOURCE_SEQUENCE
    ]
    restored_state["transition_history_anchor_sha256"] = source_event["event_sha256"]
    restored_state["validation_cutoff_at"] = source_event["occurred_at"]
    restored["current_work"] = copy.deepcopy(before["current_work"])
    restored["working_tree_snapshot"] = copy.deepcopy(before["working_tree_snapshot"])
    restored["session_handoff"] = copy.deepcopy(before["session_handoff"])
    return restored


def require_branch_semantics_reanchored_checkpoint(
    root: Path,
    checkpoint: Mapping[str, Any],
    *,
    run_external_validators: bool = False,
    require_live_snapshot: bool = True,
) -> None:
    root = seq90._safe_root(root)
    state = checkpoint.get("goal_execution")
    history = state.get("transition_history") if isinstance(state, dict) else None
    require(
        isinstance(history, list) and len(history) == REANCHOR_SEQUENCE,
        "checkpoint is not exact seq93 branch-semantics reanchor",
    )
    source_event = history[SOURCE_SEQUENCE - 1]
    event = history[REANCHOR_SEQUENCE - 1]
    gate_receipt, gate_binding = _gate_receipt_and_binding(root)
    require(
        isinstance(event, dict)
        and set(event) == EVENT_FIELDS
        and source_event.get("event_id") == SOURCE_EVENT_ID
        and source_event.get("event_sha256") == SOURCE_EVENT_SHA256
        and event.get("sequence") == REANCHOR_SEQUENCE
        and event.get("event_id") == REANCHOR_EVENT_ID
        and event.get("event_type") == REANCHOR_EVENT_TYPE
        and event.get("subject_goal_id") == GOAL_ID
        and event.get("from_status") == event.get("to_status") == "READY"
        and event.get("status_changes") == {}
        and event.get("previous_event_sha256") == SOURCE_EVENT_SHA256
        and event.get("event_sha256") == continuation.event_sha256(event)
        and event.get("source_checkpoint_binding")
        == _source_checkpoint_binding(gate_binding)
        and event.get("correction_reason") == CORRECTION_REASON
        and event.get("claim_boundary") == CLAIM_BOUNDARY,
        "seq93 reanchor authority differs",
    )
    before = _repository_before(event)
    source_raw = checkpoint_json_bytes(_restored_seq92_checkpoint(checkpoint))
    require_exact_seq92_source(
        source_raw,
        seq90.strict_json(source_raw, "restored seq92 source"),
        root,
    )
    source = seq90.strict_json(source_raw, "restored seq92 source")
    review_binding, review_result_at, independent_review_at = _load_physical_review(
        root, source
    )
    _require_seq93_event_chronology(
        source,
        event,
        gate_receipt,
        review_result_at,
        independent_review_at,
    )
    snapshot = checkpoint.get("working_tree_snapshot")
    paths = snapshot.get("managed_changed_paths") if isinstance(snapshot, dict) else None
    require(isinstance(paths, list), "seq93 managed paths differ")
    expected_checkpoint, expected_event = project_seq93(
        root,
        source,
        managed_paths=paths,
        path_set_sha256=snapshot.get("path_set_sha256"),
        content_set_sha256=snapshot.get("content_set_sha256"),
        occurred_at=event.get("occurred_at"),
        authorization_binding_value=authorization_binding(root, source),
        review_binding=review_binding,
        passed_gate_binding=gate_binding,
        replacement_contract_binding=r005_contract_binding(root),
        replacement_runner_binding=r005_runner_binding(root),
    )
    require(
        _strict_json_equal(event, expected_event)
        and _strict_json_equal(checkpoint, expected_checkpoint),
        "seq93 exact producer projection differs",
    )
    require(
        event.get("contract_supersession")
        == {
            "previous_contract_binding": r004_contract_binding(root),
            "reason_code": R005_SUCCESSOR_REASON_CODE,
            "replacement_contract_binding": r005_contract_binding(root),
        }
        and event.get("start_gate_runner_binding") == r005_runner_binding(root)
        and event.get("runtime_after") == source_event.get("runtime_after")
        and event.get("blockers_after") == source_event.get("blockers_after")
        and event.get("blocker_resolution_ids_after")
        == source_event.get("blocker_resolution_ids_after")
        and event.get("noncredit_successor_edges")
        == source_event.get("noncredit_successor_edges"),
        "seq93 successor authority differs",
    )
    require(
        all(
            isinstance(item, dict)
            and item.get("sequence") == sequence
            and item.get("event_sha256") == continuation.event_sha256(item)
            and (
                sequence == 1
                or item.get("previous_event_sha256")
                == history[sequence - 2].get("event_sha256")
            )
            for sequence, item in enumerate(history, start=1)
        ),
        "seq1-93 lineage differs",
    )
    current = checkpoint.get("current_work")
    handoff = checkpoint.get("session_handoff")
    mirror = handoff.get("source_commit_or_snapshot") if isinstance(handoff, dict) else None
    after = event["repository_context_reanchor"]["after"]
    require(
        state.get("goal_status") == "READY"
        and state.get("status_by_goal", {}).get(GOAL_ID) == "READY"
        and "IN_PROGRESS" not in state.get("status_by_goal", {}).values()
        and state.get("transition_history_anchor_sha256") == event["event_sha256"]
        and state.get("validation_cutoff_at") == event["occurred_at"]
        and isinstance(current, dict)
        and current.get("status") == "READY"
        and current.get("current_focus") == CURRENT_FOCUS
        and current.get("next_action") == NEXT_ACTION
        and current.get("release_completion_claimed") is False
        and isinstance(snapshot, dict)
        and paths == sorted(set(paths))
        and snapshot.get("scope") == SCOPE
        and snapshot.get("managed_changed_path_count") == len(paths)
        and snapshot.get("managed_changed_path_count")
        == after.get("managed_changed_path_count")
        and snapshot.get("path_set_sha256") == after.get("path_set_sha256")
        and snapshot.get("content_set_sha256") == after.get("content_set_sha256")
        and isinstance(handoff, dict)
        and handoff.get("branch") == LOGICAL_BRANCH
        and handoff.get("changed_files") == paths
        and handoff.get("current_epic") == HANDOFF_EPIC
        and handoff.get("last_updated_by_work_item") == WORK_ITEM_ID
        and handoff.get("last_verification_status") == VERIFICATION_STATUS
        and handoff.get("next_single_action") == NEXT_ACTION
        and isinstance(mirror, dict)
        and mirror.get("file_count") == len(paths)
        and mirror.get("path_set_sha256") == snapshot.get("path_set_sha256")
        and mirror.get("content_set_sha256") == snapshot.get("content_set_sha256"),
        "seq93 READY projection differs",
    )
    for field in (
        "approved_state",
        "authority_boundary",
        "canonical_bindings",
        "verification_boundary",
    ):
        require(
            _strict_json_equal(checkpoint.get(field), source.get(field)),
            f"seq93 changed {field}",
        )
    require(
        checkpoint.get("approved_state", {}).get("formal_test_not_run_count") == 279
        and checkpoint.get("verification_boundary", {}).get("release_eligible") is False
        and before["current_work"]["status"] == "READY",
        "seq93 gained formal, deployment, or release credit",
    )
    if require_live_snapshot:
        require(
            continuation.working_snapshot_hashes(root, paths)
            == (snapshot["path_set_sha256"], snapshot["content_set_sha256"]),
            "seq93 managed snapshot differs",
        )
    if run_external_validators:
        seq90._validate_projected_with_consumers(root, checkpoint)


def reconstructed_seq92_checkpoint_bytes(
    root: Path,
    checkpoint: Mapping[str, Any],
) -> bytes:
    history = checkpoint.get("goal_execution", {}).get("transition_history")
    if isinstance(history, list) and len(history) == SOURCE_SEQUENCE:
        raw = checkpoint_json_bytes(checkpoint)
        require_exact_seq92_source(raw, checkpoint, root)
        return raw
    require_branch_semantics_reanchored_checkpoint(
        root, checkpoint, require_live_snapshot=False
    )
    raw = checkpoint_json_bytes(_restored_seq92_checkpoint(checkpoint))
    require(
        len(raw) == SOURCE_CHECKPOINT_BYTE_LENGTH
        and sha256_bytes(raw) == SOURCE_CHECKPOINT_SHA256,
        "seq93 inverse does not reconstruct exact seq92",
    )
    return raw


def canonical_seq93_checkpoint_bytes(
    root: Path,
    checkpoint: Mapping[str, Any],
) -> bytes:
    require_branch_semantics_reanchored_checkpoint(
        root, checkpoint, require_live_snapshot=False
    )
    return checkpoint_json_bytes(checkpoint)


def reconstructed_seq93_checkpoint_bytes(
    root: Path,
    checkpoint: Mapping[str, Any],
) -> bytes:
    history = checkpoint.get("goal_execution", {}).get("transition_history")
    require(
        isinstance(history, list) and len(history) == REANCHOR_SEQUENCE,
        "seq94 inverse is owned by the seq94 starter",
    )
    return canonical_seq93_checkpoint_bytes(root, checkpoint)


@dataclass(frozen=True)
class Prepared:
    transport: seq90.Prepared

    @property
    def projected(self) -> Mapping[str, Any]:
        return self.transport.projected

    @property
    def event(self) -> Mapping[str, Any]:
        return self.transport.event


_AFTER_CAPTURE_HOOK: Callable[[Path], None] | None = None


def _require_private_checkpoint_mode(root: Path, phase: str) -> None:
    mode = stat.S_IMODE((root / CHECKPOINT_REL).lstat().st_mode)
    require(mode == 0o600, f"checkpoint mode differs {phase}")


def _require_prepared_exact(prepared: Prepared) -> None:
    transport = prepared.transport
    _require_private_checkpoint_mode(transport.root, "during seq93 preflight reseal")
    passed_gate_attempt_003_binding(transport.root)
    seq90._require_preflight_cohort_unchanged(
        transport.root,
        source=seq90.ReadResult(transport.source_raw, transport.source_identity),
        retained=transport.retained_inputs,
        managed=transport.managed_inputs,
        git_visible=transport.git_visible_inputs,
        git_status_raw=transport.git_status_raw,
        git_head=transport.git_head,
        git_branch=transport.git_branch,
        phase="seq93 branch-semantics commit guard",
    )


def _without_seq90_writer_temporary_status(observed: bytes, expected: bytes) -> bytes:
    if observed == expected:
        return observed
    prefix = (
        b"?? "
        + CHECKPOINT_REL.parent.as_posix().encode("utf-8")
        + b"/.walksafe-fp048-r002-seq90-write."
    )
    records = observed.split(b"\0")
    candidates: list[bytes] = []
    for index, record in enumerate(records):
        if record.startswith(prefix) and record.endswith(b".json"):
            candidate = b"\0".join(records[:index] + records[index + 1 :])
            if candidate == expected:
                candidates.append(candidate)
    require(len(candidates) == 1, "Git-visible cohort changed at seq93 commit guard")
    return candidates[0]


def _require_commit_exact(prepared: Prepared) -> None:
    transport = prepared.transport
    root = transport.root
    _require_private_checkpoint_mode(root, "at seq93 commit guard")
    passed_gate_attempt_003_binding(root)
    seq90._require_inputs_unchanged(root, transport.retained_inputs)
    seq90._require_managed_inputs_unchanged(root, transport.managed_inputs)
    seq90._require_managed_inputs_unchanged(
        root, transport.git_visible_inputs, label="full Git-visible input"
    )
    seq90._require_git_context(root, transport.git_head, transport.git_branch)
    source = seq90._stable_read(root, CHECKPOINT_REL)
    require(
        source.identity == transport.source_identity and source.raw == transport.source_raw,
        "source changed at seq93 commit guard",
    )
    status_raw, _visible = seq90.capture_git_visible_paths(root)
    require(
        _without_seq90_writer_temporary_status(status_raw, transport.git_status_raw)
        == transport.git_status_raw,
        "Git-visible cohort changed at seq93 commit guard",
    )


def prepare(
    root: Path = ROOT,
    *,
    occurred_at: str | None = None,
    validate_consumers: bool = True,
) -> Prepared:
    root = seq90._safe_root(root)
    source_read = seq90._stable_read(root, CHECKPOINT_REL)
    _require_private_checkpoint_mode(root, "before seq93 preflight")
    source = seq90.strict_json(source_read.raw, CHECKPOINT_REL.as_posix())
    require_exact_seq92_source(source_read.raw, source, root)
    receipt, gate_binding = _gate_receipt_and_binding(root)
    replacement = r005_contract_binding(root)
    git_head, git_branch = seq90._capture_git_context(root)
    expected_head = source["session_handoff"]["source_commit_or_snapshot"][
        "current_head"
    ]
    require(git_head == expected_head, "live Git HEAD differs from seq92 source")
    require(
        git_branch == PHYSICAL_GIT_BRANCH,
        "live Git branch differs from sealed R004 physical branch",
    )
    status_raw, visible = seq90.capture_git_visible_paths(root)
    paths = _final_managed_paths(source, visible)
    managed = seq90._capture_managed_inputs(root, paths)
    visible_inputs = seq90._capture_managed_inputs(root, visible)
    path_sha256, content_sha256 = seq90._managed_input_snapshot_hashes(managed)
    retained_paths = tuple(
        sorted(
            set(REVIEWED_CONTROL_PATHS)
            | set(REVIEW_PATHS)
            | {
                UNCONSUMED_GATE_DIR_REL / name
                for name, _digest, _length in UNCONSUMED_GATE_FILE_PINS
            }
        )
    )
    retained = {path: seq90._stable_read(root, path) for path in retained_paths}
    authorization = authorization_binding(root, source)
    review, _review_result_at, independent_at = _load_physical_review(root, source)
    gate_generated_at = _parse_time(receipt.get("generated_at"), "R004 gate generated_at")
    projected, event = project_seq93(
        root,
        source,
        managed_paths=paths,
        path_set_sha256=path_sha256,
        content_set_sha256=content_sha256,
        occurred_at=_event_time(
            source, occurred_at, independent_at, gate_generated_at
        ),
        authorization_binding_value=authorization,
        review_binding=review,
        passed_gate_binding=gate_binding,
        replacement_contract_binding=replacement,
        replacement_runner_binding=r005_runner_binding(root),
    )
    transport = seq90.Prepared(
        root=root,
        source_raw=source_read.raw,
        source_identity=source_read.identity,
        source=source,
        projected=projected,
        projected_raw=checkpoint_json_bytes(projected),
        event=event,
        retained_inputs=retained,
        managed_inputs=managed,
        git_visible_inputs=visible_inputs,
        git_status_raw=status_raw,
        git_head=git_head,
        git_branch=git_branch,
    )
    prepared = Prepared(transport)
    if _AFTER_CAPTURE_HOOK is not None:
        _AFTER_CAPTURE_HOOK(root)
    _require_prepared_exact(prepared)
    if validate_consumers:
        seq90._validate_projected_with_consumers(root, projected)
        _require_prepared_exact(prepared)
    return prepared


def write_checkpoint(prepared: Prepared) -> None:
    seq90.write_checkpoint(
        prepared.transport,
        commit_guard=lambda: _require_commit_exact(prepared),
    )
    try:
        _require_private_checkpoint_mode(
            prepared.transport.root, "after seq93 publication"
        )
    except BaseException as exc:
        raise seq90.PostcommitUncertain(
            "published seq93 checkpoint mode is uncertain"
        ) from exc


def prepare_review_manifest(root: Path = ROOT) -> dict[str, Any]:
    root = seq90._safe_root(root)
    _raw, source = load_exact_seq92_source(root)
    assignment = build_review_assignment(root, source)
    return {
        "authorization": authorization_binding(root, source),
        "candidate_review_assignment": _binding(REVIEW_ASSIGNMENT_REL, assignment),
        "reviewer_authored_paths": [
            REVIEW_RESULT_REL.as_posix(),
            INDEPENDENT_REVIEW_REL.as_posix(),
        ],
        "round_id": ROUND_ID,
        "source_checkpoint_binding": _source_checkpoint_binding(
            passed_gate_attempt_003_binding(root)
        ),
        "status": "PREPARED_NOT_PUBLISHED",
    }


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__, allow_abbrev=False)
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--prepare-authorization", action="store_true")
    mode.add_argument("--prepare-review", action="store_true")
    mode.add_argument("--preflight", action="store_true")
    mode.add_argument("--write", action="store_true")
    parser.add_argument("--root", type=Path, default=ROOT)
    parser.add_argument("--occurred-at")
    return parser.parse_args(argv)


def main(argv: Sequence[str] | None = None) -> int:
    args = parse_args(argv)
    published = False
    try:
        if args.prepare_authorization:
            _raw, source = load_exact_seq92_source(args.root)
            sys.stdout.buffer.write(build_authorization(args.root, source))
            return 0
        if args.prepare_review:
            sys.stdout.buffer.write(
                seq90.canonical_json_bytes(prepare_review_manifest(args.root))
            )
            return 0
        prepared = prepare(args.root, occurred_at=args.occurred_at)
        if args.write:
            write_checkpoint(prepared)
            published = True
        message = (
            "FP048-R002 seq93 branch-semantics reanchor: PASS "
            f"event_sha256={prepared.event['event_sha256']} "
            f"occurred_at={prepared.event['occurred_at']}\n"
        )
        try:
            sys.stdout.write(message)
            sys.stdout.flush()
        except BaseException as exc:
            if published:
                raise seq90.PostcommitUncertain(
                    "published seq93 PASS output delivery is uncertain"
                ) from exc
            raise
    except seq90.PostcommitUncertain as exc:
        print(
            f"FP048-R002 seq93 branch-semantics reanchor: POSTCOMMIT-UNCERTAIN: {exc}",
            file=sys.stderr,
        )
        return 2
    except (
        BranchSemanticsReanchorError,
        seq90.ControlReanchorError,
        KeyError,
        OSError,
        TypeError,
        ValueError,
    ) as exc:
        print(
            f"FP048-R002 seq93 branch-semantics reanchor: FAIL: {exc}",
            file=sys.stderr,
        )
        return 1
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
