#!/usr/bin/env python3
"""Prepare or append FP-048 R002 GOAL_STARTED seq92 after its R003 PASS gate."""
...
class StartApplyError(RuntimeError):
    """The exact seq91 correction and fresh R003 PASS gate do not authorize seq92."""


transport_reanchor = seq90
sha256_bytes = seq90.sha256_bytes
json_bytes = seq90.checkpoint_json_bytes
...
def require_exact_source(
    ...
    *,
    ...
    correction = _correction()
    try:
        correction.require_control_corrected_checkpoint(root, source)
        authority = gate.bind_corrected_source(root, source)
    except Exception as exc:
        raise StartApplyError(f"source seq91 correction authority differs: {exc}") from exc
    ...
    _require(
        ready.get("event_id") == correction.READY_EVENT_ID
    ...
    _require(
        ...
        "source seq91 correction differs",
    )
...
def _source_activation(source: dict[str, Any], digest: str) -> dict[str, Any]:
    ...
    _require(len(matches) == 1, "source seq91 trust anchor differs")
    return matches[0]
...
def _assert_zero_credit_projection(
    ...
    for key in (
        ...
        _require(
            ...
            f"seq92 changed zero-credit state: {key}",
        )
...
def project_seq92(
    ...
    *,
    ...
    _require(event_id == EVENT_ID, "seq92 event ID differs")
    ...
    _require(set(event) == EVENT_FIELDS, "seq92 GOAL_STARTED field set differs")
    ...
    if final_sha256_by_path is None:
        ...
        _require(
            ...
            "seq92 final managed path inventory differs",
        )
    ...
    require_exact_seq92_projection(
        root,
        checkpoint,
        state,
        event,
...
def require_exact_seq92_projection(
    root: Path,
    ...
    *,
    ...
    _require(
        ...
        "seq92 history position differs",
    )
    ...
    occurred_at = _parse_timestamp(
        event.get("occurred_at"), label="seq92 occurred_at", second_precision=True
    )
    _require(
    ...
    _require(
        ...
        == STARTED_VERIFICATION_STATUS
        and handoff.get("next_single_action") == correction.CORRECTION_NEXT_ACTION
...
def validate_history_suffix(root: Path, checkpoint: Mapping[str, Any]) -> list[str]:
    try:
        ...
        history = state.get("transition_history") if isinstance(state, dict) else None
        _require(isinstance(history, list) and len(history) >= EVENT_SEQUENCE, "seq92 is missing")
        control = history[EVENT_SEQUENCE - 2]
        event = history[EVENT_SEQUENCE - 1]
        _require(isinstance(control, dict) and isinstance(event, dict), "seq92 history differs")
        binding = event.get("implementation_start_gate_binding")
        _require(
        ...
        _require(
            ...
            "seq92 gate receipt file binding differs",
        )
        receipt = _strict_json(receipt_bytes, "seq92 PASS receipt")
        require_exact_seq92_projection(
            root,
            checkpoint,
            state,
        ...
        _require(
            ...
            "seq92 repository snapshot binding differs",
        )
        reconstructed_source = _correction().reconstructed_seq91_checkpoint_bytes(
            root, checkpoint
        )
        _require(
...
def validate_projected_checkpoint(
    ...
    *,
    ...
    errors = [f"seq92: {error}" for error in validate_history_suffix(root, checkpoint)]
    descriptor, temporary_name = tempfile.mkstemp(
        dir=root / CHECKPOINT_RELATIVE.parent,
        prefix=".walksafe-fp048-r002-seq92-preflight.",
        suffix=".json",
    )
...
def validate_gate_evidence(
    ...
    *,
    ...
    activation_at = _parse_timestamp(
        ...
        label="seq91 correction occurred_at",
        second_precision=True,
    )
    _require(activation_at <= started_at, "gate precedes seq91 correction")
...
def _require_context(
    ...
    _require(
        ...
        == RUNTIME_TRUST_CLOSURE_PATHS,
        "FP048-R002 R003 gate context differs",
    )
...
def _require_prepared_exact(
    ...
    *,
    ...
    _require(
        ...
        "R003 gate evidence changed or namespace membership differs",
    )
    ...
    seq90._require_preflight_cohort_unchanged(
        ...
        phase="during seq92 publication preflight",
    )
    refreshed = gate.load_gate_context(
        prepared.root,
        require_live_snapshot=False,
    )
    _require(refreshed == prepared.context, "R003 gate context changed")
    ...
    _require(not errors, "projected seq92 validation failed: " + "; ".join(errors))
    ...
    seq90._require_preflight_cohort_unchanged(
        ...
        git_head=transport.git_head,
        git_branch=transport.git_branch,
        phase="after seq92 projected consumer validation",
    )
    observed_gate_inputs = _capture_gate_inputs(
        prepared.root,
        prepared.context,
    ...
    _require(
        ...
        "R003 gate evidence changed after projected consumer validation",
    )
...
def _fresh_exact_projected_checkpoint(
    ...
    _require(observed_raw == authority_raw, "seq92 projected checkpoint raw bytes differ")
    projected = _strict_json(observed_raw, "seq92 projected checkpoint")
    _require(
        json_bytes(projected) == observed_raw
        and json_bytes(prepared.projected_checkpoint) == authority_raw,
        "seq92 projected checkpoint authority differs",
    )
    ...
    _require(
        ...
        "seq92 projected event authority differs",
    )
    return projected
...
def _require_private_checkpoint_authority(
    ...
    *,
    ...
    _require(
        ...
        f"seq92 checkpoint authority differs {phase}",
    )
...
def _write_projection_transport(
    ...
    *,
    ...
    _require(
        ...
        "seq92 checkpoint source differs before injected writer",
    )
    guard_called = False
    ...
    try:
        ...
        if not guard_called:
            raise seq90.PostcommitUncertain(
                "seq92 checkpoint writer returned without its commit guard"
            )
        ...
        if unchanged:
            ...
            raise StartApplyError(
                "seq92 checkpoint transport failed before replacement"
            ) from exc
        raise seq90.PostcommitUncertain(
            "seq92 checkpoint transport ended with an unclassified failure"
        ) from exc
...
def _verify_published_projection(prepared: PreparedProjection) -> None:
    try:
        ...
        _require(
            ...
            "published seq92 gate evidence changed or membership differs",
        )
        errors = validate_projected_checkpoint(
            prepared.root,
            projected_for_validation,
        )
        _require(not errors, "published seq92 validation failed: " + "; ".join(errors))
        ...
        _require(
            ...
            "published seq92 checkpoint changed during terminal consumers",
        )
        ...
        _require(
            ...
            "published seq92 gate evidence changed during terminal consumers",
        )
        ...
        raise seq90.PostcommitUncertain(
            "seq92 checkpoint was replaced but terminal validation failed"
        ) from exc
...
def prepare_projection(
    ...
    *,
    ...
    _require(git_head == expected_head, "live Git HEAD differs from seq91 source")
    ...
    _require(
        source_read.raw
        == correction.reconstructed_seq91_checkpoint_bytes(root, source),
        "source checkpoint is not the exact canonical seq91 correction",
    )
    live_errors = live_validator(root)
    _require(not live_errors, "live seq91 validation failed: " + "; ".join(live_errors))
    ...
    _require(refreshed == context, "R003 gate context changed during checker")
    return prepared
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.write:
            ...
            published = True
        message = (
            "FP048-R002 GOAL_STARTED seq92: PASS "
        ...
        print(f"FP048-R002 GOAL_STARTED seq92: POSTCOMMIT-UNCERTAIN: {exc}", file=sys.stderr)
        return 2
    except (
        StartApplyError,
        gate.GateError,
        ...
        print(f"FP048-R002 GOAL_STARTED seq92: FAIL: {exc}", file=sys.stderr)
        return 1
    return 0
