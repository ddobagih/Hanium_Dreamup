#!/usr/bin/env python3
"""Validate the final R007 completion-control review."""
...
ROUND_ID = "WS-NPC-SINGLE-ADMIN-RECOVERY-REVIEW-20260813-R007"
REVIEW_DIR_REL = trace.RESULT_DIR_REL / "review-rounds" / "R007"
...
FINDING_IDS = (
    "NPC-R007-PROJECTED-LINEAGE-P1-001",
    "NPC-R007-TEST-LAYER-P1-002",
    "NPC-R007-FROZEN-R003-CONSUMER-P1-003",
    "NPC-R007-FINAL-MANAGED-SUCCESSOR-P1-004",
)
SUPERSEDED_ASSIGNMENT_PINS = (
    (*r006.SUPERSEDED_ASSIGNMENT_PINS[0],),
    (*r006.SUPERSEDED_ASSIGNMENT_PINS[1],),
    (
        r006.REVIEW_ASSIGNMENT_REL,
        "39e4d01c853d8b4adb40448725180d9a83a71d89cb639add87b7493a36ab10b4",
        6_405,
        "R003 control hashes were not superseded in the final managed closure",
    ),
)
CONTROL_PATHS = (
...
BOUNDARY = r006.BOUNDARY
ReviewError = r006.ReviewError
require = r006.require
_raw = r006._raw
_document = r006._document
_binding = r006._binding
_identity = r006._identity
...
def _scope(context: ReviewContext) -> dict[str, Any]:
    return {
        ...
        "acceptance": {
            ...
            "r007_controls_must_supersede_r003_in_final_managed_closure": True,
...
def validate_assignment(
    ...
    require(raw == trace.json_text(assignment).encode(), "R007 assignment is noncanonical")
    require(
        set(assignment)
        == {
            "schema_version",
        ...
        == "NPC_R007_FOLLOWUP_REVIEW_ASSIGNMENT"
        and assignment.get("goal_id") == trace.GOAL_ID
        and assignment.get("round_id") == ROUND_ID,
        "R007 assignment identity differs",
    )
    trace._parse_time(assignment.get("assigned_at"), "R007 assigned_at")
    ...
    require(
        ...
        "R007 reviewer is not separate",
    )
    require(assignment.get("review_scope") == _scope(context), "R007 scope differs")
    require(assignment.get("review_boundary") == BOUNDARY, "R007 boundary differs")
...
def validate_review_result(
    ...
    require(raw == trace.json_text(result).encode(), "R007 result is noncanonical")
    require(
        set(result)
        == {
            "schema_version",
    ...
    require(
        ...
        "R007 assignment or reviewer binding differs",
    )
    require(
        trace._parse_time(result.get("reviewed_at"), "R007 reviewed_at")
        >= trace._parse_time(assignment.get("assigned_at"), "R007 assigned_at"),
        "R007 review predates assignment",
    )
    require(result.get("review_scope") == _scope(context), "R007 result scope differs")
    require(
        result.get("decision") == "APPROVED"
        and result.get("findings")
        == {"blocking": [], "major_open": [], "minor_open": []},
        "R007 review is not finding-free approved",
    )
    ...
    require(
        ...
        "R007 finding disposition inventory differs",
    )
    for row in dispositions:
        require(
            ...
            "R007 finding disposition differs",
        )
    require(result.get("review_boundary") == BOUNDARY, "R007 result boundary differs")
...
def build_independent_review(
    ...
    return trace.json_text(
        {
            "schema_version": "1.0",
            "evidence_type": "NPC_R007_INDEPENDENT_INTERNAL_REVIEW",
...
def validate_followup_authority(root: Path = ROOT) -> None:
    ...
    require(
        ...
        "R007 independent review differs",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"NPC R007 follow-up review: FAIL: {exc}")
        return 1
    print("NPC R007 follow-up review: PASS")
    return 0
