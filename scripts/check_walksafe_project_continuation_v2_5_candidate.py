#!/usr/bin/env python3
"""Validate the isolated, non-effective WalkSafe v2.5 continuation candidate."""
...
CANDIDATE_FILENAME = "check_walksafe_project_continuation_v2_5_candidate.py"
ACTIVE_FILENAME = "check_walksafe_project_continuation_v2_5.py"
if WRAPPER_PATH.is_symlink() or WRAPPER_PATH.name not in {
    CANDIDATE_FILENAME,
    ACTIVE_FILENAME,
}:
    raise RuntimeError("v2.5 continuation wrapper path is not exact/regular")
...
def validate(root: Path = REPO_ROOT, *, mode: str | None = None) -> None:
    ...
    if IS_CANDIDATE_PATH and selected == "QUICK_PRECHECK":
        validation.validate_quick_precheck(
            root,
            check_id="CONTINUATION_QUICK_V2_5",
        )
        return
...
def main() -> int:
    ...
    try:
        ...
        print(
            f"WalkSafe v2.5 candidate continuation: FAIL: {exc}",
            file=sys.stderr,
        )
        return 1
    print(
        "WalkSafe v2.5 continuation: PASS "
