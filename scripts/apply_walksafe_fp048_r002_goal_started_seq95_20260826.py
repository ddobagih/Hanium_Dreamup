#!/usr/bin/env python3
"""Prepare or append FP-048 R002 GOAL_STARTED seq95 after its R006 PASS gate."""
...
STARTED_SCOPE = (
    "Graph v2.4 through FP048-R002/GAP-057 GOAL_STARTED seq95 after exact "
    "seq94 zero-credit gate-contract correction and fresh R006 five-check internal "
...
class StartApplyError(RuntimeError):
    """The exact seq94 correction and fresh R006 PASS gate do not authorize seq95."""
...
def require_exact_source(
    ...
    *,
    ...
    try:
        ...
        raise StartApplyError(f"source seq94 correction authority differs: {exc}") from exc
    ...
    expected_contract_binding = correction.r006_contract_binding(root)
    _require(
    ...
    _require(
        ...
        "source seq94 correction differs",
    )
    _require(
        ...
        "source seq94 R006 contract binding differs",
    )
...
def _source_activation(source: dict[str, Any], digest: str) -> dict[str, Any]:
    ...
    _require(len(matches) == 1, "source seq94 trust anchor differs")
    return matches[0]
...
def _assert_zero_credit_projection(
    ...
    for key in (
        ...
        _require(
            ...
            f"seq95 changed zero-credit state: {key}",
        )
...
def project_seq95(
    ...
    *,
    ...
    _require(set(event) == EVENT_FIELDS, "seq95 GOAL_STARTED field set differs")
    ...
    if final_sha256_by_path is None:
        ...
        _require(
            ...
            "seq95 final managed path inventory differs",
        )
    ...
    _require(
        ...
        "seq1-94 history prefix changed",
    )
    for field in (
        "focus_goal_id",
        "focus_goal_path",
    ...
    _require_exact_seq95_projection(
        root,
        checkpoint,
        state,
        event,
...
def _require_exact_seq95_projection(
    root: Path,
    ...
    *,
    ...
    _require(
        ...
        "seq95 history position differs",
    )
    ...
    _require(
        ...
        "seq95 gate binding differs",
    )
    occurred_at = _parse_timestamp(
        event.get("occurred_at"), label="seq95 occurred_at", second_precision=True
    )
    _require(
    ...
    _require(
        ...
        "seq95 state projection differs",
    )
    correction = _correction()
    _require(
        isinstance(current, dict)
        ...
        and current.get("release_completion_claimed") is False,
        "seq95 current-work projection differs",
    )
    ...
    _require(
        ...
        "seq95 working-tree snapshot projection differs",
    )
    ...
    if require_live_snapshot:
        ...
        _require(
            ...
            "seq95 working-tree snapshot hashes differ",
        )
    ...
    _require(
        ...
        "seq95 session handoff projection differs",
    )
...
def _gate_receipt_authority(
    ...
    _require(
        ...
        "checkpoint is not exact seq95",
    )
    event = history[EVENT_SEQUENCE - 1]
    _require(isinstance(event, dict), "seq95 event differs")
    binding = event.get("implementation_start_gate_binding")
    ...
    _require(
        ...
        and isinstance(binding.get("file_sha256"), str)
        and gate._impl.SHA256_RE.fullmatch(binding["file_sha256"]) is not None,
        "seq95 gate receipt binding differs",
    )
    ...
    _require(
        ...
        "seq95 gate receipt mode or size differs",
    )
    receipt = _strict_json(receipt_read.raw, "seq95 PASS receipt")
    _require(
        receipt_read.raw == json_bytes(receipt),
        "seq95 PASS receipt bytes are not canonical",
    )
    _require(
        sha256_bytes(receipt_read.raw) == binding["file_sha256"],
        "seq95 gate receipt file binding differs",
    )
...
def goal_start_gate_receipt_binding(
    ...
    """Return a detached binding only after exact R006 receipt verification."""

    _event, _receipt, _raw, binding = _gate_receipt_authority(root, checkpoint)
    reconstructed_seq94_checkpoint_bytes(root, checkpoint)
    return binding


def _require_exact_r006_receipt(
    root: Path,
    source: dict[str, Any],
    source_raw: bytes,
    event: Mapping[str, Any],
    ...
    authority = gate.bind_published_contract_corrected_source(root, source)
    contract_binding = _correction().r006_contract_binding(root)
    _require(
        set(receipt) == gate._impl.RECEIPT_FIELDS,
        "seq95 R006 receipt field set differs",
    )
    expected_scalars = {
        "schema_version": "1.1",
    ...
    for field, expected in expected_scalars.items():
        _require(
            ...
            f"seq95 R006 receipt {field} differs",
        )
    _require(
        transport_reanchor.strict_json_equal(
    ...
    _require(
        ...
        "seq95 R006 receipt chronology differs",
    )
    ...
    _require(
        ...
        "seq95 R006 receipt check count differs",
    )
    ...
    for index, (check_id, run) in enumerate(
        ...
        _require(isinstance(run, dict), f"seq95 R006 check {index} differs")
        ...
        _require(
            ...
            f"seq95 R006 check {index} evidence mode or size differs",
        )
        executed_at = _parse_timestamp(
            run.get("executed_at"),
            label=f"R006 check {index} executed_at",
        ...
        _require(
            ...
            f"seq95 R006 check {index} authority differs",
        )
        ...
        if check_id == "REPOSITORY_STATE":
            repository_payload = _strict_json(
                output_read.raw,
                "seq95 sealed REPOSITORY_STATE log",
            )
            _require(
                output_read.raw
                == contract.canonical_json_bytes(repository_payload) + b"\n",
                "seq95 sealed REPOSITORY_STATE log is not canonical",
            )
    ...
    _require(
        ...
        "seq95 R006 evidence namespace differs",
    )
    _require(
        ...
        "seq95 R006 repository snapshot differs",
    )
    expected_receipt = {
        **expected_scalars,
    ...
    _require(
        ...
        "seq95 R006 receipt bytes differ from exact producer order",
    )
...
def _restored_seq94_checkpoint(checkpoint: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(isinstance(source_event, dict), "seq94 inverse source event differs")
...
def reconstructed_seq94_checkpoint_bytes(
    ...
    """Reconstruct and validate the exact seq94 source bound by the R006 receipt."""
    ...
    restored = _restored_seq94_checkpoint(checkpoint)
    correction = _correction()
    correction.require_contract_corrected_checkpoint(
        root,
        restored,
    ...
    canonical = correction.canonical_seq94_checkpoint_bytes(root, restored)
    reconstructed = correction.reconstructed_seq94_checkpoint_bytes(root, restored)
    _require(
        canonical == reconstructed,
        "restored seq94 canonical authority differs",
    )
    _require(
        receipt.get("source_checkpoint_sha256") == sha256_bytes(canonical),
        "seq95 gate receipt source checkpoint SHA-256 differs",
    )
    source = _strict_json(canonical, "restored seq94 checkpoint")
    _require_exact_r006_receipt(
        root,
        source,
        canonical,
        event,
...
def require_exact_seq95_projection(
    root: Path,
    checkpoint: Mapping[str, Any],
    *,
    require_live_snapshot: bool = True,
) -> None:
    """Require the exact single READY-to-IN_PROGRESS seq95 projection."""
    ...
    source_raw = reconstructed_seq94_checkpoint_bytes(root, checkpoint)
    source = _strict_json(source_raw, "reconstructed seq94 checkpoint")
    _correction().require_contract_corrected_checkpoint(
        root,
        source,
        run_external_validators=False,
        # seq94 is historical once the exact R006 namespace exists.  The
        # seq95 projection itself, above, owns current repository authority.
        require_live_snapshot=False,
    )
    authority = gate.bind_published_contract_corrected_source(root, source)
    _require(
        ...
        "seq95 R006 receipt authority differs",
    )
...
def require_started_checkpoint(
    ...
    *,
    ...
    """Public exact seq95 authority used by continuation and GoalGraph consumers."""

    require_exact_seq95_projection(
        root,
        checkpoint,
        require_live_snapshot=require_live_snapshot,
    )
    if run_external_validators:
        ...
        _require(
            not errors,
            "seq95 external consumer validation failed: " + "; ".join(errors),
        )
...
def validate_history_suffix(root: Path, checkpoint: Mapping[str, Any]) -> list[str]:
    try:
        state = checkpoint.get("goal_execution")
        history = state.get("transition_history") if isinstance(state, dict) else None
        _require(isinstance(history, list) and len(history) >= EVENT_SEQUENCE, "seq95 is missing")
        control = history[EVENT_SEQUENCE - 2]
        event = history[EVENT_SEQUENCE - 1]
        _require(isinstance(control, dict) and isinstance(event, dict), "seq95 history differs")
        binding = event.get("implementation_start_gate_binding")
        _require(
            isinstance(binding, dict)
            and set(binding) == {"document_id", "path", "file_sha256"}
        ...
        receipt = _strict_json(receipt_bytes, "seq95 PASS receipt")
        _require_exact_seq95_projection(
            root,
            checkpoint,
            state,
        ...
        _require(
            ...
            "seq95 repository snapshot binding differs",
        )
        reconstructed_source = reconstructed_seq94_checkpoint_bytes(
            root, checkpoint
        )
        _require(
...
def validate_projected_checkpoint(
    ...
    *,
    ...
    errors = [f"seq95: {error}" for error in validate_history_suffix(root, checkpoint)]
    descriptor, temporary_name = tempfile.mkstemp(
        dir=root / CHECKPOINT_RELATIVE.parent,
        prefix=".walksafe-fp048-r002-seq95-preflight.",
        suffix=".json",
    )
...
def validate_gate_evidence(
    ...
    *,
    ...
    activation_at = _parse_timestamp(
        ...
        label="seq94 correction occurred_at",
        second_precision=True,
    )
    _require(activation_at <= started_at, "gate precedes seq94 correction")
    runs = receipt.get("check_runs")
    _require(
...
def _require_context(
    ...
    _require(
        ...
        == RUNTIME_TRUST_CLOSURE_PATHS,
        "FP048-R002 R006 gate context differs",
    )
...
def _require_prepared_exact(
    ...
    *,
    ...
    _require(
        ...
        "R006 gate evidence changed or namespace membership differs",
    )
    ...
    seq90._require_preflight_cohort_unchanged(
        ...
        phase="during seq95 publication preflight",
    )
    refreshed = gate.load_gate_context(
        prepared.root,
        require_live_snapshot=False,
    )
    _require(refreshed == prepared.context, "R006 gate context changed")
    ...
    _require(not errors, "projected seq95 validation failed: " + "; ".join(errors))
    ...
    seq90._require_preflight_cohort_unchanged(
        ...
        git_head=transport.git_head,
        git_branch=transport.git_branch,
        phase="after seq95 projected consumer validation",
    )
    observed_gate_inputs = _capture_gate_inputs(
        prepared.root,
        prepared.context,
    ...
    _require(
        ...
        "R006 gate evidence changed after projected consumer validation",
    )
...
def _fresh_exact_projected_checkpoint(
    ...
    _require(observed_raw == authority_raw, "seq95 projected checkpoint raw bytes differ")
    projected = _strict_json(observed_raw, "seq95 projected checkpoint")
    _require(
        json_bytes(projected) == observed_raw
        and json_bytes(prepared.projected_checkpoint) == authority_raw,
        "seq95 projected checkpoint authority differs",
    )
    ...
    _require(
        ...
        "seq95 projected event authority differs",
    )
    return projected
...
def _require_private_checkpoint_authority(
    ...
    *,
    ...
    _require(
        ...
        f"seq95 checkpoint authority differs {phase}",
    )
...
def _write_projection_transport(
    ...
    *,
    ...
    _require(
        ...
        "seq95 checkpoint source differs before injected writer",
    )
    guard_called = False
    ...
    try:
        ...
        if not guard_called:
            raise seq90.PostcommitUncertain(
                "seq95 checkpoint writer returned without its commit guard"
            )
        ...
        if unchanged:
            ...
            raise StartApplyError(
                "seq95 checkpoint transport failed before replacement"
            ) from exc
        raise seq90.PostcommitUncertain(
            "seq95 checkpoint transport ended with an unclassified failure"
        ) from exc
...
def _verify_published_projection(prepared: PreparedProjection) -> None:
    try:
        ...
        _require(
            ...
            "published seq95 gate evidence changed or membership differs",
        )
        errors = validate_projected_checkpoint(
            prepared.root,
            projected_for_validation,
        )
        _require(not errors, "published seq95 validation failed: " + "; ".join(errors))
        ...
        _require(
            ...
            "published seq95 checkpoint changed during terminal consumers",
        )
        ...
        _require(
            ...
            "published seq95 gate evidence changed during terminal consumers",
        )
        ...
        raise seq90.PostcommitUncertain(
            "seq95 checkpoint was replaced but terminal validation failed"
        ) from exc
...
def prepare_projection(
    ...
    *,
    ...
    _require_private_checkpoint_authority(source_read, phase="before seq95 preflight")
    ...
    _require(git_head == expected_head, "live Git HEAD differs from seq94 source")
    ...
    _require(
        source_read.raw
        == correction.canonical_seq94_checkpoint_bytes(root, source),
        "source checkpoint is not the exact canonical seq94 correction",
    )
    live_errors = live_validator(root)
    _require(not live_errors, "live seq94 validation failed: " + "; ".join(live_errors))
    ...
    _require(refreshed == context, "R006 gate context changed during checker")
    return prepared
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.write:
            ...
            published = True
        message = (
            "FP048-R002 GOAL_STARTED seq95: PASS "
        ...
        print(f"FP048-R002 GOAL_STARTED seq95: POSTCOMMIT-UNCERTAIN: {exc}", file=sys.stderr)
        return 2
    except (
        StartApplyError,
        gate.GateError,
        ...
        print(f"FP048-R002 GOAL_STARTED seq95: FAIL: {exc}", file=sys.stderr)
        return 1
    return 0
